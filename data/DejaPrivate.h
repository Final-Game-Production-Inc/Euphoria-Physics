...
DEJA_F_DECL void        DejaTrace           ( const char*     pChannel, const char*    pFormat, ... );
DEJA_F_DECL void        DejaTrace           ( const wchar_t*  pChannel, const char*    pFormat, ... );
DEJA_F_DECL void        DejaTrace           ( const char*     pChannel, const wchar_t* pFormat, ... );
DEJA_F_DECL void        DejaTrace           ( const wchar_t*  pChannel, const wchar_t* pFormat, ... );

DEJA_F_DECL void        DejaTrace_VS7       ( deja_trace_reg* pTraceReg, 
                                                    int       TraceCode );

DEJA_F_DECL void        DejaTrace_VS7       ( const char*    pChannel, const char*    pFormat, ... );
DEJA_F_DECL void        DejaTrace_VS7       ( const wchar_t* pChannel, const char*    pFormat, ... );
DEJA_F_DECL void        DejaTrace_VS7       ( const char*    pChannel, const wchar_t* pFormat, ... );
DEJA_F_DECL void        DejaTrace_VS7       ( const wchar_t* pChannel, const wchar_t* pFormat, ... );
DEJA_F_DECL void        DejaTrace_VS7       ( const deja_wc* pChannel, const char*    pFormat, ... );
DEJA_F_DECL void        DejaTrace_VS7       ( const char*    pChannel, const deja_wc* pFormat, ... );
DEJA_F_DECL void        DejaTrace_VS7       ( const deja_wc* pChannel, const deja_wc* pFormat, ... );
...
DEJA_F_DECL void        DejaScriptContext   ( const deja_script_context_reg* pContextReg );
...
#define DEJA_TRACE( Channel, Format, ... )                                     \
    if( DejaSystemActive )                                                     \
    {                                                                          \
        static deja_trace_reg DejaTraceReg = { 0 };                            \
        if( DejaTraceReg.Data & 512 )                                          \
        {                                                                      \
        }                                                                      \
        else                                                                   \
        {                                                                      \
            DejaLock( __FILE__, __LINE__ );                                    \
            DejaTrace( &DejaTraceReg, #Channel, #Format, 1 );                  \
            DejaTrace( Channel, Format, ## __VA_ARGS__ );                      \
        }                                                                      \
    }                                                                          \
    else                                                                       \
    {                                                                          \
...
#define DEJA_WARNING( Channel, Format, ... )                                   \
    ...
    {                                                                          \
        ...
        {                                                                      \
            DejaLock( __FILE__, __LINE__ );                                    \
            DejaTrace( &DejaTraceReg, #Channel, #Format, 2 );                  \
            DejaTrace( Channel, Format, ## __VA_ARGS__ );                      \
        }                                                                      \
    }                                                                          \
    else                                                                       \
...
#define DEJA_ERROR( Channel, Format, ... )                                     \
    ...
    {                                                                          \
        ...
        {                                                                      \
            DejaLock( __FILE__, __LINE__ );                                    \
            DejaTrace( &DejaTraceReg, #Channel, #Format, 4 );                  \
            DejaTrace( Channel, Format, ## __VA_ARGS__ );                      \
        }                                                                      \
    }                                                                          \
    else                                                                       \
...
#define DEJA_BOOKMARK( Channel, Format, ... )                                  \
    ...
    {                                                                          \
        ...
        {                                                                      \
            DejaLock( __FILE__, __LINE__ );                                    \
            DejaTrace( &DejaTraceReg, #Channel, #Format, 8 );                  \
            DejaTrace( Channel, Format, ## __VA_ARGS__ );                      \
        }                                                                      \
    }                                                                          \
    else                                                                       \
...
#define DEJA_TRACE_OFF( Channel, Format, ... )                                 \
    if( DejaSystemActive )                                                     \
    {                                                                          \
        static deja_trace_reg DejaTraceReg = { 1 };                            \
        if( DejaTraceReg.Data & 512 )                                          \
        {                                                                      \
        }                                                                      \
        else                                                                   \
...
////
//// if( Microsoft Visual Studio 2003 )  (which doesn't have variadic)
////
//  The compiler in Visual Studio 2003 does not properly compile constructs
//  using the token splice preprocessor directive (##) and the predefined line 
...
////
//// NOT( Microsoft Visual Studio 2003 ) and NOT( variadic macros enabled )
////
...
#endif // Work-arounds specific to Visual Studio 2003 (VS7).
...
template< class T >
void DejaObjectLabelT( const T& Object, const deja_wc* pWLabelFormat, ... )
{
    va_list args;
    va_start( args, pWLabelFormat );
    DejaObjectLabel( (void*)&Object, NULL, (wchar_t*)pWLabelFormat, args );
    DejaUnlock();
}
