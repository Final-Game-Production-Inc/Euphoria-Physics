...
#if (RSG_CPU_X86 || RSG_CPU_X64) && !__RGSC_DLL

#if RSG_PC && !__RGSC_DLL
	#define _INTPTR 1
	#include <TFIT_defs_iAES1.h>
	#include <TFIT_defs_iAES2.h>
	#include <TFIT_AES_ECB_Decrypt_iAES2.h>
	#include <TFIT_AES_ECB_Encrypt_iAES1.h>
	#include "TFIT_keys_iAES1.h"
	#include "TFIT_keys_iAES2.h"
#endif

#include "multi_keys_AESo.h"
#include "multi_keys_AESd.h"
#endif

//originally taken from public domain code in libtom (available from http://libtom.org/)
//grs 15/11/2007
...
#include <stdio.h>
	...
	#if RSG_CPU_X64
		#if RSG_PC
			#pragma comment(lib,"TFIT_AES_ECB_Encrypt_iAES1_64.lib")
			#pragma comment(lib,"TFIT_AES_ECB_Decrypt_iAES2_64.lib")
			#pragma comment(lib,"TFIT_Utils_64.lib")
		#endif
		#pragma comment(lib,"intel_aes64.lib")
	#elif RSG_CPU_X86 
		#if RSG_PC
			#pragma comment(lib,"TFIT_AES_ECB_Encrypt_iAES1_32.lib")
			#pragma comment(lib,"TFIT_AES_ECB_Decrypt_iAES2_32.lib")
			#pragma comment(lib,"TFIT_Utils_32.lib")		
		#endif
		#pragma comment(lib,"intel_aes86.lib")
	#endif
// #define check_for_aes_instructions() false // uncomment this to disable new code paths.
#elif (RSG_CPU_X64 && RSG_TOOL) && !__RGSC_DLL && !__FINAL
	#pragma comment(lib,"TFIT_AES_ECB_Encrypt_iAES1_64_MD.lib")
	#pragma comment(lib,"TFIT_AES_ECB_Decrypt_iAES2_64_MD.lib")
	#pragma comment(lib,"TFIT_Utils_64_MD.lib")
#elif (RSG_CPU_X86 && RSG_TOOL) && !__RGSC_DLL && !__FINAL
	#pragma comment(lib,"TFIT_AES_ECB_Encrypt_iAES1_32_MD.lib")
	#pragma comment(lib,"TFIT_AES_ECB_Decrypt_iAES2_32_MD.lib")
	#pragma comment(lib,"TFIT_Utils_32_MD.lib")
#endif


#ifdef RUBYRAGE
	#define MULTIKEY_SUPPORT		1
#else
	#define MULTIKEY_SUPPORT		(__WIN32PC && !__FINAL)
#endif

namespace rage {

#if MULTIKEY_SUPPORT
	PARAM(aeskey,"[aes] Select which AES key is used to create content (gta4, mc4, rdr2, jimmy, mp3, bully2, rage)");
#endif

#if (HACK_GTA4 || MULTIKEY_SUPPORT) && !__FINAL	// retire the key in final builds...
static unsigned char unique_key_gta4[32] = {	
...
#endif
// This is the former "rage" key, and is what MC4 shipped with.
#if HACK_MC4 || MULTIKEY_SUPPORT
static unsigned char unique_key_mc4[32] = {	
...
#endif
#if HACK_RDR2 || MULTIKEY_SUPPORT
static unsigned char unique_key_rdr2[32] = {	
...
#endif
#if HACK_JIMMY || MULTIKEY_SUPPORT
static unsigned char unique_key_jimmy[32] = {	
0x8a, 0xd8, 0x9a, 0x58, 0x0d, 0x41, 0x68, 0x0b,
0x89, 0xc2, 0xf2, 0x7b, 0xb9, 0x85, 0xbe, 0x0e,
0x87, 0xc4, 0x17, 0x6d, 0x40, 0x86, 0x27, 0xc1,
0x89, 0x42, 0x06, 0x95, 0xb6, 0x95, 0x23, 0xa2 };
#endif
#if HACK_MP3 || MULTIKEY_SUPPORT
static unsigned char unique_key_mp3[32] = {	
0x9e, 0x44, 0x77, 0x7e, 0x5b, 0x8d, 0xf0, 0xfb,
0xa7, 0x51, 0xed, 0xe8, 0x05, 0x20, 0xfb, 0x99,
0x81, 0x18, 0x63, 0xc0, 0x87, 0x69, 0xd2, 0x43,
0x88, 0x11, 0x3b, 0x6a, 0x8b, 0x5b, 0xdb, 0x2d };
#endif
#if HACK_BULLY2 || MULTIKEY_SUPPORT
static unsigned char unique_key_bully2[32] = {	
0x96, 0xf5, 0x23, 0x11, 0x4e, 0x32, 0x26, 0x45,
0xa9, 0x17, 0xf5, 0x53, 0x6d, 0xd0, 0xdb, 0x73,
0xbe, 0xb8, 0xef, 0x69, 0xa3, 0xb9, 0x2a, 0x21,
0xbd, 0xdc, 0xf7, 0xc6, 0xf7, 0xd2, 0xcf, 0x97 };
#endif
#if (HACK_GTA4 && !__RGSC_DLL /*&& __WIN32PC*/) || MULTIKEY_SUPPORT		//__WIN32PC commented out to support data using the "old" encryption system for the time being
static unsigned char unique_key_gta5_pc[32] = {
...
#endif
#if (HACK_GTA4 && !__RGSC_DLL && RSG_ORBIS) || MULTIKEY_SUPPORT
static unsigned char unique_key_gta5_ps4[32] = {	
...
#endif
#if (HACK_GTA4 && !__RGSC_DLL && RSG_DURANGO) || MULTIKEY_SUPPORT
static unsigned char unique_key_gta5_xboxone[32] = {
...
#endif
#if __RGSC_DLL || MULTIKEY_SUPPORT
static unsigned char unique_key_rgsc_dll[32] = {
...
#endif
#if MULTIKEY_SUPPORT
static unsigned char unique_key_gta5_ps3[32] = {
...
#endif
#if MULTIKEY_SUPPORT
static unsigned char unique_key_gta5_360[32] = {
...
#endif
#if  __WIN32PC  || MULTIKEY_SUPPORT
static unsigned char unique_key_gta5save_pc[32] = {
...
#endif
#if __PS3 || RSG_ORBIS || MULTIKEY_SUPPORT
static unsigned char unique_key_gta5save_ps3[32] = {
...
#endif
#if __XENON || RSG_DURANGO || MULTIKEY_SUPPORT
static unsigned char unique_key_gta5save_360[32] = {
...
// Used when migrating a Single Player savegame from a gen8 console (PS4/XB1) to a gen9 console (PS5/XBSX)
static unsigned char unique_key_gta5save_migration[32] = {
	0x4d,  0x31,  0x80,  0xf3,  0x62,  0x91,  0xc6,  0x72,
	0xc9,  0x14,  0x53,  0x25,  0xa0,  0xbf,  0xd6,  0x92,
	0x52,  0xd2,  0xa6,  0xc7,  0x87,  0x9a,  0xdf,  0x62,
	0xf3,  0xa9,  0x12,  0x4c,  0x67,  0x51,  0x1b,  0x84
};

#if __PS3
unsigned char unique_key_gta5_ps3[32];
#elif __XENON
unsigned char unique_key_gta5_360[32];
#endif
#if (!HACK_GTA4 && !HACK_MC4 && !HACK_RDR2 && !HACK_JIMMY && !HACK_MP3 && !HACK_BULLY2 && !__RGSC_DLL) || MULTIKEY_SUPPORT
static unsigned char unique_key_rage[32] = {	
...
#define LTC_SMALL_ENCODE 1
#define LTC_SMALL_DECODE 0

// If this is set to 14, use a fully-unrolled loop.
#define LTC_FIXED_KEY_LENGTH	14
...
#if __BE
#define STORE32H(x, y) { *(unsigned long*)(y) = x; }
#define LOAD32H(x, y) { x = *(unsigned long*)(y); }
#else
#define STORE32H(x, y)                                                                     \
...
#if !(LTC_SMALL_ENCODE || LTC_SMALL_DECODE)
...
bool rijndael_setup(const unsigned char *key, int keylen, int num_rounds, symmetric_key *skey)
{
    int i, j;
    ulong32 temp, *rk;
    ulong32 *rrk;
	...
	Assert(skey->rijndael.Nr == LTC_FIXED_KEY_LENGTH);

#if INTEL_AES_INSTRUCTIONS
	if (check_for_aes_instructions()) {
		iEncExpandKey256((UCHAR*)key, (UCHAR*) skey->rijndael.eK);
		iDecExpandKey256((UCHAR*)key, (UCHAR*) skey->rijndael.dK);
		return true;
	}
#endif
    ...
    *rk   = *rrk;
    ...
    for (i = 1; i < skey->rijndael.Nr; i++) {
        rrk -= 4;
        rk  += 4;
    #if LTC_SMALL_DECODE || LTC_SMALL_ENCODE
        temp = rrk[0];
...
void rijndael_ecb_encrypt(unsigned char *ct, symmetric_key *skey)
{
...
// -Os produced much better code than -Od so let's override it in Beta builds
#if __PS3 && __DEV
#pragma control %push reg=2 sched=2
#endif


void rijndael_ecb_decrypt(unsigned char *pt, symmetric_key *skey)
{
    ulong32 s0, s1, s2, s3, t0, t1, t2, t3, *rk;
	// Note: Attempted to optimize loop by doing two decodes in parallel, but I suspect register pressure was too much, it got slower.
#if LTC_FIXED_KEY_LENGTH != 14 || LTC_SMALL_DECODE
    int Nr, r;
...
#else
#define STEP_1		t0 = Td0(byte(s0, 3)) ^ Td1(byte(s3, 2)) ^ Td2(byte(s2, 1)) ^ Td3(byte(s1, 0)) ^ rk[4]; \
					...
					rk += 8;
#define STEP_2		s0 = Td0(byte(t0, 3)) ^ Td1(byte(t3, 2)) ^ Td2(byte(t2, 1)) ^ Td3(byte(t1, 0)) ^ rk[0]; \
...
#if __PS3 && __DEV
#pragma control %pop sched reg
#endif

bool rijndael_ecb_encrypt_mem(void *data, unsigned int size, symmetric_key *skey)
{
#if INTEL_AES_INSTRUCTIONS
	if (check_for_aes_instructions()) {
		sAesData aesData;
		aesData.in_block = (UCHAR*)data;
		aesData.out_block = (UCHAR*)data;
		aesData.expanded_key = (UCHAR*)skey->rijndael.eK;
		aesData.num_blocks = size>>4;
		iEnc256(&aesData);
		return true;
	}
#endif
	// Encryption always does a single round now
	while (size > 0)
	{
		rijndael_ecb_encrypt((unsigned char*)data,skey);
		data = (void*)((char*)data + 16);
		size -= 16;
	}
	return true;
}

bool rijndael_ecb_decrypt_mem(void *data, unsigned int size, symmetric_key *skey)
{
	if (skey->rijndael.extra_rounds == 15) 
	{
#if INTEL_AES_INSTRUCTIONS
		if (check_for_aes_instructions()) {
			sAesData aesData;
			aesData.in_block = (UCHAR*)data;
			aesData.out_block = (UCHAR*)data;
			aesData.expanded_key = (UCHAR*)skey->rijndael.dK;
			aesData.num_blocks = size>>4;
			for (int i=0; i<16; i++)
				iDec256(&aesData);
			return true;
		...
		{
			for (int i=0; i<16; i++)
				rijndael_ecb_decrypt((unsigned char*)data,skey);
			data = (void*)((char*)data + 16);
			size -= 16;
		}
	}
...
AES::AES(unsigned _key_id) : key_id(_key_id)
{
	//@@: location AES_AES_ENTRY
#if HACK_MC4
	unsigned char *uk = unique_key_mc4;
#elif HACK_RDR2
	unsigned char *uk = unique_key_rdr2;
#elif HACK_MP3
	unsigned char *uk = unique_key_mp3;
#elif HACK_BULLY2
	unsigned char *uk = unique_key_bully2;
#elif HACK_JIMMY
	unsigned char *uk = unique_key_jimmy;
#elif __RGSC_DLL
	unsigned char *uk = unique_key_rgsc_dll;
#elif HACK_GTA4
#if __WIN32PC
	unsigned char *uk = unique_key_gta5_pc;
#elif RSG_ORBIS
	unsigned char *uk = unique_key_gta5_ps4;
#elif RSG_DURANGO
	unsigned char *uk = unique_key_gta5_xboxone;
#elif __PS3
	unsigned char *uk = unique_key_gta5_ps3;
#elif __XENON
	unsigned char *uk = unique_key_gta5_360;
#endif
#else
	unsigned char *uk = unique_key_rage;
#endif

#if __RESOURCECOMPILER || defined(RUBYRAGE)
	if (!_key_id) {
		if (g_sysPlatform == platform::WIN32PC || g_sysPlatform == platform::WIN64PC)
#if (RSG_CPU_X86 || RSG_CPU_X64) && !__RGSC_DLL && !__FINAL
			uk = unique_key_gta5_pc, key_id = AES_KEY_ID_WBC_GTA5_PC;
#else
			uk = unique_key_gta5_pc, key_id = AES_KEY_ID_GTA5_PC;
#endif
		else if (g_sysPlatform == platform::ORBIS)
			uk = unique_key_gta5_ps4, key_id = AES_MULTIKEY_ID_GTA5_PS4;
		else if (g_sysPlatform == platform::DURANGO)
			uk = unique_key_gta5_xboxone, key_id = AES_MULTIKEY_ID_GTA5_XBOXONE;
		else if (g_sysPlatform == platform::PS3)
			uk = unique_key_gta5_ps3, key_id = AES_KEY_ID_GTA5_PS3;
		else if (g_sysPlatform == platform::XENON)
			uk = unique_key_gta5_360, key_id = AES_KEY_ID_GTA5_360;
	}
#else
	if (!_key_id)
		key_id = AES_KEY_ID_DEFAULT;
#if !__RGSC_DLL
	else if(_key_id == AES_KEY_ID_GTA5_PC)
		key_id = AES_KEY_ID_GTA5_PC, uk = unique_key_gta5_pc;
#endif
	else if(_key_id == AES_KEY_ID_SAVEGAME)
	{
		key_id = AES_KEY_ID_GTA5_PC;
#if __WIN32PC || RSG_PC
		uk = unique_key_gta5save_pc;
#elif __PS3 ||RSG_ORBIS
		uk = unique_key_gta5save_ps3;
#elif __XENON || RSG_DURANGO
		uk = unique_key_gta5save_360;
#endif
	}
	else if (_key_id == AES_KEY_ID_GTA5SAVE_MIGRATION)
	{
		key_id = AES_KEY_ID_GTA5_PC;
		uk = unique_key_gta5save_migration;
	}
#endif

#if MULTIKEY_SUPPORT
	const char *keyname;
	switch (_key_id & AES_KEY_ID_MASK) {
		case 0:		// default for project
			if (PARAM_aeskey.GetParameter(keyname)) {
				if (keyname[0]&&keyname[1]==':')
					keyname += 3;	// allow an output directory to be passed in as well.
				if (!strncmp(keyname,"gta4",4)) uk = unique_key_gta4, key_id = AES_KEY_ID_GTA4;
				else if (!strncmp(keyname,"mc4",3)) uk = unique_key_mc4, key_id = AES_KEY_ID_MC4;
				else if (!strncmp(keyname,"rdr2",4)) uk = unique_key_rdr2, key_id = AES_KEY_ID_RDR2;
				else if (!strncmp(keyname,"jimmy",5)) uk = unique_key_jimmy, key_id = AES_KEY_ID_JIMMY;
				else if (!strncmp(keyname,"mp3",3)) uk = unique_key_mp3, key_id = AES_KEY_ID_MP3;
				else if (!strncmp(keyname,"bully2",6)) uk = unique_key_bully2, key_id = AES_KEY_ID_BULLY2;
				else if (!strncmp(keyname,"gta5",4)) {
					if (g_sysPlatform == platform::WIN32PC || g_sysPlatform == platform::WIN64PC)
#ifdef TRANSFORMIT
						uk = unique_key_gta5_pc, key_id = AES_KEY_ID_WBC_GTA5_PC;
#else
				...
				else if (!strncmp(keyname,"rage",4)) uk = unique_key_rage, key_id = AES_KEY_ID_RAGE;
				else Errorf("Unrecognized aes key name '%s', using default '%x'.",keyname,key_id);
			}
			break;
		case AES_KEY_ID_GTA4: uk = unique_key_gta4; break;
		case AES_KEY_ID_MC4: uk = unique_key_mc4; break;
		case AES_KEY_ID_RDR2: uk = unique_key_rdr2; break;
		case AES_KEY_ID_JIMMY: uk = unique_key_jimmy; break;
		case AES_KEY_ID_MP3: uk = unique_key_mp3; break;
		case AES_KEY_ID_BULLY2: uk = unique_key_bully2; break;
		#if __RGSC_DLL
		case AES_KEY_ID_RGSC: uk = unique_key_rgsc_dll; break;
		#else
		case AES_KEY_ID_GTA5_PS3: uk = unique_key_gta5_ps3; break;
		#endif
		case AES_KEY_ID_GTA5_PC: uk = unique_key_gta5_pc; break;
#if RSG_CPU_X64 
		case AES_KEY_ID_WBC_GTA5_PC: uk = unique_key_gta5_pc; break; 
		case AES_MULTIKEY_ID_GTA5_PS4: uk = unique_key_gta5_ps4, key_id = AES_MULTIKEY_ID_GTA5_PS4; break;
		case AES_MULTIKEY_ID_GTA5_XBOXONE: uk = unique_key_gta5_xboxone, key_id = AES_MULTIKEY_ID_GTA5_XBOXONE; break;
#endif
		case AES_KEY_ID_GTA5_360: uk = unique_key_gta5_360; break;
		case AES_KEY_ID_RAGE: uk = unique_key_rage; break;
		case AES_KEY_ID_GTA5SAVE_PC: uk = unique_key_gta5save_pc; break;
		case AES_KEY_ID_GTA5SAVE_PS3: uk = unique_key_gta5save_ps3; break;
		case AES_KEY_ID_GTA5SAVE_360: uk = unique_key_gta5save_360; break;
		default:
			Errorf("Unrecognized AES key id '%x', using default.",_key_id);
			break;
	}
#else
#if !__FINAL
	if ((key_id & AES_KEY_ID_MASK) == AES_KEY_ID_GTA4)
		uk = unique_key_gta4;
	else 
#endif
	if ((key_id & AES_KEY_ID_MASK) != AES_KEY_ID_DEFAULT && (key_id & AES_KEY_ID_MASK) != AES_KEY_ID_GTA5_PC)
		Quitf(ERR_GEN_INVALID,"Data has key '%x' specified, but this project only recognizes '%x' and '%x'",key_id & AES_KEY_ID_MASK,AES_KEY_ID_DEFAULT, AES_KEY_ID_GTA5_PC);
#endif

	rijndael_setup(uk,32,0,&key);
	key.rijndael.extra_rounds = _key_id >> 28;
}

AES::AES(const unsigned char *raw_key)
{
	rijndael_setup(raw_key,32,0,&key);
	key.rijndael.extra_rounds = 0;
}
...
bool TransformITEncrypt(unsigned int selector, void *data, unsigned int NOTFINAL_ONLY(size))
{
#if !__FINAL
	int retVal = 0;
	if(size <=0)
	...
	TFIT_key_iAES1_t currKey;
	switch(selector)
	{
		case 0:
			currKey = TFIT_Game_key_0_iAES1;
			break;
		case 1:
			currKey = TFIT_Game_key_1_iAES1;
			break;
		case 2:
			currKey = TFIT_Game_key_2_iAES1;
			break;
		case 3:
			currKey = TFIT_Game_key_3_iAES1;
			break;
		case 4:
			currKey = TFIT_Game_key_4_iAES1;
			break;
		case 5:
			currKey = TFIT_Game_key_5_iAES1;
			break;
		case 6:
			currKey = TFIT_Game_key_6_iAES1;
			break;
		case 7:
			currKey = TFIT_Game_key_7_iAES1;
			break;
		case 8:
			currKey = TFIT_Game_key_8_iAES1;
			break;
		case 9:
			currKey = TFIT_Game_key_9_iAES1;
			break;
		case 10:
			currKey = TFIT_Game_key_10_iAES1;
			break;
		case 11:
			currKey = TFIT_Game_key_11_iAES1;
			break;
		case 12:
			currKey = TFIT_Game_key_12_iAES1;
			break;
		case 13:
			currKey = TFIT_Game_key_13_iAES1;
			break;
		case 14:
			currKey = TFIT_Game_key_14_iAES1;
			break;
		case 15:
			currKey = TFIT_Game_key_15_iAES1;
			break;
		case 16:
			currKey = TFIT_Game_key_16_iAES1;
			break;
		case 17:
			currKey = TFIT_Game_key_17_iAES1;
			break;
		case 18:
			currKey = TFIT_Game_key_18_iAES1;
			break;
		case 19:
			currKey = TFIT_Game_key_19_iAES1;
			break;
		case 20:
			currKey = TFIT_Game_key_20_iAES1;
			break;
		case 21:
			currKey = TFIT_Game_key_21_iAES1;
			break;
		case 22:
			currKey = TFIT_Game_key_22_iAES1;
			break;
		case 23:
			currKey = TFIT_Game_key_23_iAES1;
			break;
		case 24:
			currKey = TFIT_Game_key_24_iAES1;
			break;
		case 25:
			currKey = TFIT_Game_key_25_iAES1;
			break;
		case 26:
			currKey = TFIT_Game_key_26_iAES1;
			break;
		case 27:
			currKey = TFIT_Game_key_27_iAES1;
			break;
		case 28:
			currKey = TFIT_Game_key_28_iAES1;
			break;
		case 29:
			currKey = TFIT_Game_key_29_iAES1;
			break;
		case 30:
			currKey = TFIT_Game_key_30_iAES1;
			break;
		case 31:
			currKey = TFIT_Game_key_31_iAES1;
			break;
		case 32:
			currKey = TFIT_Game_key_32_iAES1;
			break;
		case 33:
			currKey = TFIT_Game_key_33_iAES1;
			break;
		case 34:
			currKey = TFIT_Game_key_34_iAES1;
			break;
		case 35:
			currKey = TFIT_Game_key_35_iAES1;
			break;
		case 36:
			currKey = TFIT_Game_key_36_iAES1;
			break;
		case 37:
			currKey = TFIT_Game_key_37_iAES1;
			break;
		case 38:
			currKey = TFIT_Game_key_38_iAES1;
			break;
		case 39:
			currKey = TFIT_Game_key_39_iAES1;
			break;
		case 40:
			currKey = TFIT_Game_key_40_iAES1;
			break;
		case 41:
			currKey = TFIT_Game_key_41_iAES1;
			break;
		case 42:
			currKey = TFIT_Game_key_42_iAES1;
			break;
		case 43:
			currKey = TFIT_Game_key_43_iAES1;
			break;
		case 44:
			currKey = TFIT_Game_key_44_iAES1;
			break;
		case 45:
			currKey = TFIT_Game_key_45_iAES1;
			break;
		case 46:
			currKey = TFIT_Game_key_46_iAES1;
			break;
		case 47:
			currKey = TFIT_Game_key_47_iAES1;
			break;
		case 48:
			currKey = TFIT_Game_key_48_iAES1;
			break;
		case 49:
			currKey = TFIT_Game_key_49_iAES1;
			break;
		case 50:
			currKey = TFIT_Game_key_50_iAES1;
			break;
		case 51:
			currKey = TFIT_Game_key_51_iAES1;
			break;
		case 52:
			currKey = TFIT_Game_key_52_iAES1;
			break;
		case 53:
			currKey = TFIT_Game_key_53_iAES1;
			break;
		case 54:
			currKey = TFIT_Game_key_54_iAES1;
			break;
		case 55:
			currKey = TFIT_Game_key_55_iAES1;
			break;
		case 56:
			currKey = TFIT_Game_key_56_iAES1;
			break;
		case 57:
			currKey = TFIT_Game_key_57_iAES1;
			break;
		case 58:
			currKey = TFIT_Game_key_58_iAES1;
			break;
		case 59:
			currKey = TFIT_Game_key_59_iAES1;
			break;
		case 60:
			currKey = TFIT_Game_key_60_iAES1;
			break;
		case 61:
			currKey = TFIT_Game_key_61_iAES1;
			break;
		case 62:
			currKey = TFIT_Game_key_62_iAES1;
			break;
		case 63:
			currKey = TFIT_Game_key_63_iAES1;
			break;
		case 64:
			currKey = TFIT_Game_key_64_iAES1;
			break;
		case 65:
			currKey = TFIT_Game_key_65_iAES1;
			break;
		case 66:
			currKey = TFIT_Game_key_66_iAES1;
			break;
		case 67:
			currKey = TFIT_Game_key_67_iAES1;
			break;
		case 68:
			currKey = TFIT_Game_key_68_iAES1;
			break;
		case 69:
			currKey = TFIT_Game_key_69_iAES1;
			break;
		case 70:
			currKey = TFIT_Game_key_70_iAES1;
			break;
		case 71:
			currKey = TFIT_Game_key_71_iAES1;
			break;
		case 72:
			currKey = TFIT_Game_key_72_iAES1;
			break;
		case 73:
			currKey = TFIT_Game_key_73_iAES1;
			break;
		case 74:
			currKey = TFIT_Game_key_74_iAES1;
			break;
		case 75:
			currKey = TFIT_Game_key_75_iAES1;
			break;
		case 76:
			currKey = TFIT_Game_key_76_iAES1;
			break;
		case 77:
			currKey = TFIT_Game_key_77_iAES1;
			break;
		case 78:
			currKey = TFIT_Game_key_78_iAES1;
			break;
		case 79:
			currKey = TFIT_Game_key_79_iAES1;
			break;
		case 80:
			currKey = TFIT_Game_key_80_iAES1;
			break;
		case 81:
			currKey = TFIT_Game_key_81_iAES1;
			break;
		case 82:
			currKey = TFIT_Game_key_82_iAES1;
			break;
		case 83:
			currKey = TFIT_Game_key_83_iAES1;
			break;
		case 84:
			currKey = TFIT_Game_key_84_iAES1;
			break;
		case 85:
			currKey = TFIT_Game_key_85_iAES1;
			break;
		case 86:
			currKey = TFIT_Game_key_86_iAES1;
			break;
		case 87:
			currKey = TFIT_Game_key_87_iAES1;
			break;
		case 88:
			currKey = TFIT_Game_key_88_iAES1;
			break;
		case 89:
			currKey = TFIT_Game_key_89_iAES1;
			break;
		case 90:
			currKey = TFIT_Game_key_90_iAES1;
			break;
		case 91:
			currKey = TFIT_Game_key_91_iAES1;
			break;
		case 92:
			currKey = TFIT_Game_key_92_iAES1;
			break;
		case 93:
			currKey = TFIT_Game_key_93_iAES1;
			break;
		case 94:
			currKey = TFIT_Game_key_94_iAES1;
			break;
		case 95:
			currKey = TFIT_Game_key_95_iAES1;
			break;
		case 96:
			currKey = TFIT_Game_key_96_iAES1;
			break;
		case 97:
			currKey = TFIT_Game_key_97_iAES1;
			break;
		case 98:
			currKey = TFIT_Game_key_98_iAES1;
			break;
		case 99:
			currKey = TFIT_Game_key_99_iAES1;
			break;
		case 100:
			currKey = TFIT_Game_key_100_iAES1;
			break;
		default:
			break;
	}
	retVal += TFIT_wbaes_ecb_encrypt_iAES1(&currKey,  tdata, size, tdata);
...
bool AES::TransformITDecrypt(unsigned int selector, void *data, unsigned int size)
{
	//@@: range AES_TRANSFORMITDECRYPT {
	int retVal = 0;
	size = size & ~15;
	if(size <=0)
		return true;
	unsigned char * tdata = (unsigned char*)data;
	TFIT_key_iAES2_t currKey;
	//@@: location AES_TRANSFORMITDECRYPT_SET_KEY
	switch(selector)
	{
		case 0:
			currKey = TFIT_Game_key_0_iAES2;
			break;
		case 1:
			currKey = TFIT_Game_key_1_iAES2;
			break;
		case 2:
			currKey = TFIT_Game_key_2_iAES2;
			break;
		case 3:
			currKey = TFIT_Game_key_3_iAES2;
			break;
		case 4:
			currKey = TFIT_Game_key_4_iAES2;
			break;
		case 5:
			currKey = TFIT_Game_key_5_iAES2;
			break;
		case 6:
			currKey = TFIT_Game_key_6_iAES2;
			break;
		case 7:
			currKey = TFIT_Game_key_7_iAES2;
			break;
		case 8:
			currKey = TFIT_Game_key_8_iAES2;
			break;
		case 9:
			currKey = TFIT_Game_key_9_iAES2;
			break;
		case 10:
			currKey = TFIT_Game_key_10_iAES2;
			break;
		case 11:
			currKey = TFIT_Game_key_11_iAES2;
			break;
		case 12:
			currKey = TFIT_Game_key_12_iAES2;
			break;
		case 13:
			currKey = TFIT_Game_key_13_iAES2;
			break;
		case 14:
			currKey = TFIT_Game_key_14_iAES2;
			break;
		case 15:
			currKey = TFIT_Game_key_15_iAES2;
			break;
		case 16:
			currKey = TFIT_Game_key_16_iAES2;
			break;
		case 17:
			currKey = TFIT_Game_key_17_iAES2;
			break;
		case 18:
			currKey = TFIT_Game_key_18_iAES2;
			break;
		case 19:
			currKey = TFIT_Game_key_19_iAES2;
			break;
		case 20:
			currKey = TFIT_Game_key_20_iAES2;
			break;
		case 21:
			currKey = TFIT_Game_key_21_iAES2;
			break;
		case 22:
			currKey = TFIT_Game_key_22_iAES2;
			break;
		case 23:
			currKey = TFIT_Game_key_23_iAES2;
			break;
		case 24:
			currKey = TFIT_Game_key_24_iAES2;
			break;
		case 25:
			currKey = TFIT_Game_key_25_iAES2;
			break;
		case 26:
			currKey = TFIT_Game_key_26_iAES2;
			break;
		case 27:
			currKey = TFIT_Game_key_27_iAES2;
			break;
		case 28:
			currKey = TFIT_Game_key_28_iAES2;
			break;
		case 29:
			currKey = TFIT_Game_key_29_iAES2;
			break;
		case 30:
			currKey = TFIT_Game_key_30_iAES2;
			break;
		case 31:
			currKey = TFIT_Game_key_31_iAES2;
			break;
		case 32:
			currKey = TFIT_Game_key_32_iAES2;
			break;
		case 33:
			currKey = TFIT_Game_key_33_iAES2;
			break;
		case 34:
			currKey = TFIT_Game_key_34_iAES2;
			break;
		case 35:
			currKey = TFIT_Game_key_35_iAES2;
			break;
		case 36:
			currKey = TFIT_Game_key_36_iAES2;
			break;
		case 37:
			currKey = TFIT_Game_key_37_iAES2;
			break;
		case 38:
			currKey = TFIT_Game_key_38_iAES2;
			break;
		case 39:
			currKey = TFIT_Game_key_39_iAES2;
			break;
		case 40:
			currKey = TFIT_Game_key_40_iAES2;
			break;
		case 41:
			currKey = TFIT_Game_key_41_iAES2;
			break;
		case 42:
			currKey = TFIT_Game_key_42_iAES2;
			break;
		case 43:
			currKey = TFIT_Game_key_43_iAES2;
			break;
		case 44:
			currKey = TFIT_Game_key_44_iAES2;
			break;
		case 45:
			currKey = TFIT_Game_key_45_iAES2;
			break;
		case 46:
			currKey = TFIT_Game_key_46_iAES2;
			break;
		case 47:
			currKey = TFIT_Game_key_47_iAES2;
			break;
		case 48:
			currKey = TFIT_Game_key_48_iAES2;
			break;
		case 49:
			currKey = TFIT_Game_key_49_iAES2;
			break;
		case 50:
			currKey = TFIT_Game_key_50_iAES2;
			break;
		case 51:
			currKey = TFIT_Game_key_51_iAES2;
			break;
		case 52:
			currKey = TFIT_Game_key_52_iAES2;
			break;
		case 53:
			currKey = TFIT_Game_key_53_iAES2;
			break;
		case 54:
			currKey = TFIT_Game_key_54_iAES2;
			break;
		case 55:
			currKey = TFIT_Game_key_55_iAES2;
			break;
		case 56:
			currKey = TFIT_Game_key_56_iAES2;
			break;
		case 57:
			currKey = TFIT_Game_key_57_iAES2;
			break;
		case 58:
			currKey = TFIT_Game_key_58_iAES2;
			break;
		case 59:
			currKey = TFIT_Game_key_59_iAES2;
			break;
		case 60:
			currKey = TFIT_Game_key_60_iAES2;
			break;
		case 61:
			currKey = TFIT_Game_key_61_iAES2;
			break;
		case 62:
			currKey = TFIT_Game_key_62_iAES2;
			break;
		case 63:
			currKey = TFIT_Game_key_63_iAES2;
			break;
		case 64:
			currKey = TFIT_Game_key_64_iAES2;
			break;
		case 65:
			currKey = TFIT_Game_key_65_iAES2;
			break;
		case 66:
			currKey = TFIT_Game_key_66_iAES2;
			break;
		case 67:
			currKey = TFIT_Game_key_67_iAES2;
			break;
		case 68:
			currKey = TFIT_Game_key_68_iAES2;
			break;
		case 69:
			currKey = TFIT_Game_key_69_iAES2;
			break;
		case 70:
			currKey = TFIT_Game_key_70_iAES2;
			break;
		case 71:
			currKey = TFIT_Game_key_71_iAES2;
			break;
		case 72:
			currKey = TFIT_Game_key_72_iAES2;
			break;
		case 73:
			currKey = TFIT_Game_key_73_iAES2;
			break;
		case 74:
			currKey = TFIT_Game_key_74_iAES2;
			break;
		case 75:
			currKey = TFIT_Game_key_75_iAES2;
			break;
		case 76:
			currKey = TFIT_Game_key_76_iAES2;
			break;
		case 77:
			currKey = TFIT_Game_key_77_iAES2;
			break;
		case 78:
			currKey = TFIT_Game_key_78_iAES2;
			break;
		case 79:
			currKey = TFIT_Game_key_79_iAES2;
			break;
		case 80:
			currKey = TFIT_Game_key_80_iAES2;
			break;
		case 81:
			currKey = TFIT_Game_key_81_iAES2;
			break;
		case 82:
			currKey = TFIT_Game_key_82_iAES2;
			break;
		case 83:
			currKey = TFIT_Game_key_83_iAES2;
			break;
		case 84:
			currKey = TFIT_Game_key_84_iAES2;
			break;
		case 85:
			currKey = TFIT_Game_key_85_iAES2;
			break;
		case 86:
			currKey = TFIT_Game_key_86_iAES2;
			break;
		case 87:
			currKey = TFIT_Game_key_87_iAES2;
			break;
		case 88:
			currKey = TFIT_Game_key_88_iAES2;
			break;
		case 89:
			currKey = TFIT_Game_key_89_iAES2;
			break;
		case 90:
			currKey = TFIT_Game_key_90_iAES2;
			break;
		case 91:
			currKey = TFIT_Game_key_91_iAES2;
			break;
		case 92:
			currKey = TFIT_Game_key_92_iAES2;
			break;
		case 93:
			currKey = TFIT_Game_key_93_iAES2;
			break;
		case 94:
			currKey = TFIT_Game_key_94_iAES2;
			break;
		case 95:
			currKey = TFIT_Game_key_95_iAES2;
			break;
		case 96:
			currKey = TFIT_Game_key_96_iAES2;
			break;
		case 97:
			currKey = TFIT_Game_key_97_iAES2;
			break;
		case 98:
			currKey = TFIT_Game_key_98_iAES2;
			break;
		case 99:
			currKey = TFIT_Game_key_99_iAES2;
			break;
		case 100:
			currKey = TFIT_Game_key_100_iAES2;
			break;
		default:
			break;
	}
	...
	retVal += TFIT_wbaes_ecb_decrypt_iAES2(&currKey,  tdata, size, tdata);
	
	return (retVal == 0) ? true : false;

	//@@: } AES_TRANSFORMITDECRYPT
...
#if RSG_CPU_X86 || RSG_CPU_X64  

bool AES::Encrypt(unsigned int keyId, unsigned int selector, void *data, unsigned int size )
{
	AES aes(keyId);
	#if RSG_ORBIS || __RESOURCECOMPILER || RSG_TOOL
	if(keyId == AES_MULTIKEY_ID_GTA5_PS4)
	{
		rijndael_setup(unique_multikey_gta5_ps4[selector],32,0,&aes.key);
		aes.key.rijndael.extra_rounds = AES_MULTIKEY_ID_GTA5_PS4 >> 28;
	}
	#endif
	#if RSG_DURANGO || __RESOURCECOMPILER || RSG_TOOL
	if(keyId == AES_MULTIKEY_ID_GTA5_XBOXONE)
	{
		rijndael_setup(unique_multikey_gta5_xboxone[selector],32,0,&aes.key);
...
bool AES::Decrypt(unsigned int keyId, unsigned int selector, void *data, unsigned int size)
{
	AES aes(keyId);
	#if RSG_ORBIS || __RESOURCECOMPILER || RSG_TOOL
	if(keyId == AES_MULTIKEY_ID_GTA5_PS4)
	...
	//@@: range AES_DECRYPT_TRANSFORMIT_CALL {
	if(isTransformITKeyId(keyId))
		return TransformITDecrypt(selector, data, size);
	//@@: } AES_DECRYPT_TRANSFORMIT_CALL
	#endif

	if(keyId == AES_KEY_ID_FROM_CLOUD && ms_cloudAes)
	{
		return ms_cloudAes->Decrypt(data, size);
	}
    selector = 0;
	//@@: location AES_DECRYPT_GENERAL_DECRYPT
	return aes.Decrypt(data,size);
}

#endif

bool AES::Encrypt(void *data, unsigned int size)
{
	return rijndael_ecb_encrypt_mem(data,size & ~15,&key);
}

bool AES::Decrypt(void *data, unsigned int size)
{
	return rijndael_ecb_decrypt_mem(data,size & ~15,&key);
}

AES* AES::ms_cloudAes = NULL;
void AES::CreateCloudAes(const unsigned char* data)
{
    if (!ms_cloudAes)
		ms_cloudAes = rage_new AES(data);
}
...
bool rijndael_test(void)
{
	...
	for (i = 0; i < (int)(sizeof(tests)/sizeof(tests[0])); i++)
	{
		...
		if ((err = rijndael_setup(tests[i].key, tests[i].keylen, 0, &key)) != TRUE)
			return false;
		...
		for (y = 0; y < 16; y++) if (tmp[0][y] != 0) return FALSE;
	}
