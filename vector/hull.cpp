...
namespace HullLibrary {
	...
	static void BringOutYourDead(const float *verts,unsigned int vcount, float *overts,unsigned int &ocount,unsigned int *indices,unsigned indexcount);

	static bool CleanupVertices(unsigned int svcount,
...
class HullQuaternion :public float4
{
 public:
	HullQuaternion() { x = y = z = 0.0f; w = 1.0f; }
	HullQuaternion( float3 v, float t ) { v = normalize(v); w = rage::Cosf(t/2.0f); v = v*rage::Sinf(t/2.0f); x = v.x; y = v.y; z = v.z; }
	HullQuaternion(float _x, float _y, float _z, float _w){x=_x;y=_y;z=_z;w=_w;}
	float angle() const { return rage::Acosf(w)*2.0f; }
	float3 axis() const { float3 a(x,y,z); if(fabsf(angle())<0.0000001f) return float3(1,0,0); return a*(1/rage::Sinf(angle()/2.0f)); }
...
HullQuaternion& operator*=(HullQuaternion& a, float s );
HullQuaternion	operator*( const HullQuaternion& a, float s );
HullQuaternion	operator*( const HullQuaternion& a, const HullQuaternion& b);
HullQuaternion	operator+( const HullQuaternion& a, const HullQuaternion& b );
HullQuaternion	normalize( HullQuaternion a );
float		dot( const HullQuaternion &a, const HullQuaternion &b );
float3		operator*( const HullQuaternion& q, const float3& v );
float3		operator*( const float3& v, const HullQuaternion& q );
HullQuaternion	slerp( HullQuaternion a, const HullQuaternion& b, float interp );
HullQuaternion  Interpolate(const HullQuaternion &q0,const HullQuaternion &q1,float alpha); 
HullQuaternion  RotationArc(float3 v0, float3 v1 );  // returns quat q where q*v0=v1
HullQuaternion  Inverse(const HullQuaternion &q);
float4x4     MatrixFromQuatVec(const HullQuaternion &q, const float3 &v);


//------ Euler Angle -----

HullQuaternion YawPitchRoll( float yaw, float pitch, float roll );
float Yaw( const HullQuaternion& q );
float Pitch( const HullQuaternion& q );
float Roll( HullQuaternion q );
...
class HullPlane
{
	...
	float	dist;   // distance below origin - the D from plane equasion Ax+By+Cz+D=0
			HullPlane(const float3 &n,float d):normal(n),dist(d){}
			HullPlane():normal(),dist(0){}
	void	Transform(const float3 &position, const HullQuaternion &orientation);
};

inline HullPlane HullPlaneFlip(const HullPlane &plane){return HullPlane(-plane.normal,-plane.dist);}
inline int operator==( const HullPlane &a, const HullPlane &b ) { return (a.normal==b.normal && a.dist==b.dist); }
inline int coplanar( const HullPlane &a, const HullPlane &b ) { return (a==b || a==HullPlaneFlip(b)); }


//--------- Utility Functions ------

float3  HullPlaneLineIntersection(const HullPlane &plane, const float3 &p0, const float3 &p1);
float3  HullPlaneProject(const HullPlane &plane, const float3 &point);
...
float3  ThreeHullPlaneIntersection(const HullPlane &p0,const HullPlane &p1, const HullPlane &p2);
...
HullQuaternion VirtualTrackBall(const float3 &cop, const float3 &cor, const float3 &dir0, const float3 &dir1);
...
float3 normalize( const float3 &v )
{
	// this routine, normalize, is ok, provided magnitude works!!
		...
		if (d==0)
		{
		Errorf("Cant normalize ZERO vector");
...
float4x4 MatrixRotationZ(const float angle_radians)
{
	float s =  rage::Sinf(angle_radians);
	float c =  rage::Cosf(angle_radians);
	return float4x4(
		c,  s,  0,  0,
		-s, c,  0,  0,
		0,  0,  1,  0,
...
HullQuaternion operator*( const HullQuaternion& a, float b )
{
	return HullQuaternion(a.x*b, a.y*b, a.z*b ,a.w*b);
}

HullQuaternion  Inverse(const HullQuaternion &q)
{
	return HullQuaternion(-q.x,-q.y,-q.z,q.w);
}

HullQuaternion& operator*=( HullQuaternion& q, const float s )
{
		q.x *= s;
		q.y *= s;
		q.z *= s;
...
float3 operator*( const float3& , const HullQuaternion&  )
{
...
HullQuaternion operator+( const HullQuaternion& a, const HullQuaternion& b )
{
	return HullQuaternion(a.x+b.x, a.y+b.y, a.z+b.z, a.w+b.w);
}
...
HullQuaternion slerp( HullQuaternion a, const HullQuaternion& b, float interp )
{
	...
	float theta = rage::Acosf(d);
	if(theta==0.0f) { return(a);}
	return a*(rage::Sinf(theta-interp*theta)/rage::Sinf(theta)) + b*(rage::Sinf(interp*theta)/rage::Sinf(theta));
}
...
HullQuaternion YawPitchRoll( float yaw, float pitch, float roll ) 
{
	...
	yaw   *= DEG2RAD;
	pitch *= DEG2RAD;
	return HullQuaternion(float3(0.0f,0.0f,1.0f),yaw)*HullQuaternion(float3(1.0f,0.0f,0.0f),pitch)*HullQuaternion(float3(0.0f,1.0f,0.0f),roll);
}
...
float Yaw( const HullQuaternion& q )
{
	static float3 v;
	v=q.ydir();
	return (v.y==0.0&&v.x==0.0) ? 0.0f: rage::Atan2f(-v.x,v.y)*RAD2DEG;
}
...
float Pitch( const HullQuaternion& q )
{
	static float3 v;
	v=q.ydir();
	return rage::Atan2f(v.z,sqrtf(sqr(v.x)+sqr(v.y)))*RAD2DEG;
}

float Roll( HullQuaternion q )
{
	q = HullQuaternion(float3(0.0f,0.0f,1.0f),-Yaw(q)*DEG2RAD)  *q;
	q = HullQuaternion(float3(1.0f,0.0f,0.0f),-Pitch(q)*DEG2RAD)  *q;
	return rage::Atan2f(-q.xdir().z,q.xdir().x)*RAD2DEG;
}
...
void HullPlane::Transform(const float3 &position, const HullQuaternion &orientation) {
...
HullQuaternion RotationArc(float3 v0,float3 v1){
	static HullQuaternion q;
	...
	float   d = dot(v0,v1);
	if(d<=-1.0f) { return HullQuaternion(1,0,0,0);} // 180 about x axis
	float   s = sqrtf((1+d)*2);
	q.x = c.x / s;
	q.y = c.y / s;
	q.z = c.z / s;
...
float DistanceBetweenLines(const float3 &ustart, const float3 &udir, const float3 &vstart, const float3 &vdir, float3 *upoint, float3 *vpoint)
{
	...
	if(upoint) 
		...
		*upoint = HullPlaneLineIntersection(plane,ustart,ustart+udir);
	}
	if(vpoint) 
		{
		HullPlane plane;
		...
		*vpoint = HullPlaneLineIntersection(plane,vstart,vstart+vdir);
	}
	return dist;
}


HullQuaternion VirtualTrackBall(const float3 &cop, const float3 &cor, const float3 &dir1, const float3 &dir2) 
{
	// routine taken from game programming gems.
	...
	float3 u= HullPlaneLineIntersection(HullPlane(nrml,dist),cop,cop+dir1);
	u=u-cor;
	u=u*fudgefactor;
	m= magnitude(u);
	if(m>1)
	...
	float3 v= HullPlaneLineIntersection(HullPlane(nrml,dist),cop,cop+dir2);
	v=v-cor;
	v=v*fudgefactor;
	m= magnitude(v);
	if(m>1) 
...
ConvexH *ConvexHDup(ConvexH *src) {
	ConvexH *dst = rage_new ConvexH(src->vertices.count,src->edges.count,src->facets.count);
	...
	memcpy(dst->facets.element,src->facets.element,sizeof(HullPlane)*src->facets.count);
	return dst;
}


int HullPlaneTest(const HullPlane &p, const REAL3 &v) {
	REAL a  = dot(v,p.normal)+p.dist;
	int   flag = (a>planetestepsilon)?OVER:((a<-planetestepsilon)?UNDER:COPLANAR);
	return flag;
}

int SplitTest(ConvexH &convex,const HullPlane &plane) {
	int flag=0;
	for(int i=0;i<convex.vertices.count;i++) {
		flag |= HullPlaneTest(plane,convex.vertices[i]);
	}
	return flag;
}
...
int AssertIntact(ConvexH &convex) {
	...
	for(i=0;i<convex.edges.count;i++) {
		assert(COPLANAR==HullPlaneTest(convex.facets[convex.edges[i].p],convex.vertices[convex.edges[i].v]));
		if(COPLANAR!=HullPlaneTest(convex.facets[convex.edges[i].p],convex.vertices[convex.edges[i].v])) return 0;
		if(convex.edges[estart].p!= convex.edges[i].p) {
			estart=i;
		}
		int i1 = i+1;
...
ConvexH *test_cube() {
	ConvexH *convex = rage_new ConvexH(8,24,6);
	convex->vertices[0] = REAL3(0,0,0);
	...
	convex->facets[0] = HullPlane(REAL3(-1,0,0),0);
	convex->facets[1] = HullPlane(REAL3(1,0,0),-1);
	convex->facets[2] = HullPlane(REAL3(0,-1,0),0);
	convex->facets[3] = HullPlane(REAL3(0,1,0),-1);
	convex->facets[4] = HullPlane(REAL3(0,0,-1),0);
	convex->facets[5] = HullPlane(REAL3(0,0,1),-1);
...
ConvexH *ConvexHMakeCube(const REAL3 &bmin, const REAL3 &bmax) {
	...
	convex->facets[0] = HullPlane(REAL3(-1,0,0), bmin.x);
	convex->facets[1] = HullPlane(REAL3(1,0,0), -bmax.x);
	convex->facets[2] = HullPlane(REAL3(0,-1,0), bmin.y);
	convex->facets[3] = HullPlane(REAL3(0,1,0), -bmax.y);
	convex->facets[4] = HullPlane(REAL3(0,0,-1), bmin.z);
	convex->facets[5] = HullPlane(REAL3(0,0,1), -bmax.z);
	return convex;
}
ConvexH *ConvexHCrop(ConvexH &convex,const HullPlane &slice)
{
	int i;
	...
	// HullPlaneFlag planeflag[128];
	HalfEdge  tmpunderedges[512];
	HullPlane	  tmpunderplanes[128];
	...
	for(i=0;i<convex.vertices.count;i++) {
		vertflag[i].planetest = HullPlaneTest(slice,convex.vertices[i]);
		if(vertflag[i].planetest == COPLANAR) {
			...
			vertflag[i].overmap  = vertcountover++;
			vertflag[i].undermap = 0xff; // for debugging purposes
		}
	}
		...
		do{
			...
			if(vertflag[edge0.v].planetest == OVER && vertflag[edge1.v].planetest == OVER){
				...
				if(edge0.ea < e0) {
					...
					HullPlane &p0 = convex.facets[edge0.p];
					HullPlane &pa = convex.facets[edgea.p];
					createdverts.Add(ThreeHullPlaneIntersection(p0,pa,slice));
					//createdverts.Add(HullPlaneProject(slice,HullPlaneLineIntersection(slice,convex.vertices[edge0.v],convex.vertices[edgea.v])));
					//createdverts.Add(HullPlaneLineIntersection(slice,convex.vertices[edge0.v],convex.vertices[edgea.v]));
					vout = vertcountunder++;
				}
				under_edge_count++;
	...
	ConvexH *punder = rage_new ConvexH(vertcountunder,under_edge_count+coplanaredges_num,underplanescount);
	...
	memcpy(under.facets.element,tmpunderplanes,sizeof(HullPlane)*underplanescount);
	return punder;
}



static int candidateplane(HullPlane *planes,int planes_count,ConvexH *convex,float epsilon)
{
	int p = -1;
	REAL md = -1e30f;
	int i;
...
int maxdirsterid(const T *p,int count,const T &dir,Array<int> &allow)
{
	...
	while(m==-1)
	{
		...
		{
			float s = rage::Sinf(DEG2RAD*(x));
			float c = rage::Cosf(DEG2RAD*(x));
			...
			if(ma!=-1 && ma!=mb)  // Yuck - this is really ugly
			{
				...
				{
					float s = rage::Sinf(DEG2RAD*(xx));
					float c = rage::Cosf(DEG2RAD*(xx));
...
void checkit(Tri *ASSERT_ONLY(t))
{
	int i;
	mthAssertf(tris[t->id]==t, "Triangle ID is incorrect");
	for(i=0;i<3;i++)
	{
		ASSERT_ONLY(int i1=(i+1)%3);
		ASSERT_ONLY(int i2=(i+2)%3);
		ASSERT_ONLY(int a = (*t)[i1]);
		ASSERT_ONLY(int b = (*t)[i2]);
		mthAssertf(a!=b, "Degenerate triangle, two verts have the same index (%d)", a);
		mthAssertf( tris[t->n[i]]->neib(b,a) == t->id, "This tri isn't is neighbor's neighbor");
	}
}
void extrude(Tri *t0,int v)
{
	int3 t= *t0;
	int n = tris.count;
	Tri* ta = rage_new Tri(v,t[1],t[2]);
	ta->n = int3(t0->n[0],n+1,n+2);
	tris[t0->n[0]]->neib(t[1],t[2]) = n+0;
	Tri* tb = rage_new Tri(v,t[2],t[0]);
	tb->n = int3(t0->n[1],n+2,n+0);
	tris[t0->n[1]]->neib(t[2],t[0]) = n+1;
	Tri* tc = rage_new Tri(v,t[0],t[1]);
...
int4 FindSimplex(float3 *verts,int verts_count,Array<int> &allow)
{
	...
	if(dot(verts[p3]-verts[p0],cross(verts[p1]-verts[p0],verts[p2]-verts[p0])) <0) {SwapEm(p2,p3);}
...
int calchullgen(float3 *verts,int verts_count, int vlimit)
{
	...
	Tri *t0 = rage_new Tri(p[2],p[3],p[1]); t0->n=int3(2,3,1);
	Tri *t1 = rage_new Tri(p[3],p[2],p[0]); t1->n=int3(3,2,0);
	Tri *t2 = rage_new Tri(p[0],p[1],p[3]); t2->n=int3(0,1,3);
	Tri *t3 = rage_new Tri(p[1],p[0],p[2]); t3->n=int3(1,0,2);
...
int calchullpbev(float3 *verts,int verts_count,int vlimit, Array<HullPlane> &planes,float bevangle) 
{
	int i,j;
	planes.count=0;
	int rc = calchullgen(verts,verts_count,vlimit);
	...
	for(i=0;i<tris.count;i++)if(tris[i])
	{
		...
		for(j=0;j<3;j++)
		{
			...
			planes.Add(HullPlane(n,-dot(n,verts[maxdir(verts,verts_count,n)])));
		}
	}
...
int overhull(HullPlane *planes,int planes_count,float3 *verts, int verts_count,int maxplanes, 
...
//	inflate *=diameter;   // RELATIVE INFLATION
	...
	//return c;
	faces_out = (int*)malloc(sizeof(int)*(1+c->facets.count+c->edges.count));     // rage_new int[1+c->facets.count+c->edges.count];
	faces_count_out=0;
	i=0;
	...
	verts_out = c->vertices.element; // rage_new float3[c->vertices.count]; 
			 ...
			 float3 *&verts_out, int &verts_count_out,  int *&faces_out, int &faces_count_out ,float inflate,float bevangle,int vlimit)
{
	...
	extern int calchullpbev(float3 *verts,int verts_count,int vlimit, Array<HullPlane> &planes,float bevangle) ;
	Array<HullPlane> planes;
