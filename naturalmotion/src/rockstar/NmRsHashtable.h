...
template<typename T> struct HashType<T*> { typedef size_t CoerceTo; };
...
{
  static inline u32 hash(const KeyType key) 
  {
    return rage::atDataHash((const char*)&key, sizeof(key));
  }
};
...
protected:
  ...
  {
    result = THashFunction::hash(typename THashType::CoerceTo(key));
    result %= hashSize;
  }
...
bool NmRsHashtable<KeyType, ValueType>::insert(const KeyType& key, const ValueType& value)
{
  ...
  while (isSlotOccupied(tableIndex))
  {
    ...
    if (THashFunction::equal(typename THashType::CoerceTo(m_pairs[tableIndex].m_key), typename THashType::CoerceTo(key)))
    {
      return false;
    }
