...
namespace ART
{
  ...
  void NmRsCBUTeeter::falling(float timeStep)
  {
    ...
    {
      bool fallingReactionRunningElsewhere = highFallTask->isActive() || m_cbuParent->m_tasks[bvid_smartFall]->isActive();
      if ((!m_highFall) && (m_parameters.callHighFall || fallingReactionRunningElsewhere))
      {
        //Turn off teeter's own fall reaction 
        m_highFall = true;
        armsWindmillTask->deactivate();
        writheTask->deactivate();
