...
namespace ART
{
  ...
  void NmRsCBUShot::initialiseCustomVariables()
  {
    ...
    m_shotFromBehind.timer = 100000.0f;//Something large so that getShotFromBehindIsRunning will return false
  }
  ...
  void NmRsCBUShot::onActivate()
  {
    ...
    m_leftShoulderAngles.Set(10.0f,10.0f,10.0f);//Set high so that 1st shoulderAngVel is high
    m_rightShoulderAngles.Set(10.0f,10.0f,10.0f);//Set high so that 1st shoulderAngVel is high
  ...
  CBUTaskReturn NmRsCBUShot::onTick(float timeStep)
  {
    ...
    if ((m_hitTime < m_parameters.exagDuration + m_parameters.exagSmooth2Zero + m_parameters.exagZeroTime) && !m_parameters.melee)
    {
      ...
      if ((m_hitTime >= startTime) && (m_hitTime < startTimeRampDown))
        painResponse = 1.f;
      if ((m_hitTime >= startTimeRampDown) && (m_hitTime < zeroTime) && m_parameters.exagSmooth2Zero > 0.00001f)
        painResponse = (zeroTime - m_hitTime)/m_parameters.exagSmooth2Zero;
    ...
    else if (!m_parameters.melee)
    {
      ...
      float painResponse = 0.f; 
      if ((m_hitTime >= startTimeRampUp) && (m_hitTime < startTime) && (m_parameters.cpainSmooth2Time > 0.00001f))
        painResponse = (m_hitTime - startTimeRampUp)/m_parameters.cpainSmooth2Time;
      if ((m_hitTime >= startTime) && (m_hitTime < startTimeRampDown))
        painResponse = 1.f;
      if ((m_hitTime >= startTimeRampDown) && (m_hitTime < endTime) && (m_parameters.cpainSmooth2Zero > 0.00001f))
        painResponse = (endTime - m_hitTime)/m_parameters.cpainSmooth2Zero;
...
#if ART_ENABLE_BSPY
    m_character->setCurrentSubBehaviour("-fling2"); 
#endif
    substateTick(&NmRsCBUShot::fling2_entryCondition, &NmRsCBUShot::fling2_entry, &NmRsCBUShot::fling2_tick, &NmRsCBUShot::fling2_exitCondition, &NmRsCBUShot::fling2_exit, m_fling2Enabled, timeStep);
#if ART_ENABLE_BSPY
    m_character->setCurrentSubBehaviour("-sfb"); 
#endif
    //done here so that the backArchedBack and leanedTooFarBack balancer fails 
    // are inhibited when shotFromBehind is enabled and for sfbRecoveryPeriod after it has finished.
    if (m_shotFromBehind.timer <= m_parameters.sfbPeriod + sfbRecoveryPeriod)
      ...
      m_parameters.cpainSmooth2Time + m_parameters.cpainDuration + m_parameters.cpainSmooth2Zero))
    {
      ...
      float blendFactor = m_parameters.spineBlendZero + ( (1.0f - motionMultiplier) * (1.f-m_parameters.spineBlendZero));
...
#undef BAL_NAME_ACTION
    ...
    bspyTaskVar(m_parameters.flingTime, true);
    ...
    bspyTaskVar(m_parameters.reachFalling, true);
    bspyTaskVar(m_parameters.reachFallingWithOneHand, true);
    bspyTaskVar(m_parameters.reachOnFloor, true);
    ...
    bspyTaskVar(m_parameters.fallingReaction, true);
    ...
    if(m_parameters.fallToKnees)
    {
      ...
      bspyTaskVar(m_parameters.ftkLungeProb, true);
      bspyTaskVar(m_parameters.ftkKneeSpin, true);
      ...
      bspyTaskVar(m_parameters.ftkReachForWound, true);
    ...
    bspyTaskVar(m_parameters.bust, true);		
    bspyTaskVar(m_parameters.bustElbowLift, true);		
    bspyTaskVar(m_parameters.cupSize, true);		
    bspyTaskVar(m_parameters.cupBust, true);		
    ...
    bspyTaskVar(m_fallToKneesEnabled,false);
    if(m_fallToKneesEnabled)
    {
      bspyTaskVar(m_fTK.m_ftkStuckTimer, false);
    ...
    if(m_parameters.shotFromBehind)
    {
      ...
      bspyTaskVar(m_parameters.sfbIgnoreFail,true);
    }
    ...
    bspyTaskVar(m_parameters.shotInGuts,true);
    ...
    bspyTaskVar(m_fling2Enabled,false);
    bspyTaskVar(m_fling2.flingTimer,false);
    bspyTaskVar(m_fling2.period,false);
    bspyTaskVar(m_fling2.relaxTimeL,false);
    bspyTaskVar(m_fling2.lengthL,false);
    bspyTaskVar(m_fling2.angleL,false);
    bspyTaskVar(m_fling2.hasFlungL,false);
    bspyTaskVar(m_fling2.relaxTimeR,false);
    bspyTaskVar(m_fling2.lengthR,false);
    bspyTaskVar(m_fling2.angleR,false);
    bspyTaskVar(m_fling2.hasFlungR,false);

    bspyTaskVar(m_parameters.fling2RelaxTimeL,true);   
    bspyTaskVar(m_parameters.fling2RelaxTimeR,true);
    bspyTaskVar(m_parameters.fling2MStiffL,true);
    bspyTaskVar(m_parameters.fling2MStiffR,true);
    bspyTaskVar(m_parameters.fling2,true);
    bspyTaskVar(m_parameters.fling2Left,true);
    bspyTaskVar(m_parameters.fling2Right,true);
    bspyTaskVar(m_parameters.fling2OverrideStagger,true);   
    bspyTaskVar(m_parameters.fling2AngleMinL,true);
    bspyTaskVar(m_parameters.fling2AngleMaxL,true);
    bspyTaskVar(m_parameters.fling2Time,true);
    bspyTaskVar(m_parameters.fling2TimeBefore,true);
    bspyTaskVar(m_parameters.fling2AngleMinR,true);
    bspyTaskVar(m_parameters.fling2AngleMaxR,true);
    bspyTaskVar(m_parameters.fling2LengthMinL,true);
    bspyTaskVar(m_parameters.fling2LengthMaxL,true);
    bspyTaskVar(m_parameters.fling2LengthMinR,true);
    bspyTaskVar(m_parameters.fling2LengthMaxR,true);
