...
namespace ART
{
  ...
  {
#if ART_ENABLE_BSPY
    m_memUsed = rage::Max(ptrdiff_t_to_int(m_pCurrent - m_pStart), (int)m_memUsed);
    bspyLogf(info, L"limb manager memory used %d(%d)/%d", ptrdiff_t_to_int(m_pCurrent - m_pStart), (int)m_memUsed, (int)m_memAvailable);
#endif
