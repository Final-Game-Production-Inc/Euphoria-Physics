...
namespace ART
{
  ...
  BulletApplier::Setup BulletApplier::s_nextSetupExtra;
  ...
  // imp and position are global
  // ---------------------------------------------------------------------------------------------------------
  void BulletApplier::newHit(int partID, const rage::Vector3 &imp, rage::Vector3 &position, float equalizer)
  {
    ...
    // Extra bullet does not contribute to impulseLeakage but is affected by it.
    if (!m_extra)
    {
      ...
      m_character->m_impulseLeakage = rage::Clamp(m_character->m_impulseLeakage, m_character->m_impulseMinLeakage, 1.f);
    }
  ...
  void BulletApplier::doPartMassCompensation(int partID, rage::Vector3 &impulse, float amount)
  {
    ...
    float massProp = m_character->getArticulatedBody()->GetMass(partID).Getf() / averageMass;
  ...
  // ---------------------------------------------------------------------------------------------------------
  // NB: start has to be < end (if it isn't then negate them and negate the value when calling this function)
  //  Scale
  //     |
  // 1.0 |_______
  //     |       \
  ...
  // min |           \____
  //     |
  //     |________________ val
  //             |    |
  //          start   |
  //                 end
  // ---------------------------------------------------------------------------------------------------------
  float BulletApplier::rampDown(const float val, const float start, const float end, const float min)
  {
     float scale = 1.0f;
     if (val >= end)
     {
     ...
     else if (val > start)
     {
       ...
       {
         scale = min + (1.0f-min)*(end - val)/interval;
       }
       //else bad max and min (max<min) or min/max equal but val not > max - don't scale - scale =1.0f
     }
     //else (val < start) don't scale - scale =1.0f
     return scale;
  }
  ...
  void BulletApplier::applyImpulse()
  {
      ...
      if(m_impulseTimer + timeStep > m_setup.impulseDelay)
      {
        ...
        if(m_setup.impulsePeriod > 0.0001f)
        {
          ...
          case straightProfile:
            forceRatioThisStep = areaUnderStraightProfile(m_setup.impulsePeriod, timeNow, timeStep);  
            break;
        ...
        float noBalMult = 1.0f;
        float balStabMult = 1.0f;
        float spineAngMult = 1.0f;
        float velMult = 1.0f;

        //Balancer dependent scalings
        float balStab = dynamicBalancerTask->m_balanceInstability;
        if (dynamicBalancerTask->m_failType != dynamicBalancerTask->balOK)
          noBalMult = m_setup.impulseNoBalMult;
        else
          balStabMult = rampDown(balStab, m_setup.impulseBalStabStart, m_setup.impulseBalStabEnd, m_setup.impulseBalStabMult);

        //Spine angle dependent scalings
        NmRsHumanSpine* spine = m_character->getBody()->getSpine();
        rage::Vector3 hip2head = spine->getSpine3Part()->getPosition() - spine->getSpine0Part()->getPosition();
        hip2head.Normalize();
        float spineDotUp = hip2head.Dot(m_character->m_gUpReal);
        //Start is 
        spineAngMult = rampDown(-spineDotUp, -m_setup.impulseSpineAngStart, -m_setup.impulseSpineAngEnd, m_setup.impulseSpineAngMult);

        //Velocity dependent scalings
        float comVelMag = m_character->m_COMvelRelativeMag;
        velMult = rampDown(comVelMag, m_setup.impulseVelStart, m_setup.impulseVelEnd, m_setup.impulseVelMult);

        float totalScale = noBalMult*balStabMult*spineAngMult*velMult;
#if ART_ENABLE_BSPY && BulletBSpyScratchPad
        bspyScratchpad(characterID, m_impStr, noBalMult);
        bspyScratchpad(characterID, m_impStr, balStab);
        bspyScratchpad(characterID, m_impStr, balStabMult);
        bspyScratchpad(characterID, m_impStr, spineDotUp);
        bspyScratchpad(characterID, m_impStr, spineAngMult);
        bspyScratchpad(characterID, m_impStr, comVelMag);
        bspyScratchpad(characterID, m_impStr, velMult);
        bspyScratchpad(characterID, m_impStr, totalScale);
#endif
        //Don't apply the bullet or do expensive rigid body bullet calcs if no force is to be applied
        if (totalScale > 0.00000001f)
        {
          forceNow *= totalScale;
  ...
  #endif
          //Apply rigid body bullet
          if (rbRatio > 0.00000001f)
          {
            ...
            if (splitRBForce) 
            {
              float forceNowAdjusted = (1.f - m_character->getArticulatedBody()->GetMass(m_impulsePartID).Getf()/m_character->getTotalMass());
              float upperBodyMass = m_character->getUpperBodyMass();
              float lowerBodyMass = m_character->getLowerBodyMass();
              if (m_impulsePartID < 8)
                lowerBodyMass -= m_character->getArticulatedBody()->GetMass(m_impulsePartID).Getf();
              else
                upperBodyMass -= m_character->getArticulatedBody()->GetMass(m_impulsePartID).Getf();
              ...
              lowerShare /= (lowerBodyMass+upperBodyMass);//i.e. /(m_character->getTotalMass() - m_character->getArticulatedBody()->GetMass(m_impulsePartID).Getf())
            ...
            {
              massProp = m_character->getArticulatedBody()->GetMass(i).Getf();
    ...
    #define DISPLAY_COMPONENTS 0
          ...
          //Apply the normal bullet
          if ((1.f - rbRatio) > 0.0000001f)
          {
          ...
          //and a moment applied to the part
          ...
          {
            forceNow *= 0.5f;
            part = m_character->getBody()->getLeftLeg()->getShin();
            part->applyImpulse(forceNow, part->getPosition());
            part = m_character->getBody()->getRightLeg()->getShin();
            part->applyImpulse(forceNow, part->getPosition());
            forceNow *= 0.5f/0.75f/0.5f;
            part = m_character->getBody()->getSpine()->getPelvisPart();
            part->applyImpulse(-forceNow, part->getPosition());
          }
  #endif
#if NM_RIGID_BODY_BULLET
      ...
      if(m_impulseTimer >= (m_setup.impulseDelay + m_setup.impulsePeriod))
        m_doImpulse = false;
    } // if do impulse
  ...
  { 
	...
	bspyScratchpad(characterID, m_idStr,  m_character->m_impulseMinLeakage);
    ...
    bspyScratchpad(characterID, m_idStr, m_setup.impulseNoBalMult);
    bspyScratchpad(characterID, m_idStr, m_setup.impulseBalStabStart);
    bspyScratchpad(characterID, m_idStr, m_setup.impulseBalStabEnd);
    bspyScratchpad(characterID, m_idStr, m_setup.impulseBalStabMult);
    bspyScratchpad(characterID, m_idStr, m_setup.impulseSpineAngStart);
    bspyScratchpad(characterID, m_idStr, m_setup.impulseSpineAngEnd);
    bspyScratchpad(characterID, m_idStr, m_setup.impulseSpineAngMult);
    bspyScratchpad(characterID, m_idStr, m_setup.impulseVelStart);
    bspyScratchpad(characterID, m_idStr, m_setup.impulseVelEnd);
    bspyScratchpad(characterID, m_idStr, m_setup.impulseVelMult);
