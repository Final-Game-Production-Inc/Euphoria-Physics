...
namespace ART
{
  ...
  CBUTaskReturn NmRsCBUFallOverWall::onTick(float timeStep)
  {
    ...
    if (!m_hitWall)
    {
      rage::Vector3 hitPos(0,0,0);
      rage::Vector3 hitNormal(1,0,0);
      ...
      m_wallHitPos = hitPos;//will Be wrong if !m_hitWall
      ...
      if (rage::Abs(m_character->m_gUp.Dot(hitNormal)) > 0.7f)//not horizontal enough
      {
...
#if ART_ENABLE_BSPY && FallOverWallBSpyDraw
    if (m_hitWall)//m_wallHitPos wrong if !m_hitWall
      m_character->bspyDrawPoint(m_wallHitPos, 0.2f, rage::Vector3(1,0,0));
  ...
  {
    float partMass = m_character->getArticulatedBody()->GetMass(part->getPartIndex()).Getf();
