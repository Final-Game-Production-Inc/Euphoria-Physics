...
namespace ART
{
  ...
  void NmRsCBULearnedCrawl::enablePartFriction(NmRsGenericPart *part, bool enable)
  {
    ...
    m_character->setFrictionPreScale((enable ? friction : 0.1f)/(part->getBound()->GetMaterial(0).GetFriction()), m_character->indexToMask(part->getIndex()));
  }
  ...
  {
    m_character->setFrictionPreScale(frictionM, partToMask(part->getIndex());
  }
  ...
  void NmRsCBULearnedCrawl::onActivate()
  {
    ...
    if (m_parameters.bLearn)
    {
      ...
      if(m_parameters.learnFromAnimPlayback)
      {
        ...
        m_ctmTransforms = rage_new rage::Matrix34[m_numberOfFrames*21];
    ...
    if(m_parameters.bLearn) // do learning
    {
      ...
      {  
        ...
        {  
          ...
          {    
            // save out the desired joint angles:
            fwrite(matrices, sizeof(rage::Matrix34), NUM_PARTS, m_out);
          }
        }
