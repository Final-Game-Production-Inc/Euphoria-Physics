...
namespace ART
{
  ...
  void NmRsCBUBalancerCollisionsReaction::initialiseCustomVariables()
  {
    ...
    m_footFrictionMult = 1.0f;
    m_backFrictionMult = 1.0f;
    m_amountOfMovement = 0.0f;
    m_stableTimer = 0.f;
    m_drapeTimer = 0.f;
    m_balancing = true;
  ...
  CBUTaskReturn NmRsCBUBalancerCollisionsReaction::onTick(float timeStep)
  {
    ...
    //BCR can get confused if collision with the floor sets it off.  This doesn't happen as the balancer generally fails before hitting the floor.
    //  If the balancer fail has been inhibited then bcr takes a conservative view of balancer failure
    //  i.e. balancer has failed if if the balancer is ignoring a failure due to ignoreFailure = true
    //    or when the balancer has exceeded default failure conditions and failMustCollide = true
    //  e.g. for underwater shot (ignoreFailure = true)
    //       shotgun reaction (failMustCollide = true) 
    //Cannot be balancing after it has stopped balancing - very conservative 
    //  (we could perhaps switch balancing back on if we are sure the character is on it's feet on the ground)
    if (m_balancing)
      m_balancing = (dynamicBalancerTask->m_failType == NmRsCBUDynamicBalancer::balOK &&
                     !(dynamicBalancerTask->getIgnoringFailure() || (dynamicBalancerTask->getFailMustCollide() && dynamicBalancerTask->m_failedIfDefaultFailure)) );
    ...
    if ((!(m_balancerState == bal_Drape || m_balancerState == bal_DrapeForward)) && 
      (m_previousBalancerState == bal_Drape || m_previousBalancerState == bal_DrapeForward))
    {
    ...
    {
      if (m_balancing && m_balancerState > bal_Impact)
      {
        if (m_timeAfterImpact > 0.0001f && m_timeAfterImpact < m_parameters.reactTime && m_balancing)
      ...
      if (m_controlStiffnessStrengthScale < 1.0f)
      {
        ...
        if(m_timeAfterImpact >= m_parameters.impactWeaknessZeroDuration)
          m_controlStiffnessStrengthScale = rage::Min(1.f, m_controlStiffnessStrengthScale + timeStep/m_parameters.impactWeaknessRampDuration);//mmmmmDIVBy0 - covered by param MIN
  ...
  void NmRsCBUBalancerCollisionsReaction::decideBalancerState(float timeStep, int numOfSteps)
  {
    ...
    if (!m_impactOccurred && (!m_balancing))
	...
	float impactHeightFromGround = 0.0f;
    ...
    if ((!m_ubImpactOccurred && !m_block) || updatePivotPoint)//Don't need to look for collisions after an upperbody collision or we've decided it's a block
    {
      ...
      if (getLeftLeg()->getThigh()->collidedWithNotOwnCharacter())
      {
		...
		impactHeightFromGround = colHeight - floorHeight;
        //if (collisionInst->GetClassType() == rage::NM_RS_INSTNM_CLASS_TYPE || collisionInst->GetClassType() == rage::PH_INST_FRAG_PED)
        if (impactHeightFromGround > 0.1f && m_character->IsInstValid(collisionInst, collisionInstGenID) && collisionInst->GetLevelIndex() != m_parameters.ignoreColWithIndex)
        {
    ...
    if(impactOccurred)//otherwise collisionPos may not have been set
    {
    ...
    float exclusionZone = 0.4f;//for forwards impact rebound force and for rebound reaction identification
    if ((m_ubImpactOccurred) && (m_balancerState == bal_Impact)) //ie not glancing
    {
      ...
      {
        //hit facing forwards - don't let it just lean against the wall (with the head etc)
        //Tried and failed: leaningAway, turning, glancingSpin
        //NB: Slight chance that the rebound force with fallToKnees could give floating equilibrium that never comes out of bal_Impact 

        //only apply cheat force if infront of walland inside rebound zone
        float spine3DistFromWall = m_normal1stContact.Dot(getSpine()->getSpine3Part()->getPosition()-m_pos1stContact);
        if (m_parameters.reboundForce > 0.0f && spine3DistFromWall*m_sideOfPlane < 0.f && rage::Abs(spine3DistFromWall) < exclusionZone)
        {
          getSpine()->getSpine3Part()->applyImpulse(m_parameters.reboundForce*m_normal1stContact,getSpine()->getSpine3Part()->getPosition());
          getLeftArm()->getHand()->applyImpulse(-m_parameters.reboundForce*0.3f*m_normal1stContact,getLeftArm()->getHand()->getPosition());
          getRightArm()->getHand()->applyImpulse(-m_parameters.reboundForce*0.3f*m_normal1stContact, getRightArm()->getHand()->getPosition());
        }
        
        NmRsCBUBodyBalance* bodyBalanceTask = (NmRsCBUBodyBalance*)m_cbuParent->m_tasks[bvid_bodyBalance];
        Assert(bodyBalanceTask);
        //Use bodyBalance brace arms to push away from wall
        if (m_parameters.braceWall && bodyBalanceTask->isActive())
        {
          //do this in bodyBalance to preserve bodyBalance's settings?
          //If the rebound mode is reset then brace and headlook target will be the last seen in this state.
          //  We could set the bodyBalance parameters back to remembered ones...
          //    but I quite like the look of leaving them like this as if the character stumbles back into the wall after restart it looks aware of it and braces against it
          bodyBalanceTask->m_parameters.m_headLookPos = m_pos1stContact;
          bodyBalanceTask->m_parameters.braceDistance = 0.5f;
          m_character->levelVectorReal(bodyBalanceTask->m_parameters.m_headLookPos, getSpine()->getSpine3Part()->getPosition().Dot(m_character->m_gUpReal));
        }

        //Forwards impact reaction is slump or fallToKnees if shot is running
        NmRsCBUShot* shotTask = (NmRsCBUShot*)m_cbuParent->m_tasks[bvid_shot];
        Assert(shotTask);
        if (m_timeAfterImpact > 0.0001f && m_timeAfterImpact > m_parameters.timeToForward)
        {
          if ((m_parameters.forwardMode == 1) && shotTask->isActive())
          {
            //parameters to try to fall backwards away from wall
            shotTask->m_parameters.ftkBalanceTime = 0.0f;
            shotTask->m_parameters.ftkFallBelowStab = 10.0f;
            shotTask->m_parameters.ftkHelperForce = 0.0f;
            shotTask->m_parameters.ftkLeanHelp = 0.0f;
            shotTask->m_parameters.ftkHipAngleFall = 0.0f;
            shotTask->m_parameters.ftkPitchForwards = -0.1f;
            shotTask->m_parameters.ftkPitchBackwards = -0.1f;           
            shotTask->m_parameters.fallToKnees = true;
          }
          else
            m_balancerState = bal_Slump;
        }
    ...
    if ((m_impactOccurred) && (m_balancerState != bal_GlancingSpin) && (m_balancerState != bal_DrapeGlancingSpin))
    {
      ...
      if ((comVelDir.Dot(m_normal1stContact) > 0.2f) && (m_normal1stContact.Dot(m_character->m_COM-m_pos1stContact-m_normal1stContact*exclusionZone)*m_sideOfPlane < 0.f))//increase offset
      {
        ...
        if (m_parameters.reboundMode == 0)//fall2knees if shot running/slump if shot not running
        {
        ...
        else if (m_parameters.reboundMode == 1)//stumble
        {
          ...
          if (shotTask->isActive())
            shotTask->deactivate();
#else
          dynamicBalancerTask->setMaximumBalanceTime(dynamicBalancerTask->getTimer() + 1.0f);
          //keep shot on here otherwise would have to start a balancing behaviour
    ...
    if ((m_lbImpactOccurred) && (!m_ubImpactOccurred) && (!m_block) && (!m_ignoreBlock))
    {
      ...
      if (m_block)
      {
        ...
        {
			// Don't run drape/FOW if the collision was caused by a ped and the impact is fairly high off the ground (PH_INST_PED refers to a ped's animated bounds, PH_INST_FRAG_PED refers the a ped's ragdoll bounds)
			static float sf_MaxImpactHeightToAllowFOW = 0.4f;
			if (!(collisionInst && (collisionInst->GetClassType() == PH_INST_PED || collisionInst->GetClassType() == PH_INST_FRAG_PED) && 
				impactHeightFromGround > sf_MaxImpactHeightToAllowFOW))
			{
    ...
    if (m_balancerState == bal_DrapeGlancingSpin && (!m_balancing) && m_block)
    {
      rage::Vector3 hip2Hip = getRightLeg()->getHip()->getJointPosition() -  getLeftLeg()->getHip()->getJointPosition();
      rage::Vector3 sideVector;
      sideVector = m_normal1stContact;
  ...
  */
  ...
  void NmRsCBUBalancerCollisionsReaction::slump(float timeStep)
  {
    ...
    m_character->setFrictionPreScale(m_footFrictionMult, bvmask_FootLeft | bvmask_FootRight);
  ...
  void NmRsCBUBalancerCollisionsReaction::exaggerateImpact()
  {
    ...
    Assert(flinchTask);
    m_pushingOff = false;
    if (m_timeAfterImpact > 0.0001f && (m_timeAfterImpact < m_parameters.reactTime || (m_timeAfterImpact < 2.0f*m_parameters.reactTime && m_balancerState != bal_LeanAgainst)) && m_balancing)
    {
      m_pushingOff = true;
    ...
    //if (m_timeAfterImpact > 0.0001f && m_timeAfterImpact < m_parameters.impactExagTime && m_balancing)
    if (m_timeAfterImpact > 0.0001f && m_timeAfterImpact < m_parameters.reactTime && m_balancing)
    {
      ...
      speed *= -0.5f*rage::Max(0.f,1.f-m_timeAfterImpact/m_parameters.reactTime);//max 0.6//mmmmmDIVBy0 protected by if above
  ...
  {
    m_character->setFrictionPreScale(1.0f, (bvmask_Trunk & ~bvmask_CervicalSpine) | bvmask_FootLeft | bvmask_FootRight);
  }
  ...
  void NmRsCBUBalancerCollisionsReaction::drape(float timeStep)
  {
    ...
    if (fowTask->isActive())
    {
      ...
      else if (m_headCollided)// && m_balancing)//mmmmfow
      {
        fowTask->deactivate();
        dynamicBalancerTask->setForceBalance(false);//mmmmfow
      }
    ...
    else
    {
      ...
      if (m_drapeTimer > 0.3f && m_balancing)
      {
        m_balancerState = bal_LeanAgainst;
        m_ignoreBlock = true;
        rdsTask->deactivate();
    ...
    if ((m_balancerState == bal_DrapeForward) || fowTask->isActive())
    {
      ...
      if ((!m_balancing) && (!fowTask->isActive()))
      {
        m_balancerState = bal_LeanAgainst;
        m_ignoreBlock = true;
      }
  ...
  void NmRsCBUBalancerCollisionsReaction::probeEnvironment(const rage::Vector3& contactPos, const rage::Vector3& contactNormal)
  {
    ...
    probeTo -= m_character->m_gUp*depth*1.73205f;
  ...
  NmRsCBUBalancerCollisionsReaction::ObstacleTypes NmRsCBUBalancerCollisionsReaction::getProbeResult(const rage::Vector3& contactPos, const rage::Vector3& contactNormal)
  {
    ...
    if (probeResult == NmRsCharacter::probe_Hit)
    {
      ...
      const float impactPlaneThickness = 0.02f;//Increase if you get false table id from walls (e.g. bal_GlancingSpin will turn into bal_DrapeGlancingSpin)
...
#undef BCR_NAME_ACTION
    ...
    bspyTaskVar(m_parameters.forwardMode, true);   
    bspyTaskVar(m_parameters.timeToForward, true); 
    bspyTaskVar(m_parameters.braceWall, true);  
    bspyTaskVar(m_parameters.reboundForce, true);  
