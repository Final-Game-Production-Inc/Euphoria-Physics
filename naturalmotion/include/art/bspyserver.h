...
namespace bSpy
{
 ...
 namespace ART
 {
  ...
  public:
    ...
    const char* getStringForToken(bSpy::bSpyStringToken);
...
#if defined(NM_IA64) && !defined(NM_PLATFORM_ORBIS)
...
# define nmsock_error           errno
#elif defined(NM_PLATFORM_ORBIS)
# define nmsock_close           sceNetSocketClose
# define nmsock_select          select
// send() terminates a process with SIGPIPE for stream-oriented sockets when the destination disconnects the connection.
// This problem does not occur for sceNetSend()
# define nmsock_send            sceNetSend
# define nmsock_error			sce_net_errno
#elif defined(NM_PLATFORM_CELL_PPU)
# define nmsock_close           socketclose
# define nmsock_select          socketselect
# define nmsock_send            send
# define nmsock_error           sys_net_errno
...
typedef  rage::sysIpcThreadId	ThreadHandle;
typedef  rage::sysIpcSema       EventHandle;
...
namespace ART
{
  ...
  public:
    ...
    inline size_t getSessionUID() const { return m_sessionUID; }
    ...
    const char* getStringForToken(bSpy::bSpyStringToken token);
  ...
  public:
    ...
    inline const char* findString(bSpy::bSpyStringToken token);
  ...
  protected:
    ...
    struct HChunk
    {
      ...
      inline unsigned int findHighestSetBit()
      {
        ...
        return MultiplyDeBruijnBitPosition[((unsigned int)(v * 0x077CB531UL)) >> 27];
      }
  ...
  {
    unsigned int bkIndex = rage::atDataHash((const char*)&key, sizeof(key));
    bkIndex &= (TABLESIZE - 1);
  ...
  /*
   *  Token to string lookup to support NM_RS_VALIDATE_VITAL_VALUES. Note: This
   *  is *not* efficient at all (has to iterate the map because we're looking
   *  for a specific value, not a key), so if we're seeing a lot of validate
   *  output, we need to rethink this.
   */
  const char* bSpyStringMap::findString(bSpy::bSpyStringToken token)
  {
    int chunkIndex = 0;
    HChunk* chunk;

    while (chunkIndex < TABLESIZE)
    {
      chunk = m_buckets[chunkIndex++];
      if(chunk)
      {
        unsigned int i = 0, highBit = 31; 
        if (chunk->m_usage != 0xFFFFFFFF)
        ...
        for (; i<=highBit; ++i)
        {
          if ((chunk->m_usage & (1 << i)))
          {
            if (chunk->m_values[i].m_token == token)
            {
              return chunk->m_values[i].m_string;
            }
          }
        }
      }
...
#define bspyTaskVar_StringEnum(mbv, mbvStrings, isParameter) \
  {\
    CompileTimeAssert((mbv >=0) && (mbv <= sizeof(mbvStrings) / sizeof(const char*)));\
...
#define BSPY_DEF_SP_VARFN_BASIC(_type, _typeMemb) \
  ...
  if (ART::bSpyServer::inst() && ART::bSpyServer::inst()->isClientConnected() && ART::bSpyServer::inst()->shouldTransmit(bSpy::TransmissionControlPacket::bSpyTF_Scratchpad) && (bSpy::bs_uint32)_agentID != INVALID_AGENTID)\
  {\
    static bSpy::bs_uint8 __callstamp = 0;\
    static bSpy::bs_uint32 __framestamp = 0;\
    if (__framestamp != ART::bSpyServer::inst()->getFrameTicker())\
