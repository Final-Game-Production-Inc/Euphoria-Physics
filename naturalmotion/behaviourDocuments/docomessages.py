			...
			while line.find('}') == -1:
				...
				if parameterLine and (not commentedOutParameterLine):
					...
					if paramType.find("float") != -1 or paramType.find("vector3") != -1:
						...
						if paramDefault.find("PI") != -1:
							paramDefault = re.sub('PI', 'math.pi', paramDefault)
							paramDefault = "{0:.6f}".format(eval(paramDefault))
					...
					if paramMin.find("PI") != -1:
						paramMin = re.sub('PI', 'math.pi', paramMin)
						paramMin = "{0:.6f}".format(eval(paramMin))
					...
					if paramMax.find("PI") != -1:
						paramMax = re.sub('PI', 'math.pi', paramMax)
						paramMax = "{0:.6f}".format(eval(paramMax))
