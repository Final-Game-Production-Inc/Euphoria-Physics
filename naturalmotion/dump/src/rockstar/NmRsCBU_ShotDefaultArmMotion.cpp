...
namespace ART
{
    ...
    bspyScratchpad(m_character->getBSpyID(), "Default arms", reachSpeed);
      ...
      //releaseWound doesn't take into account stagger or bb arms being used as default reaction
    ...
    void NmRsCBUShot::defaultArmMotion_deDefaultMotion()
    {
      ...
      {
        getLeftArm()->setBodyStiffness(getLeftArmInput(), (8.0f + 2.0f*fallMag)*m_upperBodyStiffness, 1.0f, bvmask_ArmLeft & ~bvmask_ForearmLeft);
        getLeftArmInputData()->getElbow()->setStiffness((4.0f + 2.0f*fallMag)*m_upperBodyStiffness, 1.0f);
        ...
        if (setAngles)
        {
          getLeftArmInputData()->getShoulder()->setDesiredAngles(
            ...
            (l2 + getLeftArm()->getShoulder()->getMidLean2())*fallMag + (0.2f)*(1.0f-fallMag), 0.0f);
          getLeftArmInputData()->getClavicle()->setDesiredAngles(0.f, (l2 + getLeftArm()->getClavicle()->getMidLean2())*fallMag, 0.f);
          getLeftArmInputData()->getElbow()->setDesiredAngle(1.f*fallMag + 0.7f*(1.f-fallMag));//i.e. 0.7 + 0.3*fallMag 

          //blend to zero pose (always set for the arms)
          getLeftArmInputData()->getClavicle()->blendToZeroPose(getLeftArm()->getClavicle(),1.0f-fallMag);
          getLeftArmInputData()->getShoulder()->blendToZeroPose(getLeftArm()->getShoulder(),1.0f-fallMag);
          getLeftArmInputData()->getElbow()->blendToZeroPose(getLeftArm()->getElbow(),1.0f-fallMag);
          //wrists set to zeroPose (always set for the arms)
          getLeftArmInputData()->getWrist()->blendToZeroPose(getLeftArm()->getWrist(), 1.0f);
      }
#if ART_ENABLE_BSPY
      ...
      {
        getRightArm()->setBodyStiffness(getRightArmInput(), (8.f + 2.f*fallMag)*m_upperBodyStiffness, 1.f, bvmask_ArmRight & ~bvmask_ForearmRight);
        getRightArmInputData()->getElbow()->setStiffness((4.f + 2.f*fallMag)*m_upperBodyStiffness, 1.f);
        ...
        getRightArmInputData()->getShoulder()->setOpposeGravity(fallMag);//mmmnote never unset
        ...
        if (setAngles)
        {
          ...
          getRightArmInputData()->getElbow()->setDesiredAngle(0.8f*fallMag);

          //blend to zeroPose (always set for the arms)
          getRightArmInputData()->getClavicle()->blendToZeroPose(getRightArm()->getClavicle(),1.0f-fallMag);
          getRightArmInputData()->getShoulder()->blendToZeroPose(getRightArm()->getShoulder(),1.0f-fallMag);
          getRightArmInputData()->getElbow()->blendToZeroPose(getRightArm()->getElbow(),1.0f-fallMag);
          //wrists set to zeroPose (always set for the arms)
          getRightArmInputData()->getWrist()->blendToZeroPose(getRightArm()->getWrist(),1.0f);
       }
      }
    ...
    */
    void NmRsCBUShot::defaultArmMotion_armsBrace()    
    {
      ...
      const float backTwist = 0.2f;//-0.35
      const float forwardTwist = 0.2f;//-0.5
      const float forwardOffset = 0.1f;
      const float rightNessWindow = 0.4f;
      ...
      rage::Vector3 braceTarget;
      float armLength = 0.628f;//this could be measured e.g. using getLeftArmLength() in catchFall
      if (m_character->getBodyIdentifier() == gtaWilma)
        armLength = 0.58f;
      //if the maxArmLength is > armLength the arm doesn't go straight as the minElbow angle is set to 0.3
      float maxArmLength = armLength + m_character->getRandom().GetRanged(-0.117955f,0.022f);//0.55 quite bent, 0.6 Just bent, 0.7 straight
      rage::Matrix34 tmCom;
      getSpine()->getSpine1Part()->getBoundMatrix(&tmCom); 
      ...
      m_character->levelVector(forwardDir);
        rightness = reachVel.Dot(rightDir);
        if (rage::Abs(rightness) < rightNessWindow)
          rightness = -rightNessWindow;//left arm out from body if stationary

        forwardness = reachVel.Dot(forwardDir)+forwardOffset;
        goingBackwards = false;
#if ART_ENABLE_BSPY
      ...
      if (forwardness < 0.f)
      {
        ...
        armTwist = forwardTwist;//arms in front or to side, 0 = arms a bit wider(1.0f for behind)
        ...
        NmRsLimbInput ikInput = createNmRsLimbInput<NmRsIKInputWrapper>(-3, 1.0f DEBUG_LIMBS_PARAMETER("IK"));
        ...
        ikInputData->setMinimumElbowAngle(0.3f);
        getLeftArm()->postInput(ikInput);
        getLeftArmInputData()->getClavicle()->setDesiredAngles(0.0f, 0.0f, 0.0f);

        // by default, characters tip hands up (lean2) in preparation for connecting with object.  Twist is set to give more palms down than thumb up.
        float desiredLean2 = 0.f;
        if (leftHandRelativeHeight < 0.7f)
        {
          desiredLean2 -= 2.5f*(0.7f-leftHandRelativeHeight);
        }
        getLeftArmInputData()->getWrist()->setDesiredAngles(0.0f, desiredLean2, 0.24f);
        
        //mmmmarms this could be added by changing maxArmLength
        //// limbs: this will always be one frame behind as it is driving off of IK results.
        //// looks like the intention is to blend with the results of IK.
        //if (goingBackwards)
        //{
        //  getLeftArmInputData()->getElbow()->setDesiredAngle(getLeftArmInputData()->getElbow()->getDesiredAngle() + 0.5f*rage::Sinf(5.f*m_defaultArmMotion.timer2));
        //}
        
        if (m_character->m_COMvelRelativeMag < 0.5f)
        {
          float blend = 1.0f-(2.0f*m_character->m_COMvelRelativeMag);
          {
            NmRsBodyStateHelper helper(
            m_character->getBody(), 
            bvid_shot,
            (int)bvid_shot, 1,
            blend,bvmask_ArmLeft
            DEBUG_LIMBS_PARAMETER("brblend"));

            getLeftArmInputData()->getClavicle()->blendToZeroPose(getLeftArm()->getClavicle(),1.0f);
            getLeftArmInputData()->getShoulder()->blendToZeroPose(getLeftArm()->getShoulder(),1.0f);
            getLeftArmInputData()->getElbow()->blendToZeroPose(getLeftArm()->getElbow(),1.0f);
          }
          //wrists set to zeroPose (always set for the arms)
          getLeftArmInputData()->getWrist()->blendToZeroPose(getLeftArm()->getWrist(),1.0f);
        }
      ...
      m_character->levelVector(forwardDir);
        ...
        rightness = reachVel.Dot(rightDir)+forwardOffset;
        if (rage::Abs(rightness) < rightNessWindow)
          rightness = rightNessWindow;//right arm out from body if stationary
      ...
      if (forwardness < 0.f)
      {
        ...
        maxArmLength = armLength + m_character->getRandom().GetRanged(-0.12f,0.04f);//0.55 quite bent, 0.6 Just bent, 0.7 straight
        // clamp left arm not to reach too far
        mag = braceTarget.Mag();
        braceTarget.Normalize();
        braceTarget *= rage::Min(mag , maxArmLength);
        ...
        getRightArmInputData()->getWrist()->setDesiredAngles(0.0f, desiredLean2, 0.2f);
        
        //// limbs: this will always be one frame behind as it is driving off of IK results.
        //// looks like the intention is to blend with the results of IK.
        //if (goingBackwards)
        //{
        //  getRightArmInputData()->getElbow()->setDesiredAngle(getRightArmInputData()->getElbow()->getDesiredAngle() + 0.5f*rage::Sinf(5.f*m_defaultArmMotion.timer1));
        //}
        ...
        if (m_character->m_COMvelRelativeMag < 0.5f)
        {
          float blend = 1.0f-(2.0f*m_character->m_COMvelRelativeMag);
          {
            NmRsBodyStateHelper helper(
              ...
              bvid_shot,
              (int)bvid_shot, 1,
              blend,bvmask_ArmRight
              DEBUG_LIMBS_PARAMETER("brblend"));
            //blend to zeroPose (always set for the arms)
            getRightArmInputData()->getClavicle()->blendToZeroPose(getRightArm()->getClavicle(),1.0f);
            getRightArmInputData()->getShoulder()->blendToZeroPose(getRightArm()->getShoulder(),1.0f);
            getRightArmInputData()->getElbow()->blendToZeroPose(getRightArm()->getElbow(),1.0f);
        }
          //wrists set to zeroPose (always set for the arms)
          getRightArmInputData()->getWrist()->blendToZeroPose(getRightArm()->getWrist(),1.0f);
    }
