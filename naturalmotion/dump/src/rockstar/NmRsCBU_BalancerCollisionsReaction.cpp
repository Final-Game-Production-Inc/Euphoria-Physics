  ...
  void NmRsCBUBalancerCollisionsReaction::decideBalancerState(float timeStep, int numOfSteps)
  {
    ...
    if ((m_lbImpactOccurred) && (!m_ubImpactOccurred) && (!m_block) && (!m_ignoreBlock))
    {
      ...
      if (m_block)
      {
        ...
        {
			    // Don't run drape/FOW if the collision was caused by a ped and the impact is fairly high off the ground and with either thigh
          //  (PH_INST_PED refers to a ped's animated bounds, PH_INST_FRAG_PED refers the a ped's ragdoll bounds)
