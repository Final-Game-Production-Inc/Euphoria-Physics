...
namespace ART
{
  ...
  void NmRsCBURollDownStairs::onActivate()
  {
    ...
    if (!m_parameters.m_onlyApplyHelperForces)
    {
      ...
      rollUpTask->m_parameters.applyMinMaxFriction = m_parameters.applyMinMaxFriction;
      rollUpTask->m_parameters.m_effectorMask = bvmask_Full;
      // .. and activate it
      rollUpTask->activate();
    }
  ...
  CBUTaskReturn NmRsCBURollDownStairs::onTick(float timeStep)
  {
    ...
    if (!m_parameters.m_onlyApplyHelperForces)
    {
      if (m_parameters.m_StiffnessDecayTime > NM_RS_FLOATEPS)
      {
    ...
    NmRsCBURollUp* rollUpTask = (NmRsCBURollUp*)m_cbuParent->m_tasks[bvid_bodyRollUp];
    Assert(rollUpTask);
    if (!m_parameters.m_onlyApplyHelperForces && !rollUpTask->isActive())
      {
        // configure roll-up task
        rollUpTask->updateBehaviourMessage(NULL); // initialise parameters
        rollUpTask->m_parameters.m_useArmToSlowDown = m_parameters.m_UseArmsToSlowDown;
    ...
    if (!m_parameters.m_applyNewRollingCheatingTorques)
    {
      ...
      {
        float spinReductionScale = 1.0f;
        if(m_parameters.limitSpinReduction)
        {
          // Scale zAxisSpinReduction back when rotating end-over-end (somersault)
          // to give the body a chance to align with the axis of rotation.
          //
          // Scale wih alignment of long axis to axis of rotation.
          //
          rage::Vector3 comRotVelNorm(m_character->m_COMrotvel/m_character->m_COMrotvelMag);
          float rotVelScale = rage::Abs(comRotVelNorm.Dot(m_character->m_COMTM.b));
          rage::Vector3 comVelNorm(m_character->m_COMvel/m_character->m_COMvelMag);
          // Scale with body alignment flat and perpendicular to direction of travel.
          //
          // Measure of lying-flatness.  Inverse abs character up dot world up.
          //
          float lyingFlat = 1.0f - rage::Abs(m_character->m_COMTM.b.Dot(m_character->m_gUp));
          // Measure of moving sidewaysness.  Inverse abs character up dot lin vel direction.
          //
          float movingSideways = 1.0f - rage::Abs(comVelNorm.Dot(m_character->m_COMTM.b));
          // Combine scales.
          //
          float linVelScale = lyingFlat * movingSideways;
          spinReductionScale = linVelScale * rotVelScale;

#if ART_ENABLE_BSPY && RDSBSpyDraw
          bspyScratchpad(m_character->getBSpyID(), "RDS", rotVelScale);
          bspyScratchpad(m_character->getBSpyID(), "RDS", lyingFlat);
          bspyScratchpad(m_character->getBSpyID(), "RDS", movingSideways);
          bspyScratchpad(m_character->getBSpyID(), "RDS", linVelScale);
          bspyScratchpad(m_character->getBSpyID(), "RDS", comRotVelNorm);
          bspyScratchpad(m_character->getBSpyID(), "RDS", comVelNorm);
          bspyScratchpad(m_character->getBSpyID(), "RDS", spinReductionScale);
#endif
        }
        ...
        m_parameters.m_zAxisSpinReduction * spinReductionScale,
  ...
  void NmRsCBURollDownStairs::sendParameters(NmRsSpy& spy)
  {
    ...
    bspyTaskVar(m_parameters.limitSpinReduction, true);
