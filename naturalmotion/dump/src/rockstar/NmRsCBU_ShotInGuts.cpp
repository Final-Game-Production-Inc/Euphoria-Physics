...
namespace ART
{
    ...
    void NmRsCBUShot::shotInGuts_tick(float timeStep)
    {
      ...
      // ---------------------------------------------------------------------------------------
      float wt = 0.0f;
      if (m_parameters.sigPeriod > 0.00001f)//Protect against div by zero
        wt = 1.0f - m_shotInGuts.timer/m_parameters.sigPeriod;
#if ART_ENABLE_BSPY
    ...
    getSpineInputData()->applySpineLean(-m_parameters.sigSpineAmount * wt, 0.f);
      ...
      if (!m_fTK.m_bendLegs)//fallToKnees has control of forceBalance and legStraightness
      {
        ...
        if(m_shotInGuts.timer >= m_parameters.sigKneesOnset &&
           (m_parameters.sigPeriod - m_parameters.sigKneesOnset) > 0.00001f)//Protect against div by zero
        {
