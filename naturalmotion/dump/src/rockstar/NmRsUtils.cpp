...
namespace ART
{
    ...
    /**
    * quaternion -> (twist, swing1, swing2)
     */
    rage::Vector3 rsQuatToRageDriveTwistSwing(const rage::Quaternion& quat)
    {
      ...
      // Clamp twist; QuatVToLeanAndTwist is sometimes returning twist angles >PI <-PI.
      result.x = rage::Selectf((result.x - PI), (result.x - 2.0f*PI), result.x);
      result.x = rage::Selectf((result.x + PI), result.x, (result.x + 2.0f*PI));

      return result;
    }

    /**
    * (twist, Swing1, Swing2) -> quaternion
    */
    ...
    */
    rage::Quaternion rsRageLimitTwistSwingToQuat(const rage::Vector3& ts)
    {
      ...
      if (ts.Mag() > 0.0f)
      {
        ...
        ts2[0] = ts2_x;
        ts2[1] = rage::Selectf(a, ts2_y, ts2_0);
        ts2[2] = rage::Selectf(a, ts2_z, ts2_0);
...
#define NM_CONSTRAIN_TO_BALANCE 1
      ...
      bool incremental)
    {
      ...
      {        
        ...
        angle = rage::Selectf(angle, _gt, _lt);
