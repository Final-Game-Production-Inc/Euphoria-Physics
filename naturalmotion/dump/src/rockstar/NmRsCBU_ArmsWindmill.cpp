...
namespace ART
{
  ...
  void NmRsCBUArmsWindmill::updateRelativeTargetDistances()
  {
    ...
    m_relDistLRcw = r > m_leftCircle.angle ? r - m_leftCircle.angle : r + (PI2-m_leftCircle.angle);
    m_relDistLRccw = r > m_leftCircle.angle ? m_leftCircle.angle + (PI2-r) : m_leftCircle.angle - r;
  ...
  void NmRsCBUArmsWindmill::updateAdaptiveSpeeds()
  {
    ...
    else
    {
      ...
      {
        float sign = cavD < 0.0f ? 1.0f : -1.0f; 
...
// draw circles
  ...
  {      
	...
	bool leftIsLeading = m_relDistLRcw > m_relDistLRccw;
	bspyTaskVar(leftIsLeading, true);
