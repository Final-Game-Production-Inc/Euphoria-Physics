...
namespace ART
{
	bool NmRsCharacter::setupCharacter(rage::phArticulatedCollider *inputCollider)
	{
		...
		else if (!strncmp(getIdentifier(), "WiLarge", 7)) 
		{
			m_bodyIdent = gtaWilmaLarge;
			m_zUp = true;
		}
		else if (!strncmp(getIdentifier(), "Alien", 5)) 
		{
			m_bodyIdent = gtaAlien;
			m_zUp = true;
		}
...
#define NM_RS_IDMAP(name) partMapGTA[name] = gta##name; partMapRDR[name] = rdr##name;
  ...
  #undef NM_RS_IDMAP
      ...
      if (m_bodyIdent == gtaFred || m_bodyIdent == gtaFredLarge || m_bodyIdent == gtaWilma || m_bodyIdent == gtaWilmaLarge || m_bodyIdent == gtaAlien || m_bodyIdent == mp3Medium || m_bodyIdent == mp3Large)
      {
			  partMap = partMapGTA;
			  effMap = effMapGTA;
		  }
