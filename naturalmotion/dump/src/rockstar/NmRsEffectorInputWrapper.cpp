...
/*
 *  Macro to easily generate LimbComponentDataPackets
 */
#define sendLimbComponentData(_varN, _val, _type, _typeMemb) \
  { \
    LimbComponentDataPacket pp(bSpy::bSpyValueUnion::pt_##_type);\
    pp.m_data.m_value._typeMemb = _val;\
    pp.m_data.m_name = ART::bSpyServer::inst()->getTokenForString(_varN);\
    bspySendPacket(pp); \
  }
...
#define DEFINE_SET_CURRENT(_name)\
  ...
  void NmRs1DofEffectorInputWrapper::setRelaxation(const NmRsEffectorBase* effector, float mult, float *pMultDamping /*= 0*/)
  {
    ...
    float clampedStrength = rage::Clamp(eff1Dof->getInfo().m_defaultMuscleStrength * mult*mult, 2.0f, 255.0f);
  ...
  void NmRs1DofEffectorInputWrapper::reset(const NmRsEffectorBase* effector, ResetEffectorsType resetType, float scale /*= 1.0f*/)
  {
    Assert(!effector->is3DofEffector());
    const NmRs1DofEffector* eff1Dof = (const NmRs1DofEffector*)effector;
    ...
    {
      setMuscleStiffness(scale * eff1Dof->getInfo().m_defaultMuscleStiffness);
      setMuscleStrength(scale * eff1Dof->getInfo().m_defaultMuscleStrength);
      setMuscleDamping(scale * eff1Dof->getInfo().m_defaultMuscleDamping);
  ...
  void NmRs1DofEffectorInputWrapper::activePose(const NmRsEffectorBase* effector, int transformSource)
  {
    ...
    if (!effector->getJointQuaternionFromIncomingTransform(&quat, (IncomingTransformSource)transformSource))
      return;

    rage::Vector3 tss = rsQuatToRageDriveTwistSwing(quat);

    setDesiredAngle(tss.x);
  }
  ...
  void NmRs3DofEffectorInputWrapper::setRelaxation(const NmRsEffectorBase* effector, float mult, float *pMultDamping /*= 0*/)
  {
    ...
    // we square the multiplier as the natural frequency (strength*strength) is what we want to multiply by mult.
    float clampedStrength = rage::Clamp(eff3Dof->getInfo().m_defaultMuscleStrength * mult*mult, 2.0f, 255.0f);
    setMuscleStrength(clampedStrength);
  ...
  void NmRs3DofEffectorInputWrapper::reset(const NmRsEffectorBase* effector, ResetEffectorsType resetType, float scale /*=1.0f*/)
  {
    Assert(effector->is3DofEffector());
    const NmRs3DofEffector* eff3Dof = (const NmRs3DofEffector*)effector;
    ...
    if (resetType & kResetCalibrations)
    {
      setMuscleStiffness(scale * eff3Dof->getInfo().m_defaultMuscleStiffness);
      setMuscleStrength(scale * eff3Dof->getInfo().m_defaultMuscleStrength);
      setMuscleDamping(scale * eff3Dof->getInfo().m_defaultMuscleDamping);
      setOpposeGravity(0.f);
    }
    if (resetType & kResetMuscleStiffness)
    {
  ...
  // twist, lean, lean
  void NmRs3DofEffectorInputWrapper::blendToSpecifiedPose(rage::Vector3& twistLean, float t)
  {
    float clampT = rage::Clamp(t, 0.0f, 1.0f);
    float invT = 1.0f - clampT;

    // Blend with message data if this value set, otherwise set.
    if (m_data.m_flags & NmRs3DofEffectorInputData::applyDesiredTwist)
      setDesiredTwist(getDesiredTwist() * invT + twistLean.x * clampT);
    else
      setDesiredTwist(twistLean.x);

    if (m_data.m_flags & NmRs3DofEffectorInputData::applyDesiredLean1)
      setDesiredLean1(getDesiredLean1() * invT + twistLean.y * clampT);
    else
      setDesiredLean1(twistLean.y);

    if (m_data.m_flags & NmRs3DofEffectorInputData::applyDesiredLean2)
      setDesiredLean2(getDesiredLean2() * invT + twistLean.z * clampT);
    else
  ...
  void NmRs3DofEffectorInputWrapper::activePose(const NmRsEffectorBase* effector, int transformSource)
  {
    ...
    rage::Vector3 tss = rsQuatToRageDriveTwistSwing(quat);
    eff3Dof->getTwistAndSwingFromRawTwistAndSwing(tss, tss);

    setDesiredAngles(tss.y, tss.z, tss.x);
  }
  ...
  {
    sendLimbComponentData("Stiffness", m_stiffness, float, f)
    sendLimbComponentData("Damping", m_damping, float, f)
    if(m_applyMuscleStiffness)
    {
      sendLimbComponentData("Muscle Stiffness", m_muscleStiffness, float, f)
    }
  }
#endif //ART_ENABLE_BSPY_LIMBS
