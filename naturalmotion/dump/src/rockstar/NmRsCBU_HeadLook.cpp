...
namespace ART
{
  ...
  void NmRsCBUHeadLook::onActivate()
  {
    ...
    m_body->setStiffness(m_parameters.m_stiffness, m_parameters.m_damping, bvmask_CervicalSpine, NULL);
  }
  ...
  CBUTaskReturn NmRsCBUHeadLook::onTick(float /*timeStep*/)
  {
    ...
    if (! (animPoseTask->isActive() && animPoseTask->m_parameters.overideHeadlook && (animPoseTask->m_parameters.effectorMask & bvmask_CervicalSpine)))
    {
...
#if ART_ENABLE_BSPY
      float velFrameMag = velFrame.Mag();
      float angVelFrameMag = angVelFrame.Mag();
      bspyScratchpad(m_character->getBSpyID(), "HeadLook", velFrameMag);
      bspyScratchpad(m_character->getBSpyID(), "HeadLook", angVelFrameMag);
#endif
...
#if ART_ENABLE_BSPY
      bspyScratchpad(m_character->getBSpyID(), "HeadLook", dotZ);
#endif
      ...
      if (m_canLook || m_parameters.m_alwaysLook) 
      {
        ...
        // If target is behind the character, add some hysteresis to twist
        // direction switching.
        float hysteresis = 0.0f;
        if(dirTarget.Dot(spine3Back) > 0.0f)
        {
          hysteresis = 0.3f;
        }
        ...
        float sign = 1.0f;
        float dot = dirTarget.Dot(spine3Left);
        if (dot < m_twistHysteresis)
        {
          sign = -1.0f;
          m_twistHysteresis = hysteresis;
        }
        else
        {
          m_twistHysteresis = -hysteresis;
...
#if ART_ENABLE_BSPY
        bspyScratchpad(m_character->getBSpyID(), "HeadLook", dot);
        bspyScratchpad(m_character->getBSpyID(), "HeadLook", sign);
        bspyScratchpad(m_character->getBSpyID(), "HeadLook", twistCalc);
        bspyScratchpad(m_character->getBSpyID(), "HeadLook", spine3Left);
        bspyScratchpad(m_character->getBSpyID(), "HeadLook", projBack);
        bspyScratchpad(m_character->getBSpyID(), "HeadLook", dirTarget);
#endif
        ...
        if(m_parameters.twistSpine)
        {
...
#if ART_ENABLE_BSPY
          bspyScratchpad(m_character->getBSpyID(), "HeadLook", spineTwist);
#endif
          ...
          // Not convinced this works as desired. It's unclear what the intention
          // is of getDesiredTwist. If it is intended to be additive on previoulsy
          // ticking behaviours, this will no longer work (roadmap/limbs). Might
          // be a good spot to try out some kind of additive blend in the limb?
      ...
      {
        getSpine()->keepHeadAwayFromGround(getSpineInput(), 1.f, &m_character->m_groundNormal);
      }
  ...
  void NmRsCBUHeadLook::sendParameters(NmRsSpy& spy)
  {
    ...
    rage::Vector3 posTargetLocal = m_parameters.m_pos;
    rage::Vector3 worldTarget;
    m_character->instanceToWorldSpace(&worldTarget, posTargetLocal, m_parameters.m_instanceIndex);
    bspyTaskVar(worldTarget, true);
    ...
    bspyTaskVar(m_twistHysteresis, false);
