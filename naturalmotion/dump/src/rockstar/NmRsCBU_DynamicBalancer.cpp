...
#define NM_RS_WAIT_AND_ZERO_TASK(taskHandle) \
  ...
  */
  ...
  CBUTaskReturn NmRsCBUDynamicBalancer::tickPhase1(float timeStep)
  {
    ...
    SCOPED_BODY_CONTEXT(Phase1, (int)m_tickPhase)
  ...
  {
    SCOPED_BODY_CONTEXT(Phase2, (int)m_tickPhase)
  ...
  {
    SCOPED_BODY_CONTEXT(Phase3, (int)m_tickPhase)

#if ART_ENABLE_BSPY
    m_character->setCurrentSubBehaviour("tickPhase3");
  ...
  */
  ...
  void NmRsCBUDynamicBalancer::probeForUnevenTerrain()
  {
    ...
    //mmmmNote this really just warns if the knee positions are very far away from (0,0,0) 
	  ...
	  {
		  Warningf("NmRsCBUDynamicBalancer::probeForUnevenTerrain() - leftStart.Mag() = %f, rightStart.Mag() = %f", leftStart.Mag(), rightStart.Mag());
		  return;
	  }
  ...
  bool NmRsCBUDynamicBalancer::updateBalanceStatus()
  {
    ...
    //NB: the balancer does not fail in order to give stagger that extra step as it falls.  A backwards lean of GT 0.6 will generally mean the balancer will soon fail without stayUpright forces.
    if(leanedTooFarBack)
    {
      m_character->m_uprightConstraint.forceActive = false;
      m_character->m_uprightConstraint.torqueActive = false;
