...
#define highFall_DEVEL 1
  ...
  void NmRsCBUSmartFall::onActivate()
  {
    ...
    // Zero wrist angles and reset strength.  This is often not done.
    //
    m_body->resetEffectors(kResetCalibrations | kResetAngles | kResetMuscleStiffness, bvmask_HandLeft | bvmask_HandRight);
  ...
  void NmRsCBUSmartFall::duringSplat()
  {
    ...
    else
    {
      ...
      m_body->setRelaxation(mult, bvmask_Full & ~(bvmask_HandLeft | bvmask_HandRight), &multDamping);
...
#define RDS_INCREASE_STIFFNESS 0
  ...
  void NmRsCBUSmartFall::duringFall()
  {
    ...
    if (! armsWindmillAdaptiveTask->isActive() )
    {
      ...
      armsWindmillAdaptiveTask->m_parameters.leftElbowAngle  = m_character->getRandom().GetRanged(0.2f, 0.5f);
      armsWindmillAdaptiveTask->m_parameters.rightElbowAngle = m_character->getRandom().GetRanged(0.2f, 0.5f);
    ...
    armsWindmillAdaptiveTask->m_parameters.swayAmount = m_parameters.swayAmount;
