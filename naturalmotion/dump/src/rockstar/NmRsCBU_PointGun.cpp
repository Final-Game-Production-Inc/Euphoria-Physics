...
namespace ART
{
  ...
  void NmRsCBUPointGun::initialiseCustomVariables()
  {
    ...
    for(i = 0; i < 2; ++i)
    {
      ...
      armData->fireWeaponRelaxTime = 0.4f;
...
#define REDUCE_WEAPON_DISTANCE 0 
    ...
    armData.weaponDistance -= (armData.relaxTimer/armData.fireWeaponRelaxTime)*m_parameters.fireWeaponRelaxDistance;
#endif
  ...
  //better bSpy output
  void NmRsCBUPointGun::fireWeapon(NmRsHumanBody& body, int hand, float fireStrength, bool applyFireGunForceAtClavicle, float inhibitTime, rage::Vector3& direction, float split /* = 0.5 */, float supportSplit)
  {
#if ART_ENABLE_BSPY & PointGunBSpyDraw & 0
    ...
    //NmRsCharacter::AttachedObject* leftWeapon = 0, *rightWeapon = 0;//mmmmUnused
    //m_character->getAttachedWeapons(&leftWeapon, &rightWeapon);//mmmmUnused
    ...
    if (armSetup&&(gunTimer<=0))
    {
      ...
      if (applyFireGunForceAtClavicle)
      {
        ...
        if (supportSplit > 0.0f && supportSplit <= 1.0f)
          armSetup->getHand()->applyImpulse(impulse*(1.0f-supportSplit),armSetup->getHand()->getPosition());
        else if (supportSplit <= 0.0f || supportSplit > 1.0f)//supportSplit > 1.0f is a bad parameter value
          armSetup->getHand()->applyImpulse(impulse,armSetup->getHand()->getPosition());

        if((m_weaponMode == kRifle || m_weaponMode == kPistol) && supportConstraintEffective(m_armData[NmRsCharacter::kLeftHand]))
        {
          if (supportSplit > 0.0f && supportSplit <= 1.0f)
            body.getLeftArm()->getHand()->applyImpulse(impulse*supportSplit,body.getLeftArm()->getHand()->getPosition());
          else if (supportSplit < 0.0f)
            body.getLeftArm()->getHand()->applyImpulse(-impulse*supportSplit,body.getLeftArm()->getHand()->getPosition());
        }
      }
      else
      {
        ...
        if (supportSplit > 0.0f && supportSplit < 1.0f)
          armSetup->getHand()->applyImpulse(forceV*(1.0f-supportSplit),armSetup->getHand()->getPosition());
        else if (supportSplit <= 0.0f || supportSplit > 1.0f)//supportSplit > 1.0f is a bad parameter value)
        ...
        {
          if (supportSplit > 0.0f && supportSplit <= 1.0f)
            body.getLeftArm()->getHand()->applyImpulse(forceV*supportSplit,body.getLeftArm()->getHand()->getPosition());
          else if (supportSplit < 0.0f)
            body.getLeftArm()->getHand()->applyImpulse(-forceV*supportSplit,body.getLeftArm()->getHand()->getPosition());
        }
      }
    ...
    m_armData[hand].fireWeaponRelaxTime = m_parameters.fireWeaponRelaxTime;
    ...
    // This should really be done through setArmMuscles.  e.g. should respect armStiffnessDetSupport, have a timeWarpScale
    //  Although the modulation of the supporting arm is not done in setArmMuscles therefore is only active for a frame
    if(hand == NmRsCharacter::kRightHand)
    {
  ...
  void NmRsCBUPointGun::updateArmData(float timeStep)
  {
    ...
    for(i = 0; i < 2; ++i)
    {
      ...
      // fire weapon relax.
      armData->relaxScale = amount + (1.0f-armData->relaxTimer/armData->fireWeaponRelaxTime) * (1.0f - amount);
      armData->relaxScale = amount + (1.0f-armData->relaxTimer/armData->fireWeaponRelaxTime) * (1.0f - amount);
      if(timeStep < 0.0f)
      {
...
#endif
  {
    ...
    float scale = 1.0f-armData.relaxTimer/armData.fireWeaponRelaxTime;
#if ART_ENABLE_BSPY & PointGunBSpyDraw & 0
...
#undef HA_NAME_ACTION
    ...
    bspyTaskVar(armDataR->fireWeaponRelaxTime, false);
    ...
    bspyTaskVar(armDataL->fireWeaponRelaxTime, false);
