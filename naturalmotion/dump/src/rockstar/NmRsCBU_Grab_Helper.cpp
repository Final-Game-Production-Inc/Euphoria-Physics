...
namespace ART
{
    ...
    void NmRsCBUGrab::moveArm(rage::Vector3 &handTarget, rage::Vector3 &handTargetNormal, BehaviourMask effectorMask, rage::phConstraintHandle &constraint, NmRsHumanArm *limb, NmRsLimbInput& input, int direction, bool &isConstrained, float &holdTimer, float &localPullUpStrength, bool useHardConstraint /* = true */)
    {
      //Note if grab is changed to be able to use BOTH spherical or distance constraint then code will have to be added to allow on the fly changing of constraint type
      const bool distanceContraint = false;//always use a spherical constraint
      ...
      if (!constraint.IsValid())
      {
        ...
        // limbs: this was set relative. appears to be slamming the limits
        // anyway. we don't have the implementation as it's not often used
        ...
        if (m_parameters.fromEA)
          m_character->cleverHandIK(handTarget,limb->getHand(),(float)direction,!m_parameters.justBrace && useHardConstraint, softStr, &constraint, NULL, m_character->m_patches[m_grabPatchR].instLevelIndex, threshold,m_character->m_patches[m_grabPatchR].boundIndex, distanceContraint);
        else
#endif//#if NM_EA
        m_character->cleverHandIK(handTarget,limb->getHand(),(float)direction,!m_parameters.justBrace && useHardConstraint, softStr, &constraint, NULL, m_parameters.instanceIndex, threshold,m_parameters.boundIndex, distanceContraint);
      }
...
#if NM_EA
				if (m_parameters.fromEA)
					m_character->cleverHandIK(handTarget,limb,(float)direction,!m_parameters.justBrace && useHardConstraint,grabSt,&constraint,NULL,m_character->m_patches[m_grabPatchR].instLevelIndex, threshold,m_character->m_patches[m_grabPatchR].boundIndex, distanceContraint);
				else
#endif//#if NM_EA
					m_character->cleverHandIK(handTarget,limb->getHand(),(float)direction,!m_parameters.justBrace && useHardConstraint,grabSt,&constraint,NULL,m_parameters.instanceIndex,threshold,m_parameters.boundIndex, distanceContraint);

			m_character->setConstraintBreakable(constraint, m_parameters.grabStrength != -1, m_parameters.grabStrength);
      ...
      inputData->getClavicle()->setDesiredTwist(limb->getClavicle()->getActualTwist()); 
        ...
        {
          m_character->ReleaseConstraintSafely(constraint);
        }
    ...
    float NmRsCBUGrab::softWristDirectionConstraint(NmRsGenericPart* endPart,rage::Vector3 &normal,float direction)
    {
      ...
      return angle;
		...
		void NmRsCBUGrab::decideToGrab(bool &moveLeft, bool &moveRight)
		{
#if NM_EA
    ...
    void NmRsCBUGrab::decideToGrabFromParameters(bool &moveLeft, bool &moveRight)
    {
      bool doGrab = false;
