...
namespace ART
{
  ...
  CBUTaskReturn NmRsCBUYanked::onTick(float timeStep)
  {
    ...
    if (dynamicBalancerTask->m_failType != dynamicBalancerTask->balOK)//mmmmtodo add in bcr? 
    {
      if (!m_characterIsFalling)
      {
        ...
        catchFallTask->m_comVelRDSThresh = m_parameters.m_comVelRDSThresh;//mmmmtodo mmmmnote messes up setFallingReaction setup
        catchFallTask->m_parameters.m_useHeadLook = false;
        catchFallTask->m_armReduceSpeed = 1.5f;//mmmmtodo mmmmnote messes up setFallingReaction setup
        catchFallTask->activate();

        m_characterIsFalling = true;
        setFrictionMultipliers(m_parameters.m_groundFriction);//mmmmnote overwrites catchFall settings
      }
    ...
    else
    {
      ...
      m_character->setFrictionPreScale(m_parameters.m_footFriction, bvmask_FootLeft | bvmask_FootRight);
      ...
      if (!staggerFallTask->isActive())
      {
        ...
        else if (m_YankTimer <= m_parameters.m_timeAtStartValues + m_parameters.m_rampTimeFromStartValues + NM_RS_FLOATEPS) //NM_RS_FLOATEPS protects against m_parameters.m_rampTimeFromStartValues being zero and thus a DIVBy0 below
        {
  ...
  void NmRsCBUYanked::HeadLookOrAvoid(float timeStep)
  {
    ...
    if (m_characterIsFalling && (m_character->hasCollidedWithEnvironment(bvmask_CervicalSpine | bvmask_ClavicleRight | bvmask_ClavicleLeft)))
    {
      ...
      getSpine()->setBodyStiffness(getSpineInput(), m_parameters.m_groundSpineStiffness, 1.f, bvmask_CervicalSpine, &stiff);
  ...
  {
    m_character->setFrictionPreScale(frictionMult, bvmask_JustSpine | bvmask_LowerBody | bvmask_ArmLeft | bvmask_ArmRight);
  }
