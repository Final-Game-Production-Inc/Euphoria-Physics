...
namespace ART
{
  ...
  void NmRsCBUDragged::onActivate()
  {
      ...
      && (m_parameters.m_ropedBodyPart == getRightArm()->getHand()->getPartIndex()) )   
    {
      ...
      m_character->constrainPart(m_bodyPartConstraint, fixPartIndex,0, (partPos - m_ropeAttachmentPos).Mag(), partPos, m_ropeAttachmentPos, m_parameters.m_ropeAttachedToInstance, false, true,NM_MIN_STABLE_DISTANCECONSTRAINT_DISTANCE);//has to be a distance constraint           
    ...
    if (m_grabbedLeft && m_grabbedRight)
    {
      ...
      m_character->constrainPart(m_upperConstraint, fixPartIndex,0, (partPos - m_ropeAttachmentPos).Mag(), partPos, m_ropeAttachmentPos, m_parameters.m_ropeAttachedToInstance, false, true, NM_MIN_STABLE_DISTANCECONSTRAINT_DISTANCE);//has to be a distance constraint           
      ...
      m_character->constrainPart(m_lowerConstraint, m_reachArm->getHand()->getPartIndex(),m_ropeArm->getHand()->getPartIndex(), (ropeArmPosition - reachArmPos).Mag(), reachArmPos, ropeArmPosition, m_character->getFirstInstance()->GetLevelIndex(), false, true, NM_MIN_STABLE_DISTANCECONSTRAINT_DISTANCE);      
    }
    else
    {
      if (m_grabbedRight)
  ...
  void NmRsCBUDragged::onDeactivate()
  {
    ...
    m_character->ReleaseConstraintSafely(m_bodyPartConstraint);
    m_character->ReleaseConstraintSafely(m_upperConstraint);
    m_character->ReleaseConstraintSafely(m_lowerConstraint);
        ...
        ||((m_ropeArm == getRightArm()) && (!m_parameters.m_grabRight)) ) )))
      {
        ...
        if (!m_parameters.m_grabLeft)
        {
          ...
          if ((m_ropeArm == getLeftArm()) && (m_upperConstraint.IsValid()))
          {
            ...
            m_character->constrainPart(m_upperConstraint, m_reachArm->getHand()->getPartIndex(),0, (handPos - m_ropeAttachmentPos).Mag(), handPos, m_ropeAttachmentPos, m_parameters.m_ropeAttachedToInstance, false, true, NM_MIN_STABLE_DISTANCECONSTRAINT_DISTANCE);//has to be a distance constraint          
  ...
  void NmRsCBUDragged::GrabRope()    
  {
    ...
    if ((s > 0.f) && (s < 1.f) && (distanceAlongRope - tolerance <= m_grabAlongDistance) && (perpDistanceFromLine < m_parameters.m_radiusTolerance)) //on rope  
    {
      ...
      if (!firstHandConstraint) //bodyPart roped, 1st time hand was constrained therefore don't make hand2hand
      {
        //Constrain other hand to the other hand
        p1 = m_ropeArm->getHand()->getPosition();
        m_character->constrainPart(m_lowerConstraint, m_ropeArm->getHand()->getPartIndex(),m_reachArm->getHand()->getPartIndex(), (p1 - handPos).Mag(), p1, handPos, m_character->getFirstInstance()->GetLevelIndex(), false, true, NM_MIN_STABLE_DISTANCECONSTRAINT_DISTANCE);      
      }
    ...
    else if ((m_timeTillRelax < 0.5f) && (s > 1.f) && (hand2handDist >= 0.1f) && (hand2handDist <= 0.2f) && (perpDistanceFromLine < m_parameters.m_radiusTolerance)) //on rope  
    {
      if ((m_upperConstraint.IsValid()))
      {
        ...
        //Constrain other hand to the other hand
        p1 = m_ropeArm->getHand()->getPosition();
        m_character->constrainPart(m_lowerConstraint, m_reachArm->getHand()->getPartIndex(),m_ropeArm->getHand()->getPartIndex(), (p1 - handPos).Mag(), handPos, p1, m_character->getFirstInstance()->GetLevelIndex(), false, true, NM_MIN_STABLE_DISTANCECONSTRAINT_DISTANCE);//has to be a distance constraint           
        m_grabAlongDistance = -0.1f;
        m_reachTimer = -0.1f;
        m_pullUpTimer = 1.f;
        m_timeTillRelax = 1.0f;
