...
namespace ART
{
  ...
  CBUTaskReturn NmRsCBUCatchFall::onTick(float timeStep)
  {
    ...
    else
    {
      ...
      if (rage::Min(strengthLeft, strengthRight) < 0.30001f)
        m_character->sendFeedbackFinish(NMCatchFallFeedbackName);
      if (rage::Min(strengthLeft, strengthRight) < 0.9f && m_bodyStrength < m_successStrength)
      {
...
#define INCREASE_CHEST_STIFFNESS 1
      ...
      // Enforce minimum arm strength to avoid getting arms wrapped around
      // torso when rolling.
      //
      ...
      //
      float minArmStrengthLeft = 3.0f + 12.0f * m_slope * m_faceUpness * -m_slopeAlignment;
      getLeftArm()->setBodyStiffness(getLeftArmInput(), rage::Max(m_parameters.m_armsStiffness*strengthLeft, minArmStrengthLeft), 1.0f, bvmask_Full, &effectorStiffnessArms);
      float minArmStrengthRight = 3.0f + 12.0f * m_slope * m_faceUpness * m_slopeAlignment;
      getRightArm()->setBodyStiffness(getRightArmInput(), rage::Max(m_parameters.m_armsStiffness*strengthRight, minArmStrengthRight), 1.0f, bvmask_Full, &effectorStiffnessArms);
#if ART_ENABLE_BSPY
      bspyScratchpad(m_character->getBSpyID(), "CatchFall", strengthLeft);
      bspyScratchpad(m_character->getBSpyID(), "CatchFall", strengthRight);
      bspyScratchpad(m_character->getBSpyID(), "CatchFall", minArmStrengthLeft);
      bspyScratchpad(m_character->getBSpyID(), "CatchFall", minArmStrengthRight);
      float setStrengthLeft = rage::Max(m_parameters.m_armsStiffness*strengthLeft, minArmStrengthLeft);
      float setStrengthRight = rage::Max(m_parameters.m_armsStiffness*strengthRight, minArmStrengthRight);
      ...
      if (m_parameters.m_useHeadLook)
      {
        ...
        float neckStrength = rage::Max(strengthLeft, strengthRight);
      ...
      if (m_fall2Knees)
      {
        ...
        if (m_ftk_armsIn || m_ftk_armsOut)
        {
          ...
          else//fling out arms
          {
            ...
            float minStrength = rage::Min(m_rightArmState.m_strength, m_leftArmState.m_strength);
            m_leftArmState.m_strength = minStrength;
            m_rightArmState.m_strength = minStrength;
          }
  ...
  void NmRsCBUCatchFall::ArmState::tick(float timeStep)
  {
    ...
    //
    float minStrength = 3.0f;//mmmmtodo can't be more than 1.0 anyway
    //Keep armStrength high if only arms hit the ground if falling forward (strength will ramp down when spine hits ground)
    //Ramp down armStrength if falling backwards as soon as the arms hit the floor (stops the character getting stuck propped up on elbows)
    ...
    {
      m_strength = rage::Clamp(m_strength - m_parent->m_armReduceSpeed*timeStep, minStrength, m_parent->m_bodyStrength);
    }
    else
    {
      m_strength = rage::Clamp(m_strength + 2.f*timeStep, minStrength, maxStrength);
    }
