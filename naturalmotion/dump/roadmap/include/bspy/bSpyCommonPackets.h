  ...
  struct FeedbackPacket : public PacketBase
  {
    ...
    struct FeedbackUserdata
    {
      ...
      union
      {
        ...
        float               m_float;
        bool                m_bool;
#if defined(BSPY_APP) && defined(_WIN64)
        bs_int32            m_ptr;
#else
        void*               m_ptr;
#endif // 64b bspy
...
#define DECLARE_LIMB_COMPONENT_VALUE(type, name)\
  ...
  struct LimbComponentSetStiffnessPacket : public PacketBase
  {
    inline LimbComponentSetStiffnessPacket() : PacketBase()
    {
      BSPY_HEADER(LimbComponentSetStiffness)
    }

    inline void endianSwap()
    {
      bSpy::utils::endianSwap(m_stiffness);
      bSpy::utils::endianSwap(m_damping);
      bSpy::utils::endianSwap(m_muscleStiffness);
      //bSpy::utils::endianSwap(m_applyMuscleStiffness);
    }
