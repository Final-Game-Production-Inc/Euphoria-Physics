...
if len(sys.argv) != 4:
	...
	# Preprocessor flags (0 = clear, 1 = set, -1 = disregard completely)
	preprocessorFlags = [ "ALLOW_DEBUG_BEHAVIOURS", "ALLOW_TRAINING_BEHAVIOURS", "NM_NEW_SHOTMESSAGE"]
	preprocessorValues = [ 0, 0, -1 ]
	count = 0
	for flag in preprocessorFlags:
		if preprocessorValues[count] == -1:
			print "%s Ignored" % flag
		else:
		...
		if line.find("BEHAVIOUR(") != -1 and visible == 1:
			...
			while line.find('}') == -1:
				...
				if line.find("PARAMETER(") != -1:
					...
					if words[2].find("rage::Vector3") != -1:
						paramType = "vector3"
						paramDefault = "%s, %s, %s" % (words[3].strip(), words[4].strip(), words[5].strip())
						paramMin = words[8]
						paramMax = words[9]
						paramDescription = comment[1] #words[10 to len(words)]
					else:
						paramDefault = words[2].strip()
						paramType = words[3]
						paramMin = words[4]
						paramMax = words[5]
						paramDescription = comment[1] #words[6 to len(words)]
					...
					if paramMin.find("_MAX") != -1:
						paramMin = ''
						
					paramMax = re.sub('"', '', paramMax)
					paramMax = re.sub('f', '0', paramMax)
					if paramMax.find("_MAX") != -1:
						paramMax = ''
						
					# bools and strings don't have range
					if (paramType.find("bool") != -1) or (paramType.find("string") != -1):
						paramMin = ''
						paramMax = ''
				...
				if line.find("FEEDBACK(") != -1:
					if firstFeedback == 1:				
						...
						outFileHTML.write("\t\t<td style=\"background-color: rgb(220, 180, 30);\" width = 20%><b>&nbsp;Behaviours Used</b></td>\n")
					...
					description = re.sub('\A[^a-zA-Z0-9]*', '', words[3]) 	# strip out the leading non-alpha, non-numeric characters
