  ...
  { 
    ...
    */
      ...
      )
    {
      ...
      ///NM_RS_CBU_DRAWPOINT(progCOMLeft,3.f,rage::Vector3(0.0f,0.7f,0.0f));//mmmmmaddtobspy
      ///NM_RS_CBU_DRAWPOINT(progCOMRight,3.f,rage::Vector3(0.7f,0.0f,0.0f));//mmmmmaddtobspy
//      //MMMMbalancerState if progCom wrong side of obstacle put infront of obstacle by reflecting but only by offset infront
//      if (ro.m_impactOccurred)
//      {
//        float movement = (body.m_COMvel + body.m_COMrotvel).Mag();
//
//        float exclusionZone = 0.2f + rage::Min(movement*0.15f,0.4f);//0.6f;
//        if ((ro.m_balancerState == ro.bal_GlancingSpin) || (ro.m_balancerState == ro.bal_DrapeGlancingSpin) )
//          exclusionZone = 0.4f;
//        if (ro.m_balancerState == ro.bal_Rebound)
//          exclusionZone = 0.1f;
//        if (ro.m_normal1stContact.Dot(progCOMLeft-ro.m_pos1stContact-ro.m_normal1stContact*exclusionZone)*ro.m_sideOfPlane > 0.f)//increase offset
//        {
//          progCOMLeft -= ro.m_normal1stContact.Dot(progCOMLeft - ro.m_pos1stContact-ro.m_normal1stContact*exclusionZone)* ro.m_normal1stContact;
//        }
//        if (ro.m_normal1stContact.Dot(progCOMRight-ro.m_pos1stContact-ro.m_normal1stContact*exclusionZone)*ro.m_sideOfPlane > 0.f)//increase offset
//        {
//          progCOMRight -= ro.m_normal1stContact.Dot(progCOMRight - ro.m_pos1stContact-ro.m_normal1stContact*exclusionZone)* ro.m_normal1stContact;
//        }
//        NM_RS_DBG_LOGF(L"-------exclusionZone = %.4f", exclusionZone);
//
//#if NM_RS_ENABLE_DEBUGDRAW
//        rage::Vector3 exclusionPt1 = ro.m_pos1stContact;
//        ro.levelVector(exclusionPt1);
//        exclusionPt1 += ro.m_normal1stContact*exclusionZone;
//        rage::Vector3 exclusionPt2;
//        exclusionPt2.Cross(ro.m_normal1stContact,ro.m_gUp);
//        exclusionPt1 += exclusionPt2;
//        exclusionPt2 = exclusionPt1 - 2.f*exclusionPt2;
//        NM_RS_CBU_DRAWLINE(exclusionPt1,exclusionPt2,rage::Vector3(1.0f,1.0f,1.0f));
//        NM_RS_CBU_DRAWPOINT(ro.m_pos1stContact+ro.m_normal1stContact*0.6f,1.f,rage::Vector3(1.0f,1.0f,1.0f));
//        NM_RS_CBU_DRAWPOINT(progCOMLeft,3.f,rage::Vector3(0.0f,1.0f,0.0f));
//        NM_RS_CBU_DRAWPOINT(progCOMRight,3.f,rage::Vector3(1.0f,0.0f,0.0f));
//#endif
//      }     
      ...
      rage::Vector3 desiredPosLeft = progCOMLeft  -  body.m_COMTM.a * ro.m_lateralStepOffset;
        ///rage::Vector3 leftCol(1.0f, 0.0f, 0.0f);
        ///rage::Vector3 rightCol(0.0f, 1.0f, 0.0f);
        ///rage::Vector3 startCol(1.0f, 1.0f, 0.0f);
        ///NM_RS_CBU_DRAWPOINT( desiredPosLeft, 1.03f, leftCol);//mmmmmaddtobspy
        ///NM_RS_CBU_DRAWPOINT( progCOM - body.m_COMTM.a * ro.m_lateralStepOffset, 0.01f, leftCol);//mmmmmaddtobspy
        ///NM_RS_CBU_DRAWPOINT( desiredPosRight, 0.03f, rightCol);//mmmmmaddtobspy
        ///NM_RS_CBU_DRAWPOINT( progCOM + body.m_COMTM.a * ro.m_lateralStepOffset, 0.01f, rightCol);//mmmmmaddtobspy
        ///rage::Vector3 footStartW = (foot.m_footChoice == NmRsCBUDynBal_FootState::kRightStep)? foot.m_leftFootPos + foot.m_stepFootStart : foot.m_rightFootPos + foot.m_stepFootStart;
        ///NM_RS_CBU_DRAWPOINT(footStartW, 0.3f, startCol);//mmmmmaddtobspy
...
//#endif // NM_RS_ENABLE_DEBUGDRAW
      ...
      bool stepExit = false;
      {
		...
		Assertf(dif.Mag2()>1.0e-12f && rage::FPIsFinite(dif.Mag2()), "desiredPos.Mag() = %f. (*ikFootPos).Mag() = %f.", desiredPos.Mag(), (*ikFootPos).Mag());
        ro.levelVector(dif);
