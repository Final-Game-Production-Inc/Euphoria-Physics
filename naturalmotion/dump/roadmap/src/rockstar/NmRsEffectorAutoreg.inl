...
#define NM_RS_PARAMETER(classname, type, name, _min, _max, _default) \
      ...
      inline void set##name(type _v, const char * setBy = 0) {\
        NM_RS_VALIDATE_VAL(_v, _min, _max, getJointIndex(), currentBehaviour());\
        if (!isMasked()) {m_##name = _v;\
          if(setBy) {\
            ...
            char buffer[1024];\
            sprintf(buffer, "%s | %s", getBvidNameSafe(currentBehaviour()), currentSubBehaviour());\
            m_##name##SetBy = bSpyServer::inst()->getTokenForString(buffer);\
          }\
          else{\
            m_##name##SetBy = bSpyServer::inst()->getTokenForString(getBvidNameSafe(currentBehaviour()));\
          }\
...
#define NM_RS_PARAMETER(classname, type, name, _min, _max, _default) \
      ...
      inline void set##name(type _v) {\
        NM_RS_VALIDATE_VAL(_v, _min, _max, 0, 0);\
        if (!isMasked())\
          m_##name = _v;\
      }\
      protected:\
