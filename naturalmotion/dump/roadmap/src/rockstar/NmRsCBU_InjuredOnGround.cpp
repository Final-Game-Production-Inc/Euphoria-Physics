...
namespace ART
{
  ...
  void NmRsCBUInjuredOnGround::onActivate()
  {
    ...
    for (int iPart = 0; iPart < m_character->getNumberOfParts(); iPart++)
      m_character->getGenericPartByIndex(iPart)->setFrictionMultiplier(frictionMultiplier);
    ...
    getLeftArm()->getHand()->setFrictionMultiplier(armFrictionMultiplier);
    getRightArm()->getHand()->setFrictionMultiplier(armFrictionMultiplier);
    getLeftArm()->getUpperArm()->setFrictionMultiplier(armFrictionMultiplier);
    getRightArm()->getUpperArm()->setFrictionMultiplier(armFrictionMultiplier);
    getLeftArm()->getLowerArm()->setFrictionMultiplier(armFrictionMultiplier);
    getRightArm()->getLowerArm()->setFrictionMultiplier(armFrictionMultiplier);
    getLeftArm()->getClaviclePart()->setFrictionMultiplier(armFrictionMultiplier);
    getRightArm()->getClaviclePart()->setFrictionMultiplier(armFrictionMultiplier);
  ...
  void NmRsCBUInjuredOnGround::onDeactivate()
  {
    ...
    m_character->ReleaseConstraintSafetly(m_ChestConstraint);
    m_character->ReleaseConstraintSafetly(m_PelvisConstraint);
  }
...
#if 0
    // limbs todo. this is awkward to support in the new system. it should
...
#endif
      float stiff = m_character->getEffector(iJoint)->getMuscleStrength();
      if (stiff != 0.0f)
        stiff = sqrt(stiff);
      static float stiffMax = 9.0f;
      if (stiff > stiffMax)
        m_character->getEffector(iJoint)->setStiffness(stiffMax, 1.0f);
#if ART_ENABLE_BSPY 
  ...
  void NmRsCBUInjuredOnGround::GrabForWound()
  {
    ...
    if (m_parameters.m_numInjuries > 0)
    {
      ...
      if (!m_parameters.m_dontReachWithLeft && (m_reachType == kLeft || m_reachType == kBoth || m_painState != kRelaxed))
      {
        ...
        m_character->reachForBodyPart(m_body, &m_reachLeft, false, cleverIKstrength, false, false);
      ...
      if (!m_parameters.m_dontReachWithRight && (m_reachType == kRight || m_reachType == kBoth || m_painState != kRelaxed))
      {
       ...
       getRightArm()->setBodyStiffness(getRightArmInput(), m_painState == kRollRight ? armStiffnessInsideRoll : armStiffness, 1.0f);
        ...
        m_character->reachForBodyPart(m_body, &m_reachRight, false, cleverIKstrength, false, false);
      ...
      if (m_doBrace)
      {
        ...
        //	flinchTask->m_parameters.m_righthanded = bracedWithRight;
        //	flinchTask->m_parameters.m_lefthanded = bracedWithLeft;
  ...
  void NmRsCBUInjuredOnGround::Roll(float timeStep)
  {
		...
		!m_PelvisConstraint.IsValid() && getSpine()->getPelvisPart()->collidedWithEnvironment() && m_upFacing)
    {
      ...
      m_character->fixPart(getSpine()->getSpine3Part()->getPartIndex(),partPos,partPos,constrLengthChest, m_ChestConstraint);
	  partPos = getSpine()->getPelvisPart()->getPosition();
	  m_character->fixPart(getSpine()->getPelvisPart()->getPartIndex(),partPos,partPos,constrLengthPelivs, m_PelvisConstraint);
    }
