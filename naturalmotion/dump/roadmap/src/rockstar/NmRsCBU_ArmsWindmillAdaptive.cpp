...
namespace ART
{
  ...
  CBUTaskReturn NmRsCBUArmsWindmillAdaptive::onTick(float timeStep)
  {
    ...
    if (m_doLeftArm || m_doRightArm)
    {
      ...
      {
        m_rotationAxis = m_character->m_COMTM.a;//bodyRight
      m_comAngVel = m_character->m_COMrotvel;
      ...
      sign = (float)__fsel(cavD, -1.0f, 1.0f);
#else
      if (cavD < 0.f)
        sign = 1.f;             // Falling forwards.
      else
        sign = -1.f;            // Falling backwards.
      ...
      {
        getSpineInputData()->setBackAngles(0.0f, 0.0f, 0.4f*rage::Sinf(m_armsWindmillAdaptiveAngle));
        getSpineInputData()->getLowerNeck()->setDesiredTwist(-0.3f*rage::Sinf(m_armsWindmillAdaptiveAngle));
      }
    }
    else
    {
  ...
  void NmRsCBUArmsWindmillAdaptive::moveArm(float timeStep, NmRsHumanArm *arm, NmRsArmInputWrapper* inputData, float angle, float elbowBlendAngle, bool bendElbow, bool forwards)
  {
    ...
    float swing = nmrsGetActualAngle(arm->getElbow());
    float elbowSwingMin = arm->getElbow()->getMinAngle();
    float elbowSwingMax = arm->getElbow()->getMaxAngle();
    if (swing < m_ElbowBend)
    { 
      swing = rage::Clamp(swing + timeStep * m_Elbow_w, elbowSwingMin, elbowSwingMax);//assumes elbowSwingMax>elbowSwingMin
      inputData->getElbow()->setDesiredAngle(swing);
    }
    if (elbowBlendAngle > -0.01f)
      inputData->getElbow()->setDesiredAngle(rage::Clamp(swing + 2.0f*m_parameters.elbowRate*timeStep * (elbowBlendAngle - swing), elbowSwingMin, elbowSwingMax));
  ...
  void NmRsCBUArmsWindmillAdaptive::sendParameters(NmRsSpy& spy)
  {
    ...
    bspyTaskVar(m_comAngVel, false);
    bspyTaskVar(m_rotationAxis, false);
