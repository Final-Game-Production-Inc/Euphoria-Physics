...
namespace ART
{
  ...
  CBUTaskReturn NmRsCBUShootDodge::onTick(float timeStep)
  {
    ...
    if(m_hardKeyed && m_character->isPartInMask(m_parameters.hardKeyMask, 0))
    {
      ...
      {
        NMutils::NMMatrix4& fromTM = prevItPtr[rootIndex];
        NMutils::NMMatrix4& toTM = itPtr[rootIndex];
  ...
  void NmRsCBUShootDodge::BracingState::armIK(const ArmSetup* arm)
  {
    ...
    float braceDistance = maxBraceDistance;

    // set brace distance based on collision distance and our max reach.
    braceDistance = rage::Clamp(collisionDirection.Mag(), 0.2f, maxBraceDistance);
    collisionDirection.Normalize();
  ...
  void NmRsCBUShootDodge::writeITM(NMutils::NMMatrix4 *itPtr, int index)
  {
    NMutils::NMMatrix4 &itm = itPtr[index];
  rage::Displayf("{NMutils::NMMatrix4 &itm = itPtr[%d];", index);
    rage::Displayf("itm[0][0]=%ff; itm[0][1]=%ff; itm[0][2]=%ff;", itm[0][0], itm[0][1], itm[0][2]);
    rage::Displayf("itm[1][0]=%ff; itm[1][1]=%ff; itm[1][2]=%ff;", itm[1][0], itm[1][1], itm[1][2]);
    rage::Displayf("itm[2][0]=%ff; itm[2][1]=%ff; itm[2][2]=%ff;", itm[2][0], itm[2][1], itm[2][2]);
  rage::Displayf("itm[3][0]=%ff; itm[3][1]=%ff; itm[3][2]=%ff;}", itm[3][0], itm[3][1], itm[3][2]);
  }
#endif
