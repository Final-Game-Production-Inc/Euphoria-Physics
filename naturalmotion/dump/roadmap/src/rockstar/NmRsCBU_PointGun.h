...
#define PGNP_STATES(_action) \
  ...
  _action(npsByFace)             /*2 */  \
  _action(npsByHip)              /*3 */  \
  _action(npsAcrossFront)        /*4 */  \
  _action(npsPointing2Connect)   /*5 */  \
  _action(npsByFaceSupport)      /*6 */  \
  _action(npsByHipSupport)       /*7 */  \
  _action(npsAcrossFrontSupport) /*8 */  \
  _action(npsBySide)             /*9 */  \
  _action(npsPointingDontConnect)/*10*/
...
#undef PGNP_ENUM_ACTION
    ...
    struct Parameters
    {
      ...
      bool          forceNeutralToConstrain;//mmmmtodo mmmmUnused
  ...
  protected:
    ...
    void calculateGunHand(ArmData& armData);
    ArmStatus moveGunArm(ArmData& armData, const ArmStatus gunArmStatus, bool calculate);
    ArmStatus pointGunHand(ArmData& armData, bool calculate);
