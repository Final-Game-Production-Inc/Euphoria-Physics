...
namespace ART
{
    ...
    void NmRsCBUShot::newHit(NmRsHumanBody& body)
    {
      ...
      if (m_velForwards >= 0.f) // need to find exit wound to reach for...
      {
        ...
        rage::phPolygon::Index culledPolys[DEFAULT_MAX_CULLED_POLYS];
        rage::phShapeTest<rage::phShapeProbe> probeTester(culledPolys,DEFAULT_MAX_CULLED_POLYS);
      ...
      if(m_injuredLLeg || m_injuredRLeg)
      {
        if (m_injuredLLeg)
        {
          useLeft = true;
      ...
      */
      if (offsetTargets)
      {
        ...
        {
          localOffset.Set(offsetValue, 0.f, 0.f);
        ...
        {
          localOffset.Set(0.0f, offsetValue, 0.f);
          if(m_hitPointLocal.x > 0.f)                       // invert offset if reaching to right side of body center
            localOffset *= -1.f;
        }
        else
        {
          localOffset.Set(0.f, 0.f, 0.f);
        }
        m_woundROffset += localOffset;
        m_woundLOffset -= localOffset;
      }//if offsetTargets
