...
namespace ART
{
  ...
  */
  ...
  public:
    ...
    // ART::TransformSource
    bool getComponentTMs(NmRsCharacter* rsCharacter, NMutils::NMMatrix4* dest, size_t destLength) const;
    int getComponentTMs(AgentID characterID, NMutils::NMMatrix4* dest) const;
    bool setInitialTM(ART::AgentID characterID, int componentIdx, const NMutils::NMMatrix4 tm, const NMutils::NMMatrix4 *previousFrameTm);
    bool setComponentTMs(NmRsCharacter* rsCharacter, const NMutils::NMMatrix4* src, size_t srcLength, IncomingTransformStatus itmStatusFlags, IncomingTransformSource source);
    ...
    void setLeadInAnimationTMs(NMutils::NMMatrix4* tms, ART::AssetID assetID);//TEST ONLY
