...
namespace ART
{
  //struct MemoryManager;
    ...
    //   const char* m_bvIDNames in naturalmotion\src\rockstar\NmRsCBU_Shared.cpp
...
#define USE_NEW_NM_RIG 1
    ...
    const BehaviourMask bvmask_UpperBody  = (bvmask_JustSpine | bvmask_ArmLeft | bvmask_ArmRight | bvmask_CervicalSpine);
    const BehaviourMask bvmask_LowerBody  = (bvmask_Pelvis | bvmask_Legs);
    const BehaviourMask bvmask_BodyExceptHandsAndFeet  = (bvmask_UpperBody | bvmask_ThighLeft | bvmask_ShinLeft| bvmask_ThighRight | bvmask_ShinRight);
...
#if 0 //defined(NM_RS_VALIDATE_VITAL_VALUES) || ART_ENABLE_BSPY
# define NM_RS_VALIDATE_VAL(_v, _min, _max, _jointIndex, _behaviour) { Assert(_v == _v); Assertf(_v <= _max, "NaturalMotion[j=%d, setBy %s]: " #_v " = %.3f (max = %.3f)", _jointIndex, m_bvIDNames[_behaviour + 1], _v, _max); Assertf(_v >= _min, "NaturalMotion[j=%d,b=%s]: " #_v " = %.3f (min = %.3f)", _jointIndex, m_bvIDNames[_behaviour + 1], _v, _min); }
# define NM_RS_VALIDATE_VAL_WARN(_v, _min, _max, _jointIndex, _behaviour) { Assert(_v == _v); if (_v >= _max || _v <= _min) Warningf("NaturalMotion[j=%d, setBy %s]: " #_v " = %.3f (max = %.3f, min = %.3f)", _jointIndex, m_bvIDNames[_behaviour + 1], _v, _max, _min);}
#else
