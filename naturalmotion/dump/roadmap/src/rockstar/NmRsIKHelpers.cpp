...
namespace ART
{
    ...
    rage::Matrix34& matrix2)
  {
    ...
    jt->GetOrientationChild(oriChild); //orientation of the joint's rotation axis in the child's coordinates
    ...
    const NmRsEffectorInputWrapper* input /*= 0*/)
  {
    ...
    if(input)
    {
      ...
      if(effector->is3DofEffector())
      {
        ...
        {
          tts.Set(input3Dof->getDesiredTwist(), input3Dof->getDesiredLean1(), input3Dof->getDesiredLean2());
          const NmRs3DofEffector* effector3Dof = static_cast<const NmRs3DofEffector*>(effector);
          effector3Dof->getRawTwistAndSwingFromTwistAndSwing(tts, tts);
          driveQuat = rsRageDriveTwistSwingToQuat(tts);
        }
      ...
      else
      {
        ...
        {
          tts.Set(input1Dof->getDesiredAngle(), 0, 0);
          driveQuat = rsRageDriveTwistSwingToQuat(tts);
        }
        // otherwise use desired angles from effector.
        else
