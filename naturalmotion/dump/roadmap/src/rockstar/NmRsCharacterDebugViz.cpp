...
namespace ART
{
  ...
  */
  ...
  void NmRsCharacter::drawSkeleton(bool desiredSkeleton)
  {
    ...
    float totalMass = getPartMass(body->GetLink(0));
    ...
    for (int i = 0; i<getNumberOfEffectors(); i++)
    {
      ...
      if (effector->is3DofEffector())
      {
        ...
        threeDof->GetOrientationChild(orientationChild);
      ...
      else
      {
        ...
        oneDof->GetOrientationChild(orientationChild);
      ...
      float mass = getPartMass(body->GetLink(i+1));
      totalMass += mass;
  ...
  void NmRsCharacter::bspyDrawCircle(const rage::Vector3& pos, const rage::Vector3& axis, float radius, const rage::Vector3& colour, int segments /* = 8 */)
  {
    if (bspyDebugDrawIsInhibited())
      return;
  ...
  void NmRsCharacter::bspyDrawArc(const rage::Vector3& pos, const rage::Vector3& axis, const rage::Vector3& center, const float length, const rage::Vector3& colour, int segments /* = 8 */)
  {
    if (bspyDebugDrawIsInhibited())
      return;
  ...
  void NmRsCharacter::bspyDrawSphere(const rage::Vector3& pos, float radius, const rage::Vector3& colour, int segments /* = 8 */)
  {
    if (bspyDebugDrawIsInhibited())
      return;
