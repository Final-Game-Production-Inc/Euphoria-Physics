...
namespace ART
{
  ...
  const unsigned int MAX_BSPY_AGENTS_OF_EACH_TYPE = MAX_BSPY_AGENTS/NUM_ASSETS;//Make sure this is > 1 (>2 if NM_ANIM_MATRICES=1)
    ...
    m_activeNonNMRagdolls(0)
  {
    ...
    bspyMemoryUsageRecord("NmRsEngine", 
      ...
      (sizeof(NMutils::MemoryStream*) * m_maxAssets));
  ...
  bool NmRsEngine::insertAgent(ART::AgentID characterID)
  {
    ...
    NMutils::NMMatrix4* itPtr = 0, *itPrvPtr = 0;
  ...
  {     
    ...
    if (spy.shouldTransmit(bSpy::TransmissionControlPacket::bSpyTF_StaticMeshData))
    {
      ...
      if (boundType == rage::phBound::BVH && PHLEVEL->IsFixed(instLevelIndex))
      {
        ...
        for (; i<t; i++)
        {
          const rage::phPolygon& poly = bvh->GetPolygon(i);
          ...
          bvh->GetPolygonVertices(poly, vec0, vec1, vec2);
  ...
  {
    rage::phBound::BoundType boundType = (rage::phBound::BoundType)bound->GetType();
    DynamicCollisionShapePacket dcs( (bs_uint32)inst->GetLevelIndex(), (bs_uint32)bound, (bs_int16)-1, (bs_int32)inst->GetClassType() );
    ...
    case rage::phBound::GEOMETRY:
      {
        ...
        unsigned int sessionUID = getNmRsSpy()->getSessionUID();
...
//mmmmBSPY3INTEGRATION Called by the game dll's - remove and just use characterId - NOTE KEEP IN TO BSPY NON-NM CHARACTERS?
    ...
    if(haveActiveAgents)
    {
      for (NmRsCharacterVector32::iterator it = m_characterVector.begin(), end = m_characterVector.end(); it != end; ++it)
      {
        ...
        NmRsCharacter *nmrs = (*it);
        {
          ...
          nmrs->ApplyAccumulatedForcesAndImpulses();
  ...
  */
  ...
  bool NmRsEngine::getComponentTMs(NmRsCharacter* rsCharacter, NMutils::NMMatrix4* destTMs, size_t destLength) const
  {
    if (destLength == 0)
      return true; 
  ...
  int NmRsEngine::getComponentTMs(AgentID characterID, NMutils::NMMatrix4* dest) const
  {
    if (m_characterByIndex[characterID] == 0)
    {
  ...
  */
  bool NmRsEngine::setComponentTMs(NmRsCharacter* rsCharacter, const NMutils::NMMatrix4* src, size_t srcLength, IncomingTransformStatus itmStatusFlags, IncomingTransformSource source)
  {
    if (srcLength == 0)
      return true;
    ...
    rsCharacter->setIncomingTransforms((NMutils::NMMatrix4*)src, itmStatusFlags, (int)srcLength, source);
    return true;
  }
  ...
  */
  void NmRsEngine::setLeadInAnimationTMs(NMutils::NMMatrix4* tms, ART::AssetID assetID)
  { 
    Assert(assetID < NUM_ASSETS);
    ...
    {
      //convert to rage::matrix34
      NMutils::NMMatrix4 &mat1 =tms[childIndex];
      m_LeadInAnimationMatrices[assetID][childIndex].Set(
  ...
  */
  ...
  bool NmRsEngine::setInitialTM(ART::AgentID characterID, int componentIdx, const NMutils::NMMatrix4 tm, const NMutils::NMMatrix4 *previousFrameTm)
  {
    if (m_characterByIndex[characterID] == 0)
    {
