...
namespace ART
{
  ...
  void CBUTaskBase::init(NmRsCharacter* character, CBURecord* cbuParent)
  {
    ...
    // limbs todo this may not be necessary?
    m_body->setup(m_bvid, m_priority, 0, m_blend, m_mask DEBUG_LIMBS_PARAMETER(0));
  ...
  void CBUTaskBase::activate()
  {
    ...
    if (m_cbuParent->activateTask(getBvID()))
    {
      ...
      m_body->setup(getBvID(), m_priority, -10, m_blend, m_mask DEBUG_LIMBS_PARAMETER("activate"));
  ...
  CBUTaskReturn CBUTaskBase::tick(float timeStep)
  {
    if(m_active)
    {
      ...
      m_body->setup(getBvID(), m_priority, 0, m_blend, m_mask DEBUG_LIMBS_PARAMETER("tick"));

      CBUTaskReturn result = m_cbuParent->m_tasks[getBvID()]->onTick(timeStep);

      if(result == eCBUTaskComplete)
        m_body->postLimbInputs();
  ...
  void CBUTaskBase::deactivate()
  {
    if (m_cbuParent->deactivateTask(getBvID()))
    {
      ...
      if(m_active)
      {
        m_body->setup(getBvID(), m_priority, 10, m_blend, m_mask DEBUG_LIMBS_PARAMETER("deactivate"));
