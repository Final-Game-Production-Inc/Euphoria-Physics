...
#define highFall_DEVEL 1
  ...
  void NmRsCBUHighFall::initialiseCustomVariables()
  {
    ...
    m_orientationState = HFO_Back;
    m_orientationStateTimeHalved= HFO_Back;
    m_orientationStateCurrent = HFO_Back;
    m_horComVel.Zero();
  ...
  bool NmRsCBUHighFall::States( StateMachineEvent event, int state )
  {
    ...
    OnUpdate
    {
      ...
      {
        if (m_impactVisible && (m_orientationState == HFO_Front || m_orientationState == HFO_Down))
        {
          // Make sure character is going to land on its front.
          if (m_orientationState == HFO_Front)
          {
            ...
            m_orientationStateTimeHalved = getFacingDirectionFromCOMOrientation(predCOMTM);

            // Current character orientation.
            m_orientationStateCurrent = getFacingDirectionFromCOMOrientation(m_character->m_COMTM);
            ...
            if (m_orientationStateCurrent == HFO_Front && m_orientationStateTimeHalved == HFO_Front)
            {
          ...
          else // Predicted orientation state is HFO_Down and character is facing the impact, allow catch fall in this case.
          {
            SetState( HF_CatchFall );
          }
        }
        else//if (m_impactVisible && (m_orientationState == HFO_Front || m_orientationState == HFO_Down))
        {
          SetState( HF_BailOut );
        }
      }
    ...
    OnExit
    {
      getLeftLeg()->getFoot()->setFrictionMultiplier(1.0f);
      getRightLeg()->getFoot()->setFrictionMultiplier(1.0f);
    ...
    OnUpdate
    {
      ...
      bool decideToCatchFall = flatPlaneBelow && (m_hasCollidedWithWorld && m_goToCatchFallFlag && m_character->m_COMvelRelativeMag < 1.0f && (m_orientationStateCurrent != HFO_Up || m_orientationStateCurrent != HFO_Down));
      bool decideToBrace = (!decideToCatchFall) && (m_horComVel.Mag() > m_parameters.m_minSpeedForBrace) && (m_orientationState != HFO_Back) && m_impactVisible && !m_landingNormalOK;
    ...
    OnUpdate
    {
      // Brace for impact; it is likely that character is going to hit a building or a fuselage of an airplane.
          if ((m_horComVel.Mag() > m_parameters.m_minSpeedForBrace) && (m_orientationState != HFO_Back) && m_impactVisible && !m_landingNormalOK && !((m_timer - 0.25f) > m_predImpactTime))
      {
        duringCatchFall();
      }
      else
  ...
  NmRsCBUHighFall::OrientationStates NmRsCBUHighFall::getFacingDirectionFromCOMOrientation(const rage::Matrix34 &COMOrient) const
  {
    const float upDotUp = (COMOrient.b).Dot(m_character->m_gUp);
    const float leftDotUp = (-COMOrient.a).Dot(m_character->m_gUp);
    const float frontDotUp = (-COMOrient.c).Dot(m_character->m_gUp);

    NmRsCBUHighFall::OrientationStates facingDirState = (frontDotUp < 0.25f) ? HFO_Front : HFO_Back;

    // Agent sideways?
    if ((rage::Abs(leftDotUp) - rage::Abs(frontDotUp)) > 0.15f)
    {
      facingDirState = (leftDotUp < 0.0f) ? HFO_Left : HFO_Right;
    }
    ...
    {
      facingDirState = (upDotUp > 0.0f) ? HFO_Up : HFO_Down;
    }
  ...
  void NmRsCBUHighFall::duringPrepareForLanding()
  {
    ...
    if (m_parameters.m_armsUp <= -3.0f && m_orientationState == HFO_Up)
    {
  ...
  void NmRsCBUHighFall::duringForwardRoll()
  {
    ...
    // limbs todo blendToZeroPose
    //if (m_parameters.m_useZeroPose_withForwardRoll)
    //  m_character->blendToZeroPose(1,"fb");
...
*/
  void NmRsCBUHighFall::orientationControllerTick()
  {
    ...
    void NmRsCBUHighFall::probeEnvironmentAndCalcForFall() 
    {
      ...
      m_orientationState = getFacingDirectionFromCOMOrientation(predCOMTM);

      m_willLandOnFeet = (m_orientationState == HFO_Up) ? true : false;
...
#define HF_ORIENTATION_STATE_NAME_ACTION(_name) #_name ,
      HF_ORIENTATION_STATES(HF_ORIENTATION_STATE_NAME_ACTION)
