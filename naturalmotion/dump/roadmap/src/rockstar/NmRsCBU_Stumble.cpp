...
namespace ART
{
  ...
  void NmRsCBUStumble::onActivate()
  {
    ...
    if (m_parameters.m_useHeadLook)
      getSpine()->setBodyStiffness(getSpineInput(), m_parameters.m_torsoStiffness, 0.5f, bvmask_HighSpine);
  ...
  { 
    ...
    if (bodyBalanceTask->isActive())
    {
      ...
      if ((hip2head.Dot(m_character->m_gUp) < 0.7f)|| 
        (maxHipHeight < 0.6f))
        armsBrace();
    }
    else//(bodyBalanceTask->isActive())
    {
      ...
      if (m_parameters.m_useHeadLook)
        getSpine()->setBodyStiffness(getSpineInput(), m_parameters.m_torsoStiffness*strength, 0.5f, bvmask_HighSpine);
      ...
      if ((!doingTwist) && (!catchFallTask->m_handsAndKnees))
      {
        if (m_character->getBodyIdentifier() == rdrCowboy || m_character->getBodyIdentifier() == rdrCowgirl)
        {
          ...
          // limbs todo check that this is works ok.  get is coming from the a difference place than in original code.
      ...
      if (catchFallTask->m_handsAndKnees)
      {
        ...
        rage::Vector3 floorPos = m_character->m_probeHitPos[armProbeIndex];
          ...
          || getSpine()->getPelvisPart()->collidedWithEnvironment())
        {
          ...
          catchFallTask->m_callRDS = false;
          catchFallTask->m_comVelRDSThresh = 2.0f;
          catchFallTask->m_armReduceSpeed = 2.5f;
        }
  ...
  bool NmRsCBUStumble::ArmState::tick(float timeStep)
  {
    ...
    // limbs todo looks like intention is to compare with ik elbow output. will
    // not work exactly as before. this will overwrite, but the source angles
    // will be the result of the entire previous tick rather than the immediate
    // output of ik.
    //
  ...
  void NmRsCBUStumble::sendParameters(NmRsSpy& spy)
  {
    ...
    bspyTaskVar(m_parameters.injuryRate, true);
