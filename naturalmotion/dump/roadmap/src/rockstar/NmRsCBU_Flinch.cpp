...
namespace ART
{
  ...
  void NmRsCBUFlinch::onDeactivate()
  {
    ...
    getSpine()->getHeadPart()->setFrictionMultiplier(1.0f);
  ...
  */
  ...
  void NmRsCBUFlinch::braceHead()
  {
    ...
    if (m_parameters.m_useLeft || m_parameters.m_useRight)
    {
      ...
      getSpine()->getHeadPart()->setFrictionMultiplier(headFrictionMltp);
  ...
  void NmRsCBUFlinch::braceFront()
  {
    ...
    // scale the radius depending on the hieght
  ...
  {
    if (m_armCollisionDisabled==enable)
    {
      rage::phArticulatedCollider *collider = m_character->getArticulatedWrapper()->getArticulatedCollider();
      collider->SetPartsCanCollide(getRightArm()->getHand()->getPartIndex(), getLeftArm()->getHand()->getPartIndex(), enable, false);
      collider->SetPartsCanCollide(getRightArm()->getHand()->getPartIndex(), getLeftArm()->getLowerArm()->getPartIndex(), enable, false);
      collider->SetPartsCanCollide(getRightArm()->getLowerArm()->getPartIndex(), getLeftArm()->getHand()->getPartIndex(), enable, false);
  ...
  void NmRsCBUFlinch::controlStiffness_entry()
  {
    ...
    getSpine()->setBodyStiffness(getSpineInput(), m_parameters.m_bodyStiffness*0.5f, 0.3f, bvmask_HighSpine, &stiffness);
  ...
  void NmRsCBUFlinch::controlStiffness_tick(float timeStep)
  {
    ...
    m_body->setOpposeGravity(1.0f, bvmask_HighSpine | (bvmask_ArmLeft & ~ bvmask_HandLeft) | (bvmask_ArmRight & ~ bvmask_HandRight));
  ...
  void NmRsCBUFlinch::sendParameters(NmRsSpy& spy)
  {
    ...
    bspyTaskVar(m_parameters.m_lefthanded, true);
    bspyTaskVar(m_parameters.m_righthanded, true);
