...
#undef ENUM_ACTION
    ...
    */
    ...
    bool disableLegCollisions(NmRsGenericPart* gpA, NmRsGenericPart* gpB, float impactDepth);
    ...
    bool getMatrixForPart(int index, NMutils::NMMatrix4 mtm) const;
    bool getMatrixForPartByComponentIdx(int index, NMutils::NMMatrix4 mtm) const;
    ...
    // limbs todo port these to use new system properly.
private:
    void activePoseAllEffectors(); // not used at all
    void holdPoseAllEffectors();
    void storeZeroPoseAllEffectors(); // just used in character
    void blendToZeroPoseAllEffectors(float t);
    void relaxAllEffectors(float mult);
    void relaxAllEffectors_DampingOnly(float mult);
    void recalibrateAllEffectors();
    void resetAllEffectorsMuscleStiffness();
...
public:
  ...
  public:
    ...
    void setIncomingTransforms(NMutils::NMMatrix4 *ptr, IncomingTransformStatus statFlag, int tmcount, IncomingTransformSource source);
    void getIncomingTransforms(NMutils::NMMatrix4 **ptr, IncomingTransformStatus &statFlag, int &tmcount, IncomingTransformSource source) const;
    ...
    bool setInitialTM(int componentIdx, const NMutils::NMMatrix4 newTm, const NMutils::NMMatrix4 *previousFrameTm = 0);
     ...
     */
    float m_constraintDistance;
      rage::phConstraintHandle m_handToHandConstraintHandle;
    rage::Vector3 m_handToHandConstraintPos;
    ...
    void constrainSupportHand(
      ...
      float minimumConstraintDistance, 
    ...
    void ReleaseConstraintSafetly(rage::phConstraintHandle &constraint);

	void ApplyAccumulatedForcesAndImpulses();
    ...
    // NB.  Will eventually be replaced by m_strength
    ...
    inline void setGravityFactor(float gravityFactor) {m_articulatedWrapper->getArticulatedBody()->SetGravityFactor(gravityFactor);}
...
#define MAX_NUM_OF_DEFERRED_FORCES 40
	enum forceType	
	{
		kForce = 0,     
		kTorque, 
		...
		kJointTorque1Dof,
		kJointTorque3Dof,
		kJointAngImpulse1Dof,
		kJointAngImpulse3Dof
	};
	struct DeferredForce
	{
		forceType m_type;
		rage::u8 m_partIndex;
		rage::Vector3 m_force, m_offset;
	}; 
	static DeferredForce sm_DeferredForces[MAX_NUM_OF_DEFERRED_FORCES];
	static rage::u8 sm_NumDeferredForces;
	static bool sm_ApplyForcesImmediately;
	void ResetDeferredForces();
	void AddDeferredForce(const forceType type, const rage::u8 partIndex, const rage::Vector3 &force, const rage::Vector3 &offset);
...
#define NUM_OF_BULLETS 17
  ...
  public:
    ...
    void blendWithDesiredPose(NmRs3DofEffector* effector, float lean1, float lean2, float twist, float blendFactor);
    void blendWithDesiredPose(NmRs1DofEffector* effector, float angle, float blendFactor);
...
#if 0
    void matchClavicleToShoulderBetter(NmRs3DofEffector *clavicle, NmRs3DofEffector *shoulder);
    void matchClavicleToShoulderUsingTwist(NmRs3DofEffector *clavicle, NmRs3DofEffector *shoulder);
#endif
...
#undef ENUM_ACTION
    ...
    void antiSpinAroundVerticalAxisJuice(float zAxisSpinReduction);

    void fireGun(int hand, float fireStrength, bool applyFireGunForceAtClavicle, float extraLean1, float extraLean2, float inhibitTime, rage::Vector3& direction, float split = 0.5);
    ...
    void setFrictionMultiplier(float mult, BehaviourMask mask = bvmask_Full);
...
#if NM_SCRIPTING
    rage::Matrix34 getRageMatrix(NMutils::NMMatrix4 matrix);
    ...
    void AlterArmMasses(int iType, bool increase); // 1 for left, 2 for right
    void fixPart(int index, rage::Vector3 &pos, rage::Vector3 &pos2, float partSeparation, rage::phConstraintHandle &con, bool increaseArmMass = true);
    void fixPartsTogether2(int index1, int index2, float partSeparation, rage::Vector3 &pos, rage::Vector3 &pos2, int inputlevelIndex, rage::phConstraintHandle &con, bool increaseArmMass);
    void updateConstraintSeparation(rage::phConstraintHandle &con, float seperation);

    bool hasCollidedWithWorld(BehaviourMask mask = bvmask_Full); // WithNotOwnCharacter
    bool hasCollidedWithOtherCharacters(BehaviourMask mask = bvmask_Full);
    bool hasCollidedWithEnvironment(BehaviourMask mask = bvmask_Full);
    ...
    void getJointMatrix1FromParent(rage::Matrix34& jointMat1,//mmmmunused
      NmRsEffectorBase* effector,
      NmRsGenericPart* parent);
    void getJointMatrix2FromChild(rage::Matrix34& jointMat2,//mmmmunused
      NmRsEffectorBase* effector,
    ...
    // ------------------------------------------------------------------------------------------------
    void sendFeedbackSuccess(char *name);
    void sendFeedbackFailure(char *name);
    void sendFeedbackFinish(char *name);
    void sendFeedbackEvent(char *name);
    ...
    inline void enableFootSlipCompensationActive(bool enable, bool forceInitializeFootOrientation = false) 
    {
      ...
      if (!forceInitializeFootOrientation) 
      {
    ...
    void bspyDrawCircle(const rage::Vector3& pos, const rage::Vector3& axis, float radius, const rage::Vector3& colour, int segments = 8);
    void bspyDrawArc(const rage::Vector3& pos, const rage::Vector3& axis, const rage::Vector3& start, const float length, const rage::Vector3& colour, int segments /* = 8 */);
    void bspyDrawSphere(const rage::Vector3& pos, float radius, const rage::Vector3& colour, int segments = 8);
#endif
...
public:
    ...
    int feedbackBehaviourEvent(char* name, int id);
...
#undef ENUM_ACTION
  ...
  protected:
    ART::ARTFeedbackInterface *simpleFeedback(char *name);
...
#endif // ART_ENABLE_BSPY
    bool prepareFeedback(char* name, int id);
  ...
  private:
    ...
    NMutils::NMMatrix4         *m_incomingTm[ART::KITSourceCount];
