...
namespace ART
{
  ...
  void NmRsCBUPointGun::onDeactivate()
  {
    ...
    m_character->setFrictionMultiplier(1.0f, bvmask_ArmRight | bvmask_ArmLeft);
  ...
  if (m_character->m_gunToHandConstraintHandle[NmRsCharacter::kRightHand].IsValid())
  {
    ...
    if (dynamicBalancerTask->m_failType == dynamicBalancerTask->balOK)
    {
      if(m_parameters.enableRight)
        m_character->setFrictionMultiplier(0.1f, bvmask_ArmRight);
      if(m_parameters.enableLeft && m_enableLeft && (m_weaponMode != kPistolRight))
        m_character->setFrictionMultiplier(0.1f, bvmask_ArmLeft);
    }
    ...
    case kPistolLeft:
      ...
      //Gives neutral pose if allowed by m_parameters.poseUnusedOtherArm
      localArmStatus = setNeutralPoseType(m_armData[NmRsCharacter::kRightHand], npsBySide); 
      setArmStatus(m_armData[NmRsCharacter::kLeftHand], localArmStatus);
      break;
    case kDual:
      pointSingleHandedGun(m_armData[NmRsCharacter::kRightHand]);
    ...
    // limbs todo implement set limits to pose.
#if NM_RUNTIME_LIMITS & 0
    ...
    if (m_armData[NmRsCharacter::kLeftHand].armStatusChanged && 
      ...
      getLeftArm()->getWrist()->setLimitsToPose(true);
    if (m_armData[NmRsCharacter::kRightHand].armStatusChanged && 
      ...
      getRightArm()->getWrist()->setLimitsToPose(true);
#endif

    // limbs todo merge rest of code.
#if 0
  ...
  NmRsCBUPointGun::ArmStatus  NmRsCBUPointGun::moveGunArm(ArmData& armData, const NmRsCBUPointGun::ArmStatus gunArmStatus, bool calculate)
  {
    ...
    case kNeutralPointing:
      pointGunHand(armData, calculate);//NB: armData must contain the pointing target (as the target).  returns kPointingWithError or kPointing which we ignore as we know we are kNeutralPointing
  ...
  bool NmRsCBUPointGun::isGunHandTargetReachable(ArmData& armData)
  {
    ...
    float targetAngle = rage::AcosfSafe(toTarget.Dot(-spine3TM.c));
    if (toTarget.Dot(spine3TM.b) < 0.0f)//-spine3TM.b = right
      targetAngle *= -1.0f;
#if ART_ENABLE_BSPY & PointGunBSpyDraw
    bspyScratchpad(m_character->getBSpyID(), "NmRsCBUPointGun", targetAngle);
#endif
    ...
    if (m_parameters.useSpineTwist)
    {
      ...
      if (targetAngle >= 0.0f && twistAvailableLeft > 0.0f)
        targetAngle -= twistAvailableLeft;
      if (targetAngle <= 0.0f && twistAvailableRight < 0.0f)
        targetAngle -= twistAvailableRight;
#if ART_ENABLE_BSPY & PointGunBSpyDraw
    ...
    bool horizontalAngleOK = targetAngle < m_parameters.maxAngleAway*DtoR && targetAngle > -m_parameters.maxAngleAcross*DtoR;
    ...
    dotOntoTDir = centerOfCone.Dot(armData.targetDirection);
#if ART_ENABLE_BSPY & PointGunBSpyDraw
...
#define REDUCE_WEAPON_DISTANCE 0 
    ...
    //mmmmTODO leadTarget here or added after target out of range calcs?  leadTarget not used currently
    //rage::Vector3 target(armData.leadTarget);
    rage::Vector3 target(armData.target);
  ...
  NmRsCBUPointGun::ArmStatus NmRsCBUPointGun::pointGunHand(ArmData& armData, bool calculate)
  {
    if (calculate)
      calculateGunHand(armData);
#if ART_ENABLE_BSPY & PointGunBSpyDraw
  ...
  void NmRsCBUPointGun::supportConstraintUpdate(ArmData& primaryArmData, ArmData& supportArmData)
  {
    ...
    //Use rage position constraint
    float currentGunSupportDistanceFromHand = supportArmData.currentGunSupportDistanceFromHand;
    bool supportPositionClose = currentGunSupportDistanceFromHand < m_parameters.makeConstraintDistance;
    if (useConstraint && m_parameters.supportConstraint == 1 && supportPositionClose)
    {
      m_character->constrainSupportHand(
        primaryArmData.arm,
        supportArmData.arm,
        &supportArmData.currentGunSupportInWorld.d, 
        &currentGunSupportDistanceFromHand, 
  ...
  void NmRsCBUPointGun::pointDoubleHandedGun(ArmData& primaryArmData, ArmData& supportArmData)
  {
    ...
    //Arms are set in isConstraintBroken if the constraint is broken.//mmmmTODO use output of this routine but move the arms here
    ...
    if (gunArmStatus == kPointing)
    {
      ...
      calculateGunHand(primaryArmData);
      //NB isSupportHandTargetReachable returns true if supportArmData.desiredGunSupportable true
      bool pointTheGunHand = (m_parameters.enableLeft && m_enableLeft && isSupportHandTargetReachable(primaryArmData, supportArmData)) || 
    ...
    if (gunArmStatus == kNeutral && !(m_parameters.enableRight && poseUnusedArm(primaryArmData)))
      gunArmStatus = kUnused;

    //Can't be sure what an non-enabled right arm is doing therefore don't try to support it
    if (gunArmStatus == kUnused || !(m_parameters.enableLeft && m_enableLeft))
      supportArmStatus = kUnused;//Should have the same effect as setting m_parameters.enableLeft = false

    if (supportArmStatus != kUnused)
    {
      // limbs todo figure out if we need to mask this...
      (static_cast<NmRs3DofEffector*>(m_character->getEffectorDirect(supportArmData.arm->getClavicle()->getJointIndex())))->storeZeroPose(); //mmmmWhy?
          ...
          m_parameters.oneHandedPointing == 3))
    {
      ...
      // so that unreachable zeroPoses disconnect.
      ...
      case kSupporting:
        {
          ...
          if(constraintActive && (gunArmStatus == kPointing || 
  ...
  {
    m_character->ReleaseConstraintSafetly(m_rifleConstraint);
  }
  ...
  NmRsCBUPointGun::ArmStatus NmRsCBUPointGun::setNeutralPoseOrNeutralPoint(ArmData& armData)
  {
    ...
    {
      pointGunHand(armData, true);//we ignore the kPointing/kPointingWithError return values.  As we want to tell the game we are not pointing at the target
      return kNeutralPointing;
    }
    else
    {
  ...
  NmRsCBUPointGun::ArmStatus NmRsCBUPointGun::setNeutralPose(ArmData& armData)
  {
    ...
    if(poseUnusedArm(armData))
    {
      ...
      else
      {
        ...
        {
          if (m_neutralSidePoseTimer > 0.0f && m_supportConstraintInhibitTimer > 0.0f)//Only do if constraint broken
          {
            setNeutralPoseType(armData, npsBySide);
            m_neutralSidePoseTimer -= m_character->getLastKnownUpdateStep();
          }
  ...
  void NmRsCBUPointGun::fireWeapon(NmRsHumanBody& body, int hand, float fireStrength, bool applyFireGunForceAtClavicle, float inhibitTime, rage::Vector3& direction, float split /* = 0.5 */)
  {
    ...
    {
      body.setStiffness(m_armData[NmRsCharacter::kRightHand].relaxScale*m_parameters.armStiffness, m_parameters.armDamping, bvmask_ArmLeft);
    }
  }
  ...
  void NmRsCBUPointGun::updateArmData(float timeStep)
  {
    ...
    for(i = 0; i < 2; ++i)
    {
      ...
      if(i == NmRsCharacter::kRightHand)
      {
        ...
        armData->twist = m_parameters.extraTilt;//mmmmUnused?
      ...
      else
      {
        ...
        if(armData->lastTarget.Mag2() < 0.01f)
          armData->targetVel.Zero();
        else
        {
      ...
      //mmmmTODO leadTarget causes target to be bad on the frame of the message due to wrong armData->targetVel?  leadTarget not used currently
...
#endif
  {
    ...
    float scale = 1.0f-armData.relaxTimer/m_parameters.fireWeaponRelaxTime;
#if ART_ENABLE_BSPY & PointGunBSpyDraw & 0
    ...
    rage::Vector3 leanTwist = rsQuatToRageDriveTwistSwing(driveQuat);
    arm->getClavicle()->clampRawLeanTwist(leanTwist, limitAmount);
    arm->getClavicle()->getTwistAndSwingFromRawTwistAndSwing(leanTwist, leanTwist);
    armInputData->getClavicle()->setDesiredTwist(leanTwist.x);
    armInputData->getClavicle()->setDesiredLean1(leanTwist.y);
    armInputData->getClavicle()->setDesiredLean2(leanTwist.z);
    ...
    float limitAmount)
  {
    ...
    rage::Matrix34 clavOrientationChild;
    rage::phJoint* clavJt = arm->getClavicle()->getJoint();
    clavJt->GetOrientationChild(clavOrientationChild);
  ...
  */
    ...
    rage::Matrix34& shoulderMatrix1)
  {
    ...
    */
    ...
    arm->getShoulder()->clampRawLeanTwist(ts, 1.1f);
#endif
    arm->getShoulder()->getTwistAndSwingFromRawTwistAndSwing(ts, ts);
    armInputData->getShoulder()->setDesiredTwist(ts.x);
    armInputData->getShoulder()->setDesiredLean1(ts.y);
    armInputData->getShoulder()->setDesiredLean2(ts.z);
  }
...
#define POINT_CLAVICLE_FORWARD 1
    ...
    {
      armInputData->getClavicle()->blendToZeroPose(arm->getClavicle(), m_parameters.clavicleBlend);
...
#define IK_USE_BASE_POSE 0
    ...
    rage::Vector3 shoulderBaseLeanTwist(0.0f, 0.0f, 0.5f);
    rage::Quaternion shoulderBaseQuat = rsRageLimitTwistSwingToQuat(shoulderBaseLeanTwist);
    ...
    */
    ...
    wristJt->GetOrientationChild(handLocal);
    ...
    float wristBlend)
  {
    ...
    jt->GetOrientationChild(orientationChild);
    ...
    {
      armInputData->getClavicle()->blendToZeroPose(arm->getClavicle(), clavicleBlend);
    ...
    if (!(m_weaponMode == kRifle || (m_weaponMode == kPistol && arm == getLeftArm())))
    {
      ...
      rage::Vector3 targetDirection, targetDirectionLocal, desiredLeanTwist;
      ...
      desiredLeanTwist = rsQuatToRageDriveTwistSwing(desiredRotation);
      float error = arm->getShoulder()->clampRawLeanTwist(desiredLeanTwist);
      ...
      if(error > 0)
      {
        ...
        clampedRotation = rsRageDriveTwistSwingToQuat(desiredLeanTwist);
        rage::Vector3 temp;
#if ART_ENABLE_BSPY & PointGunBSpyDraw & 0      
...
#define POINTGUN_PD_HAND_POSITION_DEBUG (0 & PointGunBSpyDraw)
    ...
    rage::Vector3 leanTwist = rsQuatToRageDriveTwistSwing(driveQuat);
    arm->getWrist()->getTwistAndSwingFromRawTwistAndSwing(leanTwist, leanTwist);
    //Clamp twist.
    //Hack as rsQuatToRageDriveTwistSwing is sometimes returning twist angles >PI <-PI.
    //Also driveQuat.GetAngle() sometimes >PI <-PI
    if(leanTwist.x > PI)
      leanTwist.x -= 2.0f*PI;//HAck
    if(leanTwist.x < -PI)
      leanTwist.x += 2.0f*PI;//HAck
#if ART_ENABLE_BSPY
    ...
    leanTwist.Lerp(1.0f - wristBlend, zero);

    armInputData->getWrist()->setDesiredTwist(leanTwist.x);
    armInputData->getWrist()->setDesiredLean1(leanTwist.y);
    armInputData->getWrist()->setDesiredLean2(leanTwist.z);
    }
    else
    {
  ...
  float NmRsCBUPointGun::clampAndDriveQuat(const NmRs3DofEffector* eff, NmRs3DofEffectorInputWrapper* effData, rage::Quaternion& q, float amount)
  {
    ...
    rage::Vector3 ts = rsQuatToRageDriveTwistSwing(q);
    eff->getTwistAndSwingFromRawTwistAndSwing(ts, ts);

    effData->setDesiredTwist(ts.x);
    effData->setDesiredLean1(ts.y);
    effData->setDesiredLean2(ts.z);
...
#undef PGNP_NAME_ACTION
    ...
    bspyTaskVar(m_parameters.extraTilt, true);
    ...
    bspyTaskVar(m_parameters.stabilizeRifleStock, true);
...
#undef HA_NAME_ACTION
    ...
    bspyTaskVar(armDataR->twist, false);
    ...
    bspyTaskVar(armDataL->twist, false);
    ...
    if (m_weaponMode == kPistol || m_weaponMode == kRifle)
    {
      ...
      bspyTaskVar(currentGunSupportDistanceFromHand, false);
        ...
        //npsNone)               /*0 */
        //npsNotRequested)       /*1 */
        //npsByFace)             /*2 */Pistol by face
        //npsByHip)              /*3 */Pistol/Rifle by Hip
        //npsAcrossFront)        /*4 */Pistol/Rifle by Hip
        //npsPointing2Connect)   /*5 */
        //npsByFaceSupport)      /*6 */
        //npsByHipSupport)       /*7 */
        //npsAcrossFrontSupport) /*8 */
        //npsBySide)             /*9 */
        //npsPointingDontConnect)/*10*/
    }
    //else
    // for each arm:
