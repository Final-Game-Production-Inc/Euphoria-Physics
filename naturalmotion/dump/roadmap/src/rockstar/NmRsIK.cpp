...
{ 
  ...
  */
  ...
  // BBDD Self avoidance tech.
  inline float floatSelect(float fComparand, float fValGE, float fLT)
  {
    return (fComparand >= 0.0f ? fValGE : fLT);
  }
  ...
  inline rage::Vector3 getSelfAvoidPolarPos(NmRsLimbIKInput& input,const rage::Vector3& inputTarget)
  {
    ...
    // 6. Special case: if the linear path is going the log way around then we need to give it a negative magnitude.
  ...
  inline rage::Vector3 getSelfAvoidPos(NmRsLimbIKInput& input, const rage::Vector3& inputTarget)
  {
    ...
    if ((tr < armLength) && isInSpineBounds)
    {
      ...
      const float offset = er * (1.0f - w) + (float)__fsel(cosTheta, tr, l) * w;
#else
      const float offset = er * (1.0f - w) + floatSelect(cosTheta, tr, l) * w;
      ...
      if (input.selfAvoidance.selfAvoidanceParams.overwriteTwist)
      {
        ...
        input.twist = floatSelect(cosPhi, -minTwistRad, -maxTwistRad) * cosPhi;
      }
