...
namespace ART
{
    ...
    m_stringTokenIndex(0)
  {
    ...
    m_frameTicker = 0;
    m_sessionUID = (unsigned int)this;
...
# endif // NM_PLATFORM_X360
  ...
  DWORD WINAPI SpyReadThread_msvc(LPVOID parameter)
#elif defined(NM_PLATFORM_CELL_PPU)
  void SpyReadThread_cell(uint64_t parameter)
#endif 
  {
    bSpyServer *spy = (bSpyServer*)((unsigned int)parameter);
...
#if defined(NM_COMPILER_MSVC)
    while (WaitForSingleObject(terminationEvent, 200) == WAIT_TIMEOUT)
#elif defined(NM_PLATFORM_CELL_PPU)
    while (sys_semaphore_wait(terminationEvent, 200) == ETIMEDOUT)
#endif 
    {
      if (junkCounter)
      {
...
//#endif // ART_ENABLE_BSPY_LOGGING
  ...
  bool bSpyServer::startServer()
  {
    ...
    {
#if ART_ENABLE_BSPY_LOGGING
      Displayf("NMLOG | bSpyServer: unable to create socket");
#endif // ART_ENABLE_BSPY_LOGGING
      return false;
    }
...
#if ART_ENABLE_BSPY_LOGGING
      Displayf("NMLOG | bSpyServer: setsockopt failed");
#endif // ART_ENABLE_BSPY_LOGGING
...
# endif // ART_ENABLE_BSPY_LOGGING
  ...
  void bSpyServer::shutdownClient()
  {
    ...
    SetEvent(m_readThreadTerminator);
    WaitForSingleObject(m_readThread, INFINITE);

    CloseHandle(m_readThreadTerminator);
    CloseHandle(m_readThread);

#elif defined(NM_PLATFORM_CELL_PPU)

    sys_semaphore_post(m_readThreadTerminator, 1);

    uint64_t exit_code;
    sys_ppu_thread_join(m_readThread, &exit_code);
    sys_semaphore_destroy(m_readThreadTerminator);
  ...
  void bSpyServer::beginStep(float)
  {
      ...
      )
    {
      if (m_socketListenInterval <= 0)
      {
        ...
        nmsock_select(m_serverSocket + 1, &readSockets, 0, 0, &timeout);
        ...
        if (FD_ISSET(m_serverSocket, &readSockets))
        {
          ...
          m_readThreadTerminator = CreateEvent(NULL, 0, 0, NULL);
          m_readThread = CreateThread(NULL, 0, SpyReadThread_msvc, ( VOID* )this, 0, NULL);

#elif defined(NM_PLATFORM_CELL_PPU)

          sys_semaphore_attribute_t attr;
          sys_semaphore_attribute_initialize(attr);
          sys_semaphore_create(&m_readThreadTerminator, &attr, 0, 1);

          int ret = sys_ppu_thread_create(&m_readThread, SpyReadThread_cell, 
            (uint64_t)this, 
            0x512,
            NM_BSPY_COMMBUF_SZ + (2 * 1024),
            SYS_PPU_THREAD_CREATE_JOINABLE,
            "SpyReadThread");
                  
          FastAssert(ret == CELL_OK); (void)ret;
...
#if ART_ENABLE_BSPY_LOGGING
      Displayf("NMLOG | bSpyServer: traffic serializer activated [%08x]", (unsigned int)m_fileToSerializeTo);
#endif // ART_ENABLE_BSPY_LOGGING
...
#if ART_ENABLE_BSPY_LOGGING
      Displayf("NMLOG | bSpyServer: session UID [%08x]", m_sessionUID);
#endif // ART_ENABLE_BSPY_LOGGING
...
#if ART_ENABLE_BSPY_LOGGING
        Warningf("NMLOG Warning| bSpyServer: sendBuffer (%i bytes) - failed, closing connection", m_bufferUsed);
#endif // ART_ENABLE_BSPY_LOGGING
        return;
      }
...
#if ART_ENABLE_BSPY_LOGGING
        Warningf("NMLOG Warning| bSpyServer: sendPCBuffer (%i bytes) - failed, closing connection", m_bufferUsed);
#endif // ART_ENABLE_BSPY_LOGGING
        return;
      }
  ...
  void bSpyServer::sendIncomingTransforms(ART::NmRsCharacter* character)
  {
    ...
    for (int iS=0; iS<numStreamsToSend; iS++)
    {
      ...
      {
        NMutils::NMMatrix4& _tm = itPtr[i];

        for (int j=0; j<4; j++)
          for (int k=0; k<3; k++)
            tsp.m_tms[i].m_row[j].v[k] = _tm[j][k];
      }
  ...
  void bSpyServer::bSpyLogFormat(bool ZF, bool tryCache, bSpy::LogStringPacket::LogType type, const char* file, int line, const wchar_t* format, ...)
  {
    ...
    int bufWritten = vswprintf(m_logBuffer, BS_LOG_BUFFER_SIZE -1, format, args);
#else
    int bufWritten = _vsnwprintf(m_logBuffer, BS_LOG_BUFFER_SIZE -1, format, args);
