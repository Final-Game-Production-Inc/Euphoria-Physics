...
{
	return bk.AddSlider( m_name, &m_value, (int)p.GetMinValue(), (int)p.GetMaxValue(), p.GetStepValue(), NullCB, p.GetDescription() ) ;
}
#endif
...
void NMParam::SetDescription( const char* description )
{
	if ( description != NULL )
	{
		safecpy( m_description, description, 255 );
	}
	else
	{
		m_description[0] = 0;
...
void NMBehaviorInst::Init()
{
	...
	SetDescription( m_pBehavior->GetDescription() );
...
void NMBehaviorInst::AddWidgets( bkBank &bk )
{
	...
	m_pBkGroup = bk.PushGroup( m_name, true, m_description );
	{
		...
		m_widgets.Grow() = bk.AddText( "Instance Description", m_description, 255 );
