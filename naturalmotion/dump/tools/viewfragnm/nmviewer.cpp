...
, m_Animation(NULL)
, m_AnimFrame(NULL)
, m_TranslationPin(NULL)
, m_ActivePoseStiffness(16.0f)
, m_RotationPin(NULL)
, m_AnimPhase(0.0f)
, m_FrameRate(1.0f)
#if __BANK
...
void NMViewer::ShutdownClient()
{
	...
	if (m_TranslationPin)
	{
		PHSIM->GetConstraintMgr()->ReleaseConstraint(m_TranslationPin);
		m_TranslationPin = NULL;
	}
	if (m_RotationPin)
	{
		PHSIM->GetConstraintMgr()->ReleaseConstraint(m_RotationPin);
		m_RotationPin = NULL;
	}
        ...
        if (m_AnimFrame)
        {
			...
			if (m_UseRageActivePose || m_UseNMActivePose)
			{
				...
				if (m_ActivePoseInst->IsInLevel() && !PHLEVEL->IsActive(m_ActivePoseInst->GetLevelIndex()) && m_ActivePoseInst->GetCached())
				{
					...
					skeleton->Update();
					m_ActivePoseInst->PoseBoundsFromSkeleton();
				}
			}
			...
			if (m_UseRageActivePose)
			{
				...
				if (m_ActivePoseInst && m_Inst->GetCacheEntry()->GetHierInst()->body)
				{
					...
					if (!m_TranslationPin)
					{
						Assert(!m_RotationPin);
						phConstraintMgr* pConstraintMgr = PHSIM->GetConstraintMgr();
						m_TranslationPin = pConstraintMgr->AttachObjectToWorld('Pin1', *m_Inst, 10, 0.0f, m_TranslationPin);	
						m_RotationPin = pConstraintMgr->AttachObjectToWorldRotation('Pin2', *m_Inst, 10, 0.0f, m_RotationPin);	
					}

					m_Inst->GetCacheEntry()->GetHierInst()->body->SetStiffnessWithNMValue(m_ActivePoseStiffness);
			...
			if (m_TranslationPin && ((!m_UseRageActivePose && !m_UseNMActivePose) || 
            ...
            for (int extraInst = 1; extraInst < EXTRA_INSTS; ++extraInst)
            {
                    ...
                    (m_UseNMActivePose || !PHLEVEL->IsActive(m_Insts[extraInst]->GetLevelIndex())))
                {
                    ...
                    {
                        m_Insts[extraInst]->PoseBoundsFromSkeleton();
                    }
                    else if (m_UseNMActivePose)
                    {
