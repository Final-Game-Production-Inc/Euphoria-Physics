...
overwrite = True
rootJoint = False
if nmx.scene().object("|Scene|character").attribute("HasRootJoint").isValid():
  rootJoint = nmx.scene().object("|Scene|character").HasRootJoint.boolean()
...
def doubleTohex(value):
  ...
  # Load our own dll
  custom = ctypes.cdll.LoadLibrary(r"C:\NM\assets\euphoriaStudioProduction\Rockstar\Character\Endorphin\ExportUtils\debug\ExportUtils")
...
def writeRootData(outputFile, object, indent):
  if rootJoint == True :
	  ...
	  childObjects = object.childObjects()[0].childObjects()
	  if len(childObjects) > 0:
		object = childObjects[0]
		for childObject in childObjects:
		  if childObject.isType('Transform'):
...
def writePhysicsCharacterData(outputFile, physicsCharacter):
  ...
  if rootJoint == True :
    ...
    physicsCharacterPastRoot =  physicsCharacter.childObjects()[0]
...
def writePhysicsCharacterDataNMXML(outputFile, physicsCharacter):
  ...
  if rootJoint == True :
    ...
    rootBody = nmx.scene().object("|Scene|character|physicsCharacter|SKEL_Pelvis|SKEL_PelvisPhysicsBodyTM|SKEL_PelvisPhysicsBody")
...
def orientatePhysicsBodyFromDefault(physicsBody):
  ...
  ##redo bodies to align correctly but don't move bone i.e. after mirroring gets it wrong
  #Body_BoneAxisShouldBe = Body_BoneAxis 
  #
  physicsBody_WorldFrame = transform.WorldMatrix()
  #if BoneAxisIndex == 0:
  #  Body_BoneAxis = physicsBody_WorldFrame.xAxis()
  #if BoneAxisIndex == 1:
  #  Body_BoneAxis = physicsBody_WorldFrame.yAxis()
  #if BoneAxisIndex == 2:
  #  Body_BoneAxis = physicsBody_WorldFrame.zAxis()
  #if (Body_BoneAxisShouldBe.dotProduct(Body_BoneAxis) < 0.0):
  #  Body_BoneAxis.scale(-1.0)
...
def saveModifications(object):
  ...
  if overwrite == False and os.path.isfile(fileName) == True :
    ...
    if rootJoint == True :
      ...
      physicsCharacterRoot =  physicsCharacter.childObjects()[0]
    print "physicsCharacterRoot: ", physicsCharacterRoot.Name.string()
    #write root info 1st 
    jointChildObjects = physicsCharacterRoot.childObjects()
    bodyTransformName = physicsCharacterRoot.Name.string() +"PhysicsBodyTM"
...
def initWorkspace(physicsCharacter):
  ...
  if not physicsCharacter.attribute("AssetName").isValid():
    physicsCharacter.addStringAttribute("AssetName", r"NM_Character", False)
  if not physicsCharacter.attribute("AssetTypeId").isValid():
    physicsCharacter.addIntegerAttribute("AssetTypeId", 0, 0, 50, False)
  if not physicsCharacter.attribute("InputDirectory").isValid():
    physicsCharacter.addStringAttribute("InputDirectory", r"C:/", False)
  if not physicsCharacter.attribute("OutputDirectory").isValid():
  ...
  if not nmx.scene().object("|Scene|character").attribute("Trans2ModelX").isValid():
    nmx.scene().object("|Scene|character").addFloatAttribute("Trans2ModelX",0.0,-360.0,360.0,False)
  if not nmx.scene().object("|Scene|character").attribute("Trans2ModelY").isValid():
    nmx.scene().object("|Scene|character").addFloatAttribute("Trans2ModelY",0.0,-360.0,360.0,False)
  if not nmx.scene().object("|Scene|character").attribute("Trans2ModelZ").isValid():
    nmx.scene().object("|Scene|character").addFloatAttribute("Trans2ModelZ",0.0,-1000.0,1000.0,False)
  if not nmx.scene().object("|Scene|character").attribute("Rotate2ModelX").isValid():
    nmx.scene().object("|Scene|character").addFloatAttribute("Rotate2ModelX",0.0,-360.0,360.0,False)
  if not nmx.scene().object("|Scene|character").attribute("Rotate2ModelY").isValid():
    nmx.scene().object("|Scene|character").addFloatAttribute("Rotate2ModelY",0.0,-360.0,360.0,False)
  if not nmx.scene().object("|Scene|character").attribute("Rotate2ModelZ").isValid():
    nmx.scene().object("|Scene|character").addFloatAttribute("Rotate2ModelZ",0.0,-360.0,360.0,False)
  if not nmx.scene().object("|Scene|character").attribute("HasRootJoint").isValid():
    nmx.scene().object("|Scene|character").addBooleanAttribute("HasRootJoint",False,False)
