...
void btCompoundCollisionAlgorithm::DetectCollision (const rage::phCollisionInput& , rage::Vector3::Param , rage::phManifold& , DiscreteCollisionDetectorInterface::Result& )
{
#if 0
	btCollisionObject* colObj = static_cast<btCollisionObject*>(m_compoundProxy.m_clientObject);
	assert (colObj->m_collisionShape->isCompound());
	...
	for (i=0;i<numChildren;i++)
	{
		...
		colObj->m_collisionShape = childShape;
		m_childCollisionAlgorithms[i]->DetectCollision();
		//revert back
		colObj->m_collisionShape =orgShape;
...
#if 0
float	btCompoundCollisionAlgorithm::calculateTimeOfImpact(rage::phInst* ,rage::phInst* ,const btDispatcherInfo& )
{
    btCollisionObject* colObj = static_cast<btCollisionObject*>(m_compoundProxy.m_clientObject);
	assert (colObj->m_collisionShape->isCompound());
