...
namespace gjkepa_impl
{
	...
	typedef rage::Vector3		Vector3;
	typedef rage::Matrix34		Rotation;
...
#define cstInf				FLT_MAX
#define cstPi				PI
#define	cst2Pi				(PI * 2.0f)
...
#define EPA_accuracy		F(0.001)
	...
	struct	GJK
	{
		...
		GJK(btStackAlloc* psa,
			const Rotation& wrot0,const Vector3& pos0,const rage::phBound* shape0,
			const Rotation& wrot1,const Vector3& pos1,const rage::phBound* shape1,
			F pmargin=0)
		{
			wrotations[0]=wrot0;wrotations[0].d.Zero();positions[0]=pos0;shapes[0]=shape0;
			wrotations[1]=wrot1;wrotations[1].d.Zero();positions[1]=pos1;shapes[1]=shape1;
			sa		=psa;
		...
		{
			Vector3 rotatedD;
			wrotations[i].UnTransform(d, rotatedD);
			Vector3 result;
			result = VEC3V_TO_VECTOR3(shapes[i]->BTLOCALSUPPORT(rotatedD));
			wrotations[i].Transform(result);
...
#define SPXW(_i_)	simplex[_i_].w
		...
		inline Z		SolveSimplex2(const Vector3& ao,const Vector3& ab)
		{
			if(ab.Dot(ao)>=0)
			{
				Vector3	cabo;//(cross(ab,ao));
				cabo.Cross(ab,ao);
				if(cabo.Mag2()>GJK_sqinsimplex_eps)
				{ ray.Cross(cabo,ab); }
				else
				{ return(true); }
			}
			...
			ac)
		{
			Vector3 abac;
			abac.Cross(ab,ac);
			return(SolveSimplex3a(ao,ab,ac,abac));
		}
		//
		...
		{
			Vector3 cabcab;
			cabcab.Cross(cabc,ab);
			Vector3 cabcac;
			cabcac.Cross(cabc,ac);
			if(cabcab.Dot(ao)<-GJK_insimplex_eps)
			{ order=1;SPX(0)=SPX(1);SPX(1)=SPX(2);return(SolveSimplex2(ao,ab));	}
			else	if(cabcac.Dot(ao)>+GJK_insimplex_eps)
			...
			ac,const Vector3& ad)
		{
			...
			Vector3			crsadab;
			crsadab.Cross(ad,ab);
			if(crsabac.Dot(ao)>GJK_insimplex_eps)
			{ 
				order=2;SPX(0)=SPX(1);SPX(1)=SPX(2);SPX(2)=SPX(3);return(SolveSimplex3a(ao,ab,ac,crsabac)); 
			}
			else	if(crsacad.Dot(ao)>GJK_insimplex_eps)
			{ order=2;SPX(2)=SPX(3);return(SolveSimplex3a(ao,ac,ad,crsacad)); }
			else	if(crsadab.Dot(ao)>GJK_insimplex_eps)
			{ 
				order=2;SPX(1)=SPX(0);SPX(0)=SPX(2);SPX(2)=SPX(3);return(SolveSimplex3a(ao,ad,ab,crsadab)); 
			}
			else	return(true);
		}
		//
		...
		inline Z		EncloseOrigin()
		{
			...
			case	1:
				{
					...
					Vector3	b[3];
					b[0].Cross(ab,Vector3(1,0,0));
					b[1].Cross(ab,Vector3(0,1,0));
					b[2].Cross(ab,Vector3(0,0,1));
					const F			m[]={b[0].Mag2(),b[1].Mag2(),b[2].Mag2()};
					Rotation	r;
					r.RotateFull(ab,cst2Pi/3);
					Vector3			w(b[m[0]>m[1]?m[0]>m[2]?0:2:m[1]>m[2]?1:2]);
					Vector3         norm(w);
					norm.Normalize();
					Support(norm,simplex[4]);r.Transform3x3(w);
					Support(norm,simplex[2]);r.Transform3x3(w);
					Support(norm,simplex[3]);r.Transform3x3(w);
					order=4;
					return(true);
				}
				// break;
			...
			case	2:
				{
					Vector3	n;
					n.Cross(SPXW(1)-SPXW(0),SPXW(2)-SPXW(0));
					n.Normalize();
					Support( n,simplex[3]);
					Support(-n,simplex[4]);
					order=4;
...
#undef SPXW
	...
	struct	EPA
	{
		...
		inline Vector3	GetCoordinates(const Face* face) const
		{
			...
			Vector3 cross0; cross0.Cross(face->v[0]->w-o,face->v[1]->w-o);
			Vector3 cross1; cross1.Cross(face->v[1]->w-o,face->v[2]->w-o);
			Vector3 cross2; cross2.Cross(face->v[2]->w-o,face->v[0]->w-o);
			const F			a[]={	cross0.Mag(),
				cross1.Mag(),
				cross2.Mag()};
		...
		inline Z		Set(Face* f,const GJK::Mkv* a,const GJK::Mkv* b,const GJK::Mkv*
			c) const
		{
			Vector3	nrm; nrm.Cross(b->w-a->w,c->w-a->w);
			const F			len(nrm.Mag());
			Vector3 crossab; crossab.Cross(a->w,b->w);
			Vector3 crossbc; crossbc.Cross(b->w,c->w);
			Vector3 crossca; crossca.Cross(c->w,a->w);
			const Z			valid(	(crossab.Dot(nrm)>=-EPA_inface_eps)&&
				(crossbc.Dot(nrm)>=-EPA_inface_eps)&&
				(crossca.Dot(nrm)>=-EPA_inface_eps));
			f->v[0]	=	a;
			f->v[1]	=	b;
			f->v[2]	=	c;
			f->mark	=	0;
...
//
bool	btGjkEpaSolver::Collide(rage::phBound *shape0,const rage::Matrix34 &wtrs0,
...
#if __SPU
	btStackAlloc stackAllocOnStack(10 * 1024);
	btStackAlloc* stackAlloc = &stackAllocOnStack
#else
	btStackAlloc* stackAlloc = PHSIM->GetPerThreadStackAlloc();
#endif
	Assert(stackAlloc);
	GJK			gjk(stackAlloc,
		wtrs0,wtrs0.d,shape0,
		wtrs1,wtrs1.d,shape1,
