...
{
private:
	HeapManager *mPool		__attribute__((aligned(16)));
...
public:
	ReadOnlyPrefetchForwardIterator(HeapManager *pool,uint32_t initialEA,uint32_t endEA,int prefetchNum,int tag=0);
	~ReadOnlyPrefetchForwardIterator();
...
template <class T>
inline
ReadOnlyPrefetchForwardIterator<T>::ReadOnlyPrefetchForwardIterator(HeapManager *pool,uint32_t initialEA,uint32_t endEA,int prefetchNum,int tag)
...
ReadOnlyPrefetchForwardIterator<T>& ReadOnlyPrefetchForwardIterator<T>::operator++()
{
	...
	mIndex++;
	mPtrBuf++;
	if(PHYS_UNLIKELY(mIndex>=mPrefetchNum)) {
		cellDmaWaitTagStatusAll(1<<mDmaTag);
...
public:
	PrefetchForwardIterator(HeapManager *pool,uint32_t initialEA,uint32_t endEA,int prefetchNum,int tag=0);
	~PrefetchForwardIterator();
...
template <class T>
inline
PrefetchForwardIterator<T>::PrefetchForwardIterator(HeapManager *pool,uint32_t initialEA,uint32_t endEA,int prefetchNum,int tag)
	:mPool(pool),mDmaTag(tag),mPrefetchNum(prefetchNum),mEA(initialEA),mEndEA(endEA),mIndex(0),mCurrentBuffer(0)
{
	mBuffer[0] = (T*)mPool->allocate(sizeof(T)*mPrefetchNum,HeapManager::ALIGN128);
	mBuffer[1] = (T*)mPool->allocate(sizeof(T)*mPrefetchNum,HeapManager::ALIGN128);
...
public:
	ReadOnlyPrefetchBackwardIterator(HeapManager *pool,uint32_t initialEA,uint32_t startEA,int prefetchNum,int tag=0);
	~ReadOnlyPrefetchBackwardIterator();
...
template <class T>
inline
ReadOnlyPrefetchBackwardIterator<T>::ReadOnlyPrefetchBackwardIterator(HeapManager *pool,uint32_t initialEA,uint32_t startEA,int prefetchNum,int tag)
...
public:
	PrefetchBackwardIterator(HeapManager *pool,uint32_t initialEA,uint32_t startEA,int prefetchNum,int tag=0);
	~PrefetchBackwardIterator();
...
template <class T>
inline
PrefetchBackwardIterator<T>::PrefetchBackwardIterator(HeapManager *pool,uint32_t initialEA,uint32_t startEA,int prefetchNum,int tag)
	:mPool(pool),mDmaTag(tag),mPrefetchNum(prefetchNum),mEA(initialEA),mStartEA(startEA),mIndex(prefetchNum),mCurrentBuffer(0)
{
	mBuffer[0] = (T*)mPool->allocate(sizeof(T)*mPrefetchNum,HeapManager::ALIGN128);
	mBuffer[1] = (T*)mPool->allocate(sizeof(T)*mPrefetchNum,HeapManager::ALIGN128);
