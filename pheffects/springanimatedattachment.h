...
#define PHEFFECTS_SPRINGANIMATEDATTACHMENT_H
...
class phSpringAninmatedAttachment	: public phInstBehavior										
{
public:

	// PURPOSE: Resource constructor
	phSpringAninmatedAttachment(datResource& rsc) : phInstBehavior(rsc) { }

	// PURPOSE: Constructor, pass in a pointer to a position, it automagically keeps track of it. Also pass in spring parameters.
	phSpringAninmatedAttachment( const Vector3 &thisAttachmentPoint, const Vector3 &otherAttachmentPoint, float springConstant, float springLength, float springDampening)	: 
		m_PrevOtherAttachmentPosition(otherAttachmentPoint),
	...
	// use this to animate the attachment position
	void UpdateAttachmentPosition( const Vector3& x )
	{
		m_OtherAttachmentPosition = x;
	}
	...
	virtual void Reset() { m_PrevOtherAttachmentPosition = m_OtherAttachmentPosition; }
...
protected:
	Vector3 m_PrevOtherAttachmentPosition;
