...
namespace sce
{
	...
	namespace Gnmx
	{
		 ...
		 * @param f The float to convert. Must be in the range <c>[0..16)</c> or the function will assert.
		 ...
		 * @param f The float to convert. Must be in the range <c>[0..4096)</c>, or the function will assert.
		 ...
		 * @param f The float to convert. Must be in the range <c>(-32..32)</c>, or the function will assert.
		 ...
		 * @param f The float to convert. Must be in the range <c>(-2..2)</c>, or the function will assert.
		 ...
		 * @param f32 The float to convert.
		...
		* @brief Converts a half-precision IEEE float value to the nearest single-precision IEEE float.
		 * @param f16 The float to convert.
		 ...
		 * @param numSamples The number of samples per pixel. Must be >= <c><i>numFragments</i></c>.
		 * @param numFragments The number of fragments per pixel. Must be <= <c><i>numSamples</i></c>.
		 ...
		 * @param outVgtPrimCount The VGT primitive count will be written here. Note that you must subtract 1 from this value before passing it to setVgtControl().
		 * @param outPatchCount The number of patches per HS thread group will be written here. This value is used as an input to Gnm::TessellationDataConstantBuffer::init(), Gnm::TessellationRegisters::init(),
		 *                      and GfxContext::setLsHsShaders().
		 * @param maxHsVgprCount The maximum number of HS-stage general-purpose registers to allocate to patches for each threadgroup.
		 * @param maxHsLdsBytes The maximum amount of HS-stage local data store to allocate for patches for each threadgroup. This must also include space for the outputs of the LS stage.
		 * @param lsb The LsShader which will feed into hsb.
		...
		/** @brief Uses the CP DMA to clear a buffer to specified value (like a GPU memset) using a draw command buffer.
			...
			@param dcb					The draw command buffer to write GPU commands to.
			@param dstGpuAddr			The destination address to write the data to.
			@param srcData				The value to fill the destination buffer with.
			@param numBytes				The size of the destination buffer. This must be a multiple of 4.
			@param isBlocking    I		A flag that specifies whether the CP will block while the transfer is active.
		*/
		...
		/** @brief Uses the CP DMA to clear a buffer to specified value (like a GPU memset) using a measure draw command buffer.
			...
			@param dcb					The measure draw command buffer to write GPU commands to.
			@param dstGpuAddr			The destination address to write the data to.
			@param srcData				The value to fill the destination buffer with.
			@param numBytes				The size of the destination buffer. This must be a multiple of 4.
			@param isBlocking			A flag that specifies whether the CP will block while the transfer is active.
		*/
		...
		/** @brief Uses the CP DMA to transfer data from a source address to a destination address in a draw command buffer.
			...
			@param dcb					The draw command buffer to write GPU commands to.
			@param dstGpuAddr			The destination address to write the data to.
			@param srcGpuAddr			The source address to read the data from.
			@param numBytes				The number of bytes to transfer over.
			@param isBlocking			A flag that specifies whether the CP will block while the transfer is active.
		*/
		...
		*/
		...
		/** @brief Inserts user data directly inside the draw command buffer returning a locator for later reference.
			...
			@param dcb				The draw command buffer to write GPU commands to.
			@param dataStream		A pointer to the data to embed.
			@param sizeInDword		The size of the data in a stride of 4. Note that the maximum size of a single command packet is 2^16 bytes,
									and the effective maximum value of <c><i>sizeInDword</i></c> will be slightly less than that due to packet headers
									and padding.
			@param alignment		The alignment of the embedded copy in the CB.
		...
		*/		
		...
		/** @brief Sets the multisampling sample locations to default values in a draw command buffer. 
			...
			@param dcb				The Gnm::DrawCommandBuffer to write commands to.
			@param numAASamples		The number of samples used while multisampling.
		*/
		...
		/** @brief Sets the multisampling sample locations to default values in a measure draw command buffer.
			...
			@param dcb				The Gnm::MeasureDrawCommandBuffer to write commands to.
			@param numAASamples		The number of samples used while multisampling.
		*/
		...
		/** @brief A utility function that configures (for draw command buffers) the viewport, scissor, and guard band for the provided viewport dimensions.
			...
			@param zScale		The scale value for the Z transform from clip-space to screen-space. The correct value depends on which
			...
			@param zOffset		The offset value for the Z transform from clip-space to screen-space. The correct value depends on which
		...
		*/
		 ...
		 *
		 * @param dcb						The draw command buffer to write GPU commands to.
		 * @param destRangeByteOffset		The byte offset in GDS to the beginning of the counter range to clear. This must be a multiple of 4.
		 * @param startApiSlot				The index of the first <c>RW_Buffer</c> API slot whose counter should be updated. The valid range is <c>[0..Gnm::kSlotCountRwResource -1]</c>.
		 * @param numApiSlots				The number of consecutive slots to update. <c><i>startApiSlot</i></c> + <c><i>numApiSlots</i></c> must be less than or equal to Gnm::kSlotCountRwResource.
		...
		*/
		 ...
		 *
		 * @param dcb						The draw command buffer to write GPU commands to.
		 * @param destRangeByteOffset		The byte offset in GDS to the beginning of the counter range to update. This must be a multiple of 4.
		 * @param startApiSlot				The index of the first <c>RW_Buffer</c> API slot whose counter should be updated. The valid range is <c>[0..Gnm::kSlotCountRwResource -1]</c>
		 * @param numApiSlots				The number of consecutive slots to update. <c><i>startApiSlot</i></c> + <c><i>numApiSlots</i></c> must be less than or equal to Gnm::kSlotCountRwResource.
		 * @param srcGpuAddr				The GPU-visible address to read the new counter values from.
		 *
		 * @see Gnm::DispatchCommandBuffer::dmaData(), Gnm::DrawCommandBuffer::dmaData()
		 *
		 * @note  GDS accessible size is provided by sce::Gnm::kGdsAccessibleMemorySizeInBytes.
		 ...
		 *
		 * @param dcb						The draw command buffer to write GPU commands to.
		 * @param destGpuAddr				The GPU-visible address to write the counter values to.
		 * @param srcRangeByteOffset		The byte offset in GDS to the beginning of the counter range to read. This must be a multiple of 4.
		 * @param startApiSlot				The index of the first <c>RW_Buffer</c> API slot whose counter should be read. The valid range is <c>[0..Gnm::kSlotCountRwResource -1]</c>.
		 * @param numApiSlots				The number of consecutive slots to read. <c><i>startApiSlot</i></c> + <c><i>numApiSlots</i></c> must be less than or equal to Gnm::kSlotCountRwResource.
		 *
		 * @see Gnm::DispatchCommandBuffer::dmaData(), Gnm::DrawCommandBuffer::dmaData()
		 *
		 * @note  GDS accessible size is provided by sce::Gnm::kGdsAccessibleMemorySizeInBytes.
		 ...
		 *
		 * @param dcb						The measure command buffer to write GPU commands to.
		 * @param destGpuAddr				The GPU-visible address to write the counter values to.
		 * @param srcRangeByteOffset		The byte offset in GDS to the beginning of the counter range to read. This must be a multiple of 4.
		 * @param startApiSlot				The index of the first <c>RW_Buffer</c> API slot whose counter should be read. The valid range is <c>[0..Gnm::kSlotCountRwResource -1]</c>.
		 * @param numApiSlots				The number of consecutive slots to read. <c><i>startApiSlot</i></c> + <c><i>numApiSlots</i></c> must be less than or equal to Gnm::kSlotCountRwResource.
		...
		/* @brief Uses the CP DMA to clear a buffer to specified value (like a GPU memset) using a dispatch command buffer.
			...
			@param dcb				The dispatch command buffer to write GPU commands to.
			@param dstGpuAddr		The destination address to write the data to.
			@param srcData			The value to fill the destination buffer with.
			@param numBytes			The size of the destination buffer. This must be a multiple of 4.
			@param isBlocking		A flag that specifies whether the CP will block while the transfer is active.
		*/
		...
		/** @brief Uses the CP DMA to clear a buffer to specified value (like a GPU memset) using a measure dispatch command buffer.
			...
			@param dcb				The measure dispatch command buffer to write GPU commands to.
			@param dstGpuAddr		The destination address to write the data to.
			@param srcData			The value to fill the destination buffer with.
			@param numBytes			The size of the destination buffer. This must be a multiple of 4.
			@param isBlocking		A flag that specifies whether the CP will block while the transfer is active.
		...
		*/		
		 ...
		 * @param dcb						The dispatch command buffer to write GPU commands to.
		 * @param destRangeByteOffset		The byte offset in GDS to the beginning of the counter range to clear. This must be a multiple of 4.
		 * @param startApiSlot				The index of the first RW_Buffer API slot whose counter should be updated. The valid range is <c>[0..kSlotCountRwResource-1]</c>.
		 * @param numApiSlots				The number of consecutive slots to update. <c><i>startApiSlot</i></c> + <c><i>numApiSlots</i></c> must be <= kSlotCountRwResource.
		 * @param clearValue				The value to set the specified counters to.
 		 *
		 * @note  GDS accessible size is provided by sce::Gnm::kGdsAccessibleMemorySizeInBytes.
		 */
		...
		*/		
		 ...
		 * @param destGpuAddr				The GPU-visible address to write the counter values to.
		 * @param srcRangeByteOffset		The byte offset in GDS to the beginning of the counter range to read. This must be a multiple of 4.
		 * @param startApiSlot				The index of the first RW_Buffer API slot whose counter should be read. The valid range is <c>[0..kSlotCountRwResource-1]</c>.
		 * @param numApiSlots				The number of consecutive slots to read. <c><i>startApiSlot</i></c>+<c><i>numApiSlots</i></c> must be <= kSlotCountRwResource.
		 *
		 * @note  GDS accessible size is provided by sce::Gnm::kGdsAccessibleMemorySizeInBytes.
		 */
