...
void sce::Gnmx::setupScreenViewport(DrawCommandBuffer *dcb, uint32_t left, uint32_t top, uint32_t right, uint32_t bottom, float zScale, float zOffset)
{
	...
	SCE_GNM_VALIDATE(width >= 0 && width <= 16384, "right (%d) - left (%d) must be in the range [0..16384].", right, left);
	SCE_GNM_VALIDATE(height >= 0 && height <= 16384, "bottom (%d) - top (%d) must be in the range [0..16384].", bottom, top);
