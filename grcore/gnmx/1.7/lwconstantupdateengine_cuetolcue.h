...
namespace sce
{
	namespace Gnmx
	{
		...
		{
		public:
			// These methods are silently ignored as this doesn't apply to the LCUE
			...
			SCE_GNM_FORCE_INLINE void setGsModeOff() {} // GsMode is handled automatically by the LightweightConstantUpdateEngine, see GraphicsConstantUpdateEngine::setActiveShaderStages() for more details.
			...
			{
				// Not implemented yet
				SCE_GNM_UNUSED(acbBuffer); SCE_GNM_UNUSED(acbSizeInBytes);
			}
			...
			{
				// Not implemented yet
				SCE_GNM_UNUSED(pIndexRingBuffer); SCE_GNM_UNUSED(sizeofIndexRingBufferAlign256B);
			}
			...
			{
				// Not implemented yet
				SCE_GNM_UNUSED(left); SCE_GNM_UNUSED(top); SCE_GNM_UNUSED(right); SCE_GNM_UNUSED(bottom); 		 
			}
			...
			{
				// Not implemented yet
				SCE_GNM_UNUSED(primitiveSetup); SCE_GNM_UNUSED(clipControl);
			}
			...
			{
				// Not implemented yet
				SCE_GNM_UNUSED(dispatchDrawClipCullFlags);
			}

			
			SCE_GNM_LCUE_NOT_SUPPORTED void setDispatchDrawData(void const* pDispatchDrawData, uint32_t sizeofDispatchDrawData)
			{
				// Not implemented yet
				SCE_GNM_UNUSED(pDispatchDrawData); SCE_GNM_UNUSED(sizeofDispatchDrawData);
			}
			...
			{
				// Not implemented yet
				SCE_GNM_UNUSED(bufferInputData); SCE_GNM_UNUSED(numBlocksTotal); SCE_GNM_UNUSED(numPrimsPerVgt);
			}
			...
			{
				// Not implemented yet
				SCE_GNM_UNUSED(csvsb); SCE_GNM_UNUSED(shaderModifierVs); SCE_GNM_UNUSED(fetchShaderAddrVs); SCE_GNM_UNUSED(shaderModifierCs); SCE_GNM_UNUSED(fetchShaderAddrCs);
			}
			...
			{
				// Not implemented yet
				SCE_GNM_UNUSED(csb); SCE_GNM_UNUSED(shaderModifierCs); SCE_GNM_UNUSED(fetchShaderAddrCs);
			}
			...
			{
				LightweightConstantUpdateEngine::generateInputResourceOffsetTable(&m_boundShaderResourceOffsets[Gnm::kShaderStageEs], Gnm::kShaderStageEs, esb);
				GraphicsContext::setOnChipEsShader(esb, ldsSizeIn512Bytes, shaderModifier, fetchShaderAddr, &m_boundShaderResourceOffsets[Gnm::kShaderStageEs]);
			}
			...
			{
				LightweightConstantUpdateEngine::generateInputResourceOffsetTable(&m_boundShaderResourceOffsets[Gnm::kShaderStageGs], Gnm::kShaderStageGs, gsb);
				GraphicsContext::setOnChipGsVsShaders(gsb, gsPrimsPerSubGroup, &m_boundShaderResourceOffsets[Gnm::kShaderStageGs]);
			}
			...
			{
				SCE_GNM_VALIDATE((lsb && hsb) || (!lsb && !hsb), "lsb (0x%010llX) and hsb (0x%010llX) must either both be NULL, or both be non-NULL.", (unsigned long long)lsb, (unsigned long long)hsb);
				if (lsb && hsb)
				{
			...
			SCE_GNM_FORCE_INLINE void setTessellationFactorBuffer(void *tessFactorMemoryBaseAddr)
			{
				...
				GraphicsContext::setGlobalDescriptor(Gnm::kShaderGlobalResourceTessFactorBuffer, &tessFactorBuffer);
			}
