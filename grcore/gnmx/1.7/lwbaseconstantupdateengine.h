...
#define SCE_GNM_LCUE_VALIDATE_SHADER_PREFETCH_AREA(a, b)	SCE_GNM_ASSERT( sce::Gnmx::LightweightConstantUpdateEngine::validateShaderPreFetchArea(a, b) == LightweightConstantUpdateEngine::kErrorOk )
	...
	namespace Gnmx
	{
		...
		namespace LightweightConstantUpdateEngine
		{
			...
			*/
			...
			// kMax* constants define how many resources of each available type will be stored and handled by the LCUE, this also defines the maximum valid API-slot.
			// Default values are based on the average expected usage, users are encouraged to modify the defaults to whatever best suits the title.
...
//				uint8_t userInternalSrtDataPtrSgpr;		///< *Note: Not supported for now*.
				...
				uint8_t gdsMemoryRangeSgpr;				///< SGPR containing the GDS address range for storage.
				uint8_t ldsEsGsSizeSgpr;				///< SGPR containing the GWS resource base offset.
...
//				bool streamOutOffsetIsBound[kMaxStreamOutBufferCount];		// No info available, stream-outs are in the VS but there's footer for the VS in a VsGs shader (only GS has a footer).
			...
			public:
				 ...
				 *	@param enable A flag that specifies whether to enable the prefetch of the shader code.
				...
				SCE_GNM_API_DEPRECATED(setGlobalResourceTableAddr(void* globalResourceTableAddr))
				SCE_GNM_FORCE_INLINE void setGlobalInternalResourceTable(void* globalResourceTableAddr) { setGlobalResourceTableAddr(globalResourceTableAddr); }
				...
				SCE_GNM_API_DEPRECATED(setGlobalDescriptor(Gnm::ShaderGlobalResourceType resourceType, const Gnm::Buffer* buffer))
				...
				bool		m_prefetchShaderCode;						// Have the LCUE warm up the L2 Cache by prefetching the shaders in preDraw/preDispatch
				sce::Gnm::Buffer* m_globalInternalResourceTableAddr;	// Cached address to the global resource table, shared among shader stages on a preDraw/preDispatch basis
