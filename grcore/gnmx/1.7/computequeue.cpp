...
{
	SCE_GNM_VALIDATE((uintptr_t(ringBaseAddr)&0xFF)==0, "Ring Base Address (0x%p) must be 256 bytes aligned.", ringBaseAddr);
	SCE_GNM_VALIDATE((uintptr_t(readPtrAddr)&0x3)==0, "Read Ptr Address (0x%p) must be 4 bytes aligned.", readPtrAddr);
