	...
	namespace Gnmx
	{
		...
		*/
		...
		/** @brief
			...
			@param[out] shaderInfo		Receives the shader information. This pointer must not be NULL.
			@param[in] data				A pointer to the contents of an <c>.sb</c> file in memory. This pointer must not be NULL.
		*/
		...
		/** @brief
			...
			@param[out] vsShaderInfo		Receives the information for the GS shader. This pointer must not be NULL.
			@param[out] gsShaderInfo		Receives the information for the VS shader. This pointer must not be NULL.
			@param[in] data				A pointer to the contents of an <c>.sb</c> file in memory. This pointer must not be NULL.
		*/
		...
		/** @brief
			...
			Extracts, from an <c>.sb</c> file, shader information regarding the CS-VS shader as well
			as shader information for its associated CS compute shader.  

			@param[out] csvsShaderInfo	Receives the information for the CS-VS shader. This pointer must not be NULL.
			@param[out] csShaderInfo		Receives the information for the CS shader. This pointer must not be NULL.
			@param[in] data				A pointer to the contents of an <c>.sb</c> file in memory. This pointer must not be NULL.
		*/
