...
void ConstantUpdateEngine::init(void *cpramShadowBuffer, void *heapAddr, uint32_t numRingEntries)
{
	...
	SCE_GNM_VALIDATE(cpramShadowBuffer != NULL, "cpramShadowBuffer must not be NULL.");
	SCE_GNM_VALIDATE((uintptr_t(heapAddr) & 0x3) == 0, "heapAddr (0x%p) must be aligned to a 4-byte boundary.", heapAddr);
	SCE_GNM_VALIDATE(((uintptr_t)cpramShadowBuffer & 0x3) == 0, "cpramShaderBuffer (0x%p) must be aligned to a 4-byte boundary.", cpramShadowBuffer);
	...
	m_pDispatchDrawData        = NULL;
	m_sizeofDispatchDrawData   = 0;
	m_ldsSizeIn512Bytes = 0;
	...
	SCE_GNM_VALIDATE(kPerStageDwordSize*sizeof(uint32_t)*numStages <= computeCpRamShadowSize(), "internal check failed: CPRAM shadow buffer won't fit in CPRAM!"); // total size of all stages' resources must fit in the available CPRAM!
	...
	for(int iStage=0; iStage<Gnm::kShaderStageCount; ++iStage)
	{
		...
		for (uint32_t iTRB=0; iTRB<kChunkCountResource; ++iTRB)
		{
			SCE_GNM_VALIDATE(uintptr_t(freeAddr) <= uintptr_t(heapAddr)+m_heapSize, "freeAddr (0x%p) outside of heap bounds [0x%p-0x%p].", freeAddr, heapAddr, static_cast<uint8_t*>(heapAddr)+m_heapSize-1);
		...
		for (uint32_t iURB=0; iURB<kChunkCountRwResource; ++iURB)
		{
			SCE_GNM_VALIDATE(uintptr_t(freeAddr) <= uintptr_t(heapAddr)+m_heapSize, "freeAddr (0x%p) outside of heap bounds [0x%p-0x%p].", freeAddr, heapAddr, static_cast<uint8_t*>(heapAddr)+m_heapSize-1);
		...
		for (uint32_t iSRB=0; iSRB<kChunkCountSampler; ++iSRB)
		{
			SCE_GNM_VALIDATE(uintptr_t(freeAddr) <= uintptr_t(heapAddr)+m_heapSize, "freeAddr (0x%p) outside of heap bounds [0x%p-0x%p].", freeAddr, heapAddr, static_cast<uint8_t*>(heapAddr)+m_heapSize-1);
		...
		for (uint32_t iVRB=0; iVRB<kChunkCountVertexBuffer; ++iVRB)
		{
			SCE_GNM_VALIDATE(uintptr_t(freeAddr) <= uintptr_t(heapAddr)+m_heapSize, "freeAddr (0x%p) outside of heap bounds [0x%p-0x%p].", freeAddr, heapAddr, static_cast<uint8_t*>(heapAddr)+m_heapSize-1);
		...
		for (uint32_t iCRB=0; iCRB<kChunkCountConstantBuffer; ++iCRB)
		{
			SCE_GNM_VALIDATE(uintptr_t(freeAddr) <= uintptr_t(heapAddr)+m_heapSize, "freeAddr (0x%p) outside of heap bounds [0x%p-0x%p].", freeAddr, heapAddr, static_cast<uint8_t*>(heapAddr)+m_heapSize-1);
		...
		for (uint32_t iSOB=0; iSOB<kChunkCountStreamoutBuffer; ++iSOB)
		{
			SCE_GNM_VALIDATE(uintptr_t(freeAddr) <= uintptr_t(heapAddr)+m_heapSize, "freeAddr (0x%p) outside of heap bounds [0x%p-0x%p].", freeAddr, heapAddr, static_cast<uint8_t*>(heapAddr)+m_heapSize-1);
		...
		for (uint32_t iEUDRB=0; iEUDRB<kChunkCountExtendedUserData; ++iEUDRB)
		{
			SCE_GNM_VALIDATE(uintptr_t(freeAddr) <= uintptr_t(heapAddr)+m_heapSize, "freeAddr (0x%p) outside of heap bounds [0x%p-0x%p].", freeAddr, heapAddr, static_cast<uint8_t*>(heapAddr)+m_heapSize-1);
...
{
	SCE_GNM_VALIDATE(addr != NULL, "addr must not be NULL.");
...
void ConstantUpdateEngine::setTextures(ShaderStage stage, uint32_t startApiSlot, uint32_t numApiSlots, const Texture *textures)
{
	...
	SCE_GNM_VALIDATE(startApiSlot < m_activeResourceSlotCounts[stage], "startApiSlot (%d) must be in the range [0..%d].", startApiSlot, m_activeResourceSlotCounts[stage]-1);
	SCE_GNM_VALIDATE(numApiSlots <= m_activeResourceSlotCounts[stage], "numApiSlots (%d) must be in the range [0..%d].", numApiSlots, m_activeResourceSlotCounts[stage]);
...
void ConstantUpdateEngine::setRwTextures(ShaderStage stage, uint32_t startApiSlot, uint32_t numApiSlots, const Texture *rwTextures)
{
	if (numApiSlots == 0)
		return;
	SCE_GNM_VALIDATE(!m_betweenPreAndPostDraw, "This function must not be called between preDraw() and postDraw() or preDispatch() and postDispatch().");
	SCE_GNM_VALIDATE(startApiSlot < m_activeRwResourceSlotCounts[stage], "startApiSlot (%d) must be in the range [0..%d].", startApiSlot, m_activeRwResourceSlotCounts[stage]-1);
	SCE_GNM_VALIDATE(numApiSlots <= m_activeRwResourceSlotCounts[stage], "numApiSlots (%d) must be in the range [0..%d].", numApiSlots, m_activeRwResourceSlotCounts[stage]);
...
void ConstantUpdateEngine::setSamplers(ShaderStage stage, uint32_t startApiSlot, uint32_t numApiSlots, const Sampler *samplers)
{
	if (numApiSlots == 0)
		return;
	SCE_GNM_VALIDATE(!m_betweenPreAndPostDraw, "This function must not be called between preDraw() and postDraw() or preDispatch() and postDispatch().");
	SCE_GNM_VALIDATE(startApiSlot < m_activeSamplerSlotCounts[stage], "startApiSlot (%d) must be in the range [0..%d].", startApiSlot, m_activeSamplerSlotCounts[stage]-1);
	SCE_GNM_VALIDATE(numApiSlots <= m_activeSamplerSlotCounts[stage], "numApiSlots (%d) must be in the range [0..%d].", numApiSlots, m_activeSamplerSlotCounts[stage]);
...
void ConstantUpdateEngine::setConstantBuffers(ShaderStage stage, uint32_t startApiSlot, uint32_t numApiSlots, const Buffer *buffers)
{
	if (numApiSlots == 0)
		return;
	SCE_GNM_VALIDATE(!m_betweenPreAndPostDraw, "This function must not be called between preDraw() and postDraw() or preDispatch() and postDispatch().");
	SCE_GNM_VALIDATE(startApiSlot < m_activeConstantBufferSlotCounts[stage], "startApiSlot (%d) must be in the range [0..%d].", startApiSlot, m_activeConstantBufferSlotCounts[stage]-1);
	SCE_GNM_VALIDATE(numApiSlots <= m_activeConstantBufferSlotCounts[stage], "numApiSlots (%d) must be in the range [0..%d].", numApiSlots, m_activeConstantBufferSlotCounts[stage]);
...
void ConstantUpdateEngine::setVertexBuffers(ShaderStage stage, uint32_t startApiSlot, uint32_t numApiSlots, const Buffer *buffers)
{
	if (numApiSlots == 0)
		return;
	SCE_GNM_VALIDATE(!m_betweenPreAndPostDraw, "This function must not be called between preDraw() and postDraw() or preDispatch() and postDispatch().");
	SCE_GNM_VALIDATE(startApiSlot < m_activeVertexBufferSlotCounts[stage], "startApiSlot (%d) must be in the range [0..%d].", startApiSlot, m_activeVertexBufferSlotCounts[stage]-1);
	SCE_GNM_VALIDATE(numApiSlots <= m_activeVertexBufferSlotCounts[stage], "numApiSlots (%d) must be in the range [0..%d].", numApiSlots, m_activeVertexBufferSlotCounts[stage]);
...
void ConstantUpdateEngine::setStreamoutBuffers(uint32_t startApiSlot, uint32_t numApiSlots, const Buffer *buffers)
{
	...
	if (numApiSlots == 0)
		return;
	SCE_GNM_VALIDATE(!m_betweenPreAndPostDraw, "This function must not be called between preDraw() and postDraw() or preDispatch() and postDispatch().");
	SCE_GNM_VALIDATE(startApiSlot < m_activeStreamoutBufferSlotCount, "startApiSlot (%d) must be in the range [0..%d].", startApiSlot, m_activeStreamoutBufferSlotCount-1);
	SCE_GNM_VALIDATE(numApiSlots <= m_activeStreamoutBufferSlotCount, "numApiSlots (%d) must be in the range [0..%d].", numApiSlots, m_activeStreamoutBufferSlotCount);
...
void ConstantUpdateEngine::setFloatConstants(ShaderStage stage, uint32_t startApiSlot, uint32_t numApiSlots, const float *floats)
{
	if (numApiSlots == 0)
		return;
	SCE_GNM_VALIDATE(!m_betweenPreAndPostDraw, "This function must not be called between preDraw() and postDraw() or preDispatch() and postDispatch().");
	SCE_GNM_VALIDATE(startApiSlot < Gnm::kSlotCountFloatConstant, "startApiSlot (%d) must be in the range [0..%d].", startApiSlot, Gnm::kSlotCountFloatConstant-1);
	SCE_GNM_VALIDATE(numApiSlots <= Gnm::kSlotCountFloatConstant, "numApiSlots (%d) must be in the range [0..%d].", numApiSlots, Gnm::kSlotCountFloatConstant);
...
void ConstantUpdateEngine::setAppendConsumeCounterRange(ShaderStage stage, uint32_t baseOffsetInBytes, uint32_t rangeSizeInBytes)
{
	...
	SCE_GNM_VALIDATE(baseOffsetInBytes < kGdsAccessibleMemorySizeInBytes, "baseOffsetInBytes (%d) must be less than %i.", baseOffsetInBytes, kGdsAccessibleMemorySizeInBytes);
	...
	SCE_GNM_VALIDATE((baseOffsetInBytes+rangeSizeInBytes) <= kGdsAccessibleMemorySizeInBytes, "baseOffsetInBytes (%d) + rangeSizeInBytes (%d) must not exceed %i.", baseOffsetInBytes, rangeSizeInBytes, kGdsAccessibleMemorySizeInBytes);
...
void ConstantUpdateEngine::setGdsMemoryRange(ShaderStage stage, uint32_t baseOffsetInBytes, uint32_t rangeSizeInBytes)
{
	SCE_GNM_VALIDATE(!m_betweenPreAndPostDraw, "This function must not be called between preDraw() and postDraw() or preDispatch() and postDispatch().");
	SCE_GNM_VALIDATE(((baseOffsetInBytes | rangeSizeInBytes)&3) == 0, "baseOffsetInBytes (%d) as well as baseOffsetInBytes (%d) must be multiple of 4s.", baseOffsetInBytes, rangeSizeInBytes);
	SCE_GNM_VALIDATE(baseOffsetInBytes < kGdsAccessibleMemorySizeInBytes, "baseOffsetInBytes (%d) must be less than %i.", baseOffsetInBytes, kGdsAccessibleMemorySizeInBytes);
	...
	uint16_t dwordOffset = getGdsMemoryRangeDwordOffset(stage, 0);
	const uint32_t ringBasedOffset = baseOffsetInBytes;
	m_cpRamShadowDwords[dwordOffset] = (ringBasedOffset << 16) | rangeSizeInBytes;
}
...
void ConstantUpdateEngine::setVsShader(const VsShader *vsb, uint32_t shaderModifier, void *fetchShaderAddr)
{
	...
	if (vsb != NULL)
	{
		...
		{
			SCE_GNM_ERROR("VsShader vsb [0x%p] expects a fetch shader, but fetchShaderAddr==0.", vsb);
			return;
		}
	}
#endif
...
void ConstantUpdateEngine::setCsVsShaders(const CsVsShader *csvsb, uint32_t shaderModifierVs, void *fetchShaderAddrVs, uint32_t shaderModifierCs, void *fetchShaderAddrCs)
{
	...
	SCE_GNM_VALIDATE(m_currentAcbCSB->m_common.m_embeddedConstantBufferSizeInDQW == 0 || m_currentAcbCSB->m_common.m_embeddedConstantBufferSizeInDQW == csvsb->getVertexShader()->m_common.m_embeddedConstantBufferSizeInDQW, "CsVsShader csvsb [0x%p] asynchronous compute shader embedded constant buffer does not match VS shader embedded constant buffer.", csvsb);
}
...
void ConstantUpdateEngine::setAsynchronousComputeShader(const CsShader *csb, uint32_t shaderModifierCs, void *fetchShaderAddrCs)
{
	...
	if (csb != NULL)
	{
		...
		{
			SCE_GNM_ERROR("AsynchronousComputeShader csb [0x%p] shader expects a fetch shader, but fetchShaderAddrCs==0.", csb);
			return;
		}
	}
#endif
...
void ConstantUpdateEngine::setLsShader(const LsShader *lsb, uint32_t shaderModifier, void *fetchShaderAddr, const LsStageRegisters *lsRegs)
{
	SCE_GNM_VALIDATE(!m_betweenPreAndPostDraw, "This function must not be called between preDraw() and postDraw() or preDispatch() and postDispatch().");
	...
	if (lsb != NULL)
	{
		...
		{
			SCE_GNM_ERROR("LsShader lsb [0x%p] expects a fetch shader, but fetchShaderAddr=0.", lsb);
			return;
		}
	}
#endif
	...
	if (lsb->m_common.m_embeddedConstantBufferSizeInDQW)
	{
		...
		void *shaderAddr = (void*)( ((uintptr_t)lsRegs->m_spiShaderPgmHiLs << 32) + lsRegs->m_spiShaderPgmLoLs );
...
{
	setLsShader(lsb, shaderModifier, fetchShaderAddr, &lsb->m_lsStageRegisters);
}

void ConstantUpdateEngine::setEsShader(const EsShader *esb, uint32_t shaderModifier, void *fetchShaderAddr)
{
	return setOnChipEsShader(esb, 0, shaderModifier, fetchShaderAddr);
}
void ConstantUpdateEngine::setOnChipEsShader(const sce::Gnmx::EsShader *esb, uint32_t ldsSizeIn512Bytes, uint32_t shaderModifier, void *fetchShaderAddr)
{
	...
	if (esb != NULL)
	{
		...
		{
			SCE_GNM_ERROR("EsShader esb [0x%p] expects a fetch shader, but fetchShaderAddr=0.", esb);
			return;
		}
	}
#endif
	...
	m_ldsSizeIn512Bytes = ldsSizeIn512Bytes;
...
{
	SCE_GNM_VALIDATE(res != NULL, "res must not be NULL.");
...
void ConstantUpdateEngine::invalidateAllBindings(void)
{
	...
	setVsShader(NULL, 0, NULL);
	setPsShader(NULL);
	setEsShader(NULL, 0, NULL);
	setGsVsShaders(NULL);
	setCsShader(NULL);
	setLsShader(NULL, 0, NULL);
	setHsShader(NULL, NULL);
	setCsVsShaders(NULL, 0, NULL, 0, NULL);
	setAsynchronousComputeShader(NULL, 0, NULL);

	// Mark all of CPRAM as dirty, and clear the old contents
	memset(m_cpRamShadowDwords, 0, computeCpRamShadowSize());
	memset(m_dirtyResourceChunkBits,         0xFF, sizeof(m_dirtyResourceChunkBits        ));
	memset(m_dirtyRwResourceChunkBits,       0xFF, sizeof(m_dirtyRwResourceChunkBits      ));
	memset(m_dirtySamplerChunkBits,          0xFF, sizeof(m_dirtySamplerChunkBits         ));
	memset(m_dirtyVertexBufferChunkBits,     0xFF, sizeof(m_dirtyVertexBufferChunkBits    ));
	memset(m_dirtyConstantBufferChunkBits,   0xFF, sizeof(m_dirtyConstantBufferChunkBits  ));
	memset(m_dirtyStreamoutBufferChunkBits,  0xFF, sizeof(m_dirtyStreamoutBufferChunkBits ));
	memset(m_dirtyExtendedUserDataChunkBits, 0xFF, sizeof(m_dirtyExtendedUserDataChunkBits));
	memset(m_dirtyResourceSlotBits,          0xFF, sizeof(m_dirtyResourceSlotBits        ));
	memset(m_dirtyRwResourceSlotBits,        0xFF, sizeof(m_dirtyRwResourceSlotBits      ));
	memset(m_dirtySamplerSlotBits,           0xFF, sizeof(m_dirtySamplerSlotBits         ));
	memset(m_dirtyVertexBufferSlotBits,      0xFF, sizeof(m_dirtyVertexBufferSlotBits    ));
	memset(m_dirtyConstantBufferSlotBits,    0xFF, sizeof(m_dirtyConstantBufferSlotBits  ));
	memset(m_dirtyStreamoutBufferSlotBits,   0xFF, sizeof(m_dirtyStreamoutBufferSlotBits ));
	memset(m_dirtyExtendedUserDataSlotBits,  0xFF, sizeof(m_dirtyExtendedUserDataSlotBits));
}
...
void ConstantUpdateEngine::updateCpRam(ConstantCommandBuffer *ccb, ShaderStage stage, const InputUsageSlot *usageSlots, const uint32_t numInputSlots, void *outUsageChunkAddrs[16])
{
	...
	for(uint32_t iUsage=0, iUsageMax=numInputSlots; iUsage<iUsageMax; ++iUsage)
	{
		...
		case Gnm::kShaderInputUsageImmGdsMemoryRange:
			if (dstUserDataSlot >= 16 && m_dirtyExtendedUserDataSlotBits[stage][0])
				// immediate is in ExtendedUserData; copy it into the shadow-CPRAM EUD chunk.
					setDwordInExtendedUserData(stage, dstUserDataSlot, m_cpRamShadowDwords[getGdsMemoryRangeDwordOffset(stage, srcApiSlot)]);
			break;
		...
		case Gnm::kShaderInputUsageImmLdsEsGsSize:
			if (dstUserDataSlot >= 16 && m_dirtyExtendedUserDataSlotBits[stage][0])
				// immediate is in ExtendedUserData; copy it into the shadow-CPRAM EUD chunk.
				setDwordInExtendedUserData(stage, dstUserDataSlot, m_onChipEsVertsPerSubGroup * m_onChipEsExportVertexSizeInDword);
			break;
		case Gnm::kShaderInputUsageSubPtrFetchShader:
...
void ConstantUpdateEngine::applyConstantsForDraw(DrawCommandBuffer *dcb, ShaderStage stage, const InputUsageSlot *usageSlots, const uint32_t numInputSlots, void *usageChunkAddrs[16])
{
	...
	for(uint32_t iUsage=0, iUsageMax=numInputSlots; iUsage<iUsageMax; ++iUsage)
	{
		...
		case Gnm::kShaderInputUsageImmGdsMemoryRange:
			if (dstUserDataSlot < 16)
			{
				dcb->setUserData(stage, dstUserDataSlot, m_cpRamShadowDwords[getGdsMemoryRangeDwordOffset(stage, srcApiSlot)]);
			}
			break;
		case Gnm::kShaderInputUsageImmGwsBase:
			SCE_GNM_ERROR("Unsupported shader input usage type 0x%02X.", usageSlots[iUsage].m_usageType);
		...
		case Gnm::kShaderInputUsageImmLdsEsGsSize:
			if (dstUserDataSlot < 16)
			{
				dcb->setUserData(stage, dstUserDataSlot, m_onChipEsVertsPerSubGroup * m_onChipEsExportVertexSizeInDword);
			}
			break;
		case Gnm::kShaderInputUsageSubPtrFetchShader:
			// TODO: For now, fetch shaders are set manually by application and not by this class
		...
		case Gnm::kShaderInputUsagePtrDispatchDraw:
			...
			SCE_GNM_ASSERT(m_pDispatchDrawData != 0);
			dcb->setPointerInUserData(stage, dstUserDataSlot, (void*)m_pDispatchDrawData);
			break;
		case Gnm::kShaderInputUsageImmDispatchDrawInstances:
			//nothing to do
			break;
...
void ConstantUpdateEngine::applyConstantsForDispatchDrawAcb(DispatchCommandBuffer *acb, const InputUsageSlot *usageSlots, const uint32_t numInputSlots, void *usageChunkAddrs[16])
{
	...
	for(uint32_t iUsage=0, iUsageMax=numInputSlots; iUsage<iUsageMax; ++iUsage)
	{
		...
		case Gnm::kShaderInputUsageImmGdsMemoryRange:
			if (dstUserDataSlot < 16)
			{
				acb->setUserData(dstUserDataSlot, m_cpRamShadowDwords[getGdsMemoryRangeDwordOffset(stage, srcApiSlot)]);
			}
			break;
		case Gnm::kShaderInputUsageImmGwsBase:
			SCE_GNM_ERROR("Unsupported shader input usage type 0x%02X.", usageSlots[iUsage].m_usageType);
		...
		case Gnm::kShaderInputUsagePtrDispatchDraw:
			SCE_GNM_ASSERT(dstUserDataSlot + 2 <= 16); // DDD pointers can't be stored in EUD
			SCE_GNM_ASSERT(m_pDispatchDrawData != 0);
			acb->setPointerInUserData(dstUserDataSlot, (void*)m_pDispatchDrawData);
			break;
		case Gnm::kShaderInputUsageImmDispatchDrawInstances:
			//nothing to do
			break;
...
void ConstantUpdateEngine::preDraw(DrawCommandBuffer *dcb, ConstantCommandBuffer *ccb)
{
	...
	{
		dcb->setLsShader(&m_currentLsRegs, m_currentLsShaderModifier);
	...
	{
		if (m_ldsSizeIn512Bytes != 0)
		{
			Gnm::EsStageRegisters esRegsCopy = m_currentESB->m_esStageRegisters; // m_currentESB is read-only
			esRegsCopy.updateLdsSize(m_ldsSizeIn512Bytes);
			dcb->setEsShader(&esRegsCopy, m_currentEsShaderModifier);
		}
		else
		{
...
void ConstantUpdateEngine::preDispatch(DrawCommandBuffer *dcb, ConstantCommandBuffer *ccb)
{
	...
	SCE_GNM_VALIDATE(dcb, "dcb must not be NULL.");
	SCE_GNM_VALIDATE(ccb, "ccb must not be NULL.");
	SCE_GNM_VALIDATE(m_currentCSB, "m_currentCSB must not be NULL.");
	...
	{
		dcb->setCsShader(&m_currentCSB->m_csStageRegisters);
	}
	// Don't clear shader dirty bits until after the calls to applyConstantsForDraw()
...
void ConstantUpdateEngine::preDispatchDraw(DrawCommandBuffer *dcb, DispatchCommandBuffer *acb, ConstantCommandBuffer *ccb,
	DispatchOrderedAppendMode *pOutOrderedAppendMode, uint32_t *pOutDispatchDrawIndexDeallocationMask, uint32_t *pOutSgprKrbLoc, uint32_t *pOutSgprInstanceCs,
	DispatchDrawMode *pOutDispatchDrawMode, uint32_t *pOutSgprVrbLoc, uint32_t *pOutSgprInstanceVs)
{
	m_betweenPreAndPostDraw = true;
	...
	if (m_currentAcbCSB != NULL)
	{
		...
		*pOutSgprKrbLoc = (uint32_t)-1;
		...
		for (uint32_t iUsage = 0, iUsageMax = m_currentAcbCSB->m_common.m_numInputUsageSlots; iUsage < iUsageMax && !(bFoundImmGdsKickRingBufferOffset && bFoundImmDispatchDrawInstances); ++iUsage) {
			if (inputUsageSlots[iUsage].m_startRegister >= 16)
				break;
			if (!bFoundImmGdsKickRingBufferOffset && inputUsageSlots[iUsage].m_usageType == Gnm::kShaderInputUsageImmGdsKickRingBufferOffset) {
				*pOutSgprKrbLoc = inputUsageSlots[iUsage].m_startRegister;
				bFoundImmGdsKickRingBufferOffset = true;
			}
			if (!bFoundImmDispatchDrawInstances && inputUsageSlots[iUsage].m_usageType == Gnm::kShaderInputUsageImmDispatchDrawInstances) {
				if (pOutSgprInstanceCs != NULL)
					*pOutSgprInstanceCs = inputUsageSlots[iUsage].m_startRegister;
				else
					acb->setUserData(inputUsageSlots[iUsage].m_startRegister, 0);
				bFoundImmDispatchDrawInstances = true;
			}
		}
		*pOutOrderedAppendMode = (Gnm::DispatchOrderedAppendMode)m_currentAcbCSB->m_orderedAppendMode;
		*pOutDispatchDrawIndexDeallocationMask = 0x0000FFFF & (0xFFFF << m_currentAcbCSB->m_dispatchDrawIndexDeallocNumBits);
		...
		SCE_GNM_VALIDATE(m_currentAcbCSB != NULL, "dispatchDraw expects an asynchronous compute shader to be set by setCsVsShaders or setAsynchronousComputeShader");
	}
	if (m_currentVSB != NULL)
	{
		void *usageChunkAddrs[16] = {0};
		...
		*pOutSgprVrbLoc = (uint32_t)-1;
		...
		for (uint32_t iUsage = 0, iUsageMax = m_currentVSB->m_common.m_numInputUsageSlots; iUsage < iUsageMax && !(bFoundImmVertexRingBufferOffset && bFoundImmDispatchDrawInstances); ++iUsage) {
			if (inputUsageSlots[iUsage].m_startRegister >= 16)
				break;
			if (!bFoundImmVertexRingBufferOffset && inputUsageSlots[iUsage].m_usageType == Gnm::kShaderInputUsageImmVertexRingBufferOffset) {
				*pOutSgprVrbLoc = inputUsageSlots[iUsage].m_startRegister;
				bFoundImmVertexRingBufferOffset = true;
			}
			if (!bFoundImmDispatchDrawInstances && inputUsageSlots[iUsage].m_usageType == Gnm::kShaderInputUsageImmDispatchDrawInstances) {
				if (pOutSgprInstanceVs != NULL)
					*pOutSgprInstanceVs = inputUsageSlots[iUsage].m_startRegister;
				else
					dcb->setUserData(Gnm::kShaderStageVs, inputUsageSlots[iUsage].m_startRegister, 0);
				bFoundImmDispatchDrawInstances = true;
			}
		}
		*pOutDispatchDrawMode = bFoundImmVertexRingBufferOffset ? Gnm::kDispatchDrawModeIndexAndVertexRingBuffer : Gnm::kDispatchDrawModeIndexRingBufferOnly;
		...
		SCE_GNM_VALIDATE(m_currentVSB != NULL, "dispatchDraw expects a vertex shader to be set by setCsVsShaders or setVsShader");
	}
	if (m_currentPSB != NULL)
	{
		void *usageChunkAddrs[16] = {0};
	...
	// explicitly prefetch the dispatch draw data into L2 for the compute shaders:
	if (m_pDispatchDrawData)
		acb->prefetchIntoL2((void*)m_pDispatchDrawData, m_sizeofDispatchDrawData);
...
uint16_t ConstantUpdateEngine::getGdsMemoryRangeDwordOffset(ShaderStage stage, uint32_t slot) const
{
	SCE_GNM_VALIDATE(slot < Gnm::kSlotCountGdsMemoryRange, "slot (%d) must be in the range [0..%d].", slot, Gnm::kSlotCountGdsMemoryRange-1);
	uint32_t stageOffset = stage*kPerStageDwordSize;
	uint32_t typeOffset = kDwordOffsetGdsMemoryRange;
	return (uint16_t)(stageOffset + typeOffset + (slot*Gnm::kDwordSizeGdsMemoryRange));
}
...
void ConstantUpdateEngine::setTsharpInExtendedUserData(ShaderStage stage, uint8_t userDataSlot, const Texture *tex)
{
	...
	//SCE_GNM_VALIDATE(tex->isTexture(), "tex (0x%p) is invalid (isTexture() returned false).", tex);
...
{
	//Not necessarily valid. See: https://ps4.scedev.net/support/issue/22117
	//SCE_GNM_VALIDATE(buffer->isBuffer(), "buffer (0x%p) is invalid (isBuffer() returned false).", buffer);
